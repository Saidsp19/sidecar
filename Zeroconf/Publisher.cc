...
Publisher::publish(const std::string& name, bool noRename)
{
    ...
    LOGINFO << "Registering " << name << "." << type_ << " " << domain_ << " " << port_ << ' ' << noRename
            << std::endl;
    ...
    name_ = name;
    while (1) {
	auto err = ::DNSServiceRegister(&getDNSServiceRef(), noRename ? kDNSServiceFlagsNoAutoRename : 0,
                                        interface_, name_.size() ? name_.c_str() : 0, type_.c_str(),
                                        domain_.size() ? domain_.c_str() : 0, 0, htons(port_), buffer.size(),
                                        buffer.c_str(), &Publisher::RegisterCallback, this);
