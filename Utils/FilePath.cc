...
FilePath::FilePath(const std::string& dir, const std::string& file) : filePath_(dir)
{
    if (dir.size() == 0 || dir.rfind('/') != dir.size() - 1) { filePath_ += '/'; }
    filePath_ += file;
}
...
{
    struct stat status;
    return (::stat(filePath_.c_str(), &status) == 0 &&
            ((status.st_mode & S_IFMT) == S_IFREG || (status.st_mode & S_IFMT) == S_IFCHR));
}
...
FilePath::relocate(const std::string& path)
{
    ...
    if (newPath.size() && newPath[newPath.size() - 1] != '/') { newPath += '/'; }
...
FilePath::getModificationTime() const
{
    ...
    if (rc == -1) {
        Utils::Exception ex("FilePath::getModificationTime: ");
...
ConfigFilePath::locate()
{
    if (!exists()) {
        ...
        if (!exists()) { std::cerr << "ConfigFilePath: file '" << filePath() << "' does not exist\n"; }
    }
}

TemporaryFilePath::TemporaryFilePath() : filePath_(""), fd_(-1)
{
...
TemporaryFilePath::TemporaryFilePath(const std::string& path, bool clear) : filePath_(path), fd_(-1)
{
