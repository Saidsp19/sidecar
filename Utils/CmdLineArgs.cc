...
CmdLineArgs::CmdLineArgs(size_t argc, char* const* argv, const std::string& description,
                         const OptionDefVector& optDefs, const ArgumentDefVector& argDefs, bool quitOnError)
    : argc_(argc), argv_(argv), fullPath_(*argv), progName_(), description_(description), optDefs_(optDefs),
      argDefs_(argDefs), opts_(), args_(), cmdline_(""), quitOnError_(quitOnError)
{
    initialize();
}

CmdLineArgs::CmdLineArgs(size_t argc, char* const* argv, const std::string& description,
                         const OptionDef* optDefs, size_t sizeofOptDefs, const ArgumentDef* argDefs,
                         size_t sizeofArgDefs, bool quitOnError)
    : argc_(argc), argv_(argv), fullPath_(*argv), progName_(), description_(description), optDefs_(), argDefs_(),
...
struct PrintSummaryArg
{
    ...
    */
    PrintSummaryArg(std::string& l)
	: line_(l), optional_(false) {}
...
CmdLineArgs::printUsage() const
{
    ...
    if (std::for_each(argDefs_.begin(), argDefs_.end(), PrintSummaryArg(line))) {
	line += "]";
    }
...
CmdLineArgs::verifyOptionDef(char opt) const
{
    auto pos = std::find_if(optDefs_.begin(), optDefs_.end(), [opt](auto& def){return opt == def.shortName;});
    if (pos == optDefs_.end()) {
	throw std::logic_error(std::string("verifyOptionDef: invalid option - ") + opt);
    }
}
...
CmdLineArgs::processShortOption(const std::string& opt)
{
	...
	if (def.valueTag == 0 || def.valueTag[0] == 0) {
	    error("cannot assign value to option", c);
        }
