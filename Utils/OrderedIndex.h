...
public:
    ...
    OrderedIndex(const std::vector<T>& init)
	: _entries(), _ordered(true)
    ...
    void clear() { _entries.clear(); _ordered = true; }
    ...
    const_iterator begin() const { sortCheck(); return _entries.begin(); }
    const_iterator end() const { sortCheck(); return _entries.end(); }

    const_reverse_iterator rbegin() const { sortCheck(); return _entries.rbegin(); }
    const_reverse_iterator rend() const { sortCheck(); return _entries.rend(); }
    ...
    void erase(const_iterator pos)
	{
	    ...
	    std::for_each(_entries.begin(), _entries.end(), [index](auto& e){if (e.index > index) --e.index;});
	}

private:
    void sortCheck() const { if (! _ordered) const_cast<OrderedIndex*>(this)->sort(); }
