...
double
Utils::el_from_range_and_alt(double range, double altitude, double refLatitudeRadians)
{
    if (range < altitude) return 0;
    ...
    double ELEVATION =
        (::acos((CURVE_RADIUS * CURVE_RADIUS + range * range - HEIGHT * HEIGHT) / (2.0 * CURVE_RADIUS * range))) -
        (M_PI / 2.0);
    return ELEVATION;
}
