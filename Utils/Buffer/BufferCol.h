...
public:
    ...
    */
    ...
    /**
       ...
       the underlying Buffer without changing columns.
    */
    void flush();
...
BufferCol<T>::BufferCol(const Buffer<T>& b, size_t col, bool flush) :
    buf(b), dense(vsip::Domain<1>(buf.length), bufIndex(0)), allvec(dense),
    subview(vsip::Domain<1>(col, buf.cols, buf.dr), allvec.block()), v(subview), col(col), doflush(flush)
{
    assert(col < buf.dc);
    dense.admit(true);
}
