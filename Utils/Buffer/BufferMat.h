...
/**
   ...
   - Matrix(Dense&) needs the Dense to be constructed *before* the Matrix, but the Dense should be created and destroyed
   by the BufferMat.
   - the overloaded operator= causes problems

   As illustrated in \ref fig1 "Figure 1", the BufferMat provides a subview of a parent Buffer object.  Unlike the
   strict alignment of BufferRow and BufferCol within the parent Buffer, BufferMat is somewhat free to move around.  The
   only requirements are:
   - the maximum size of (rows, cols) be able to fit within the parent Buffer's data dimensions of
   (Buffer::getDataRows(), Buffer::getDataCols())
   ...
   As with the BufferRow, the row indices of BufferMat are taken modulo b.dr to allow for a rolling window into the
   data.

   \anchor fig1
   \image html BufferMat.png "Placement of a BufferMat within a Buffer"

   Only matrices up to size (Buffer::getWindowRows(), Buffer::getWindowRows()) are guaranteed to be within the Buffer;
   lager matrices may fall outside the allocated space depending on their initial row and column within the parent
   Buffer.
*/
template <class T>
...
public:
    ...
    /**
       ...
       \param col Index of the first column in the parent Buffer.   Limited to the range - Buffer::getWindowCols() < col
       < Buffer::getDataCols()
    ...
    */
    ...
    /**
       ...
       the underlying Buffer without changing the row or column.
    */
...
private:
    ...
    inline T* bufIndex(size_t r, int c)
    {
        ...
        if (index < 0) index += buf.dr * buf.cols;
...
private:
    ...
    //
    void pause() { internalflush(false); }
    void resume()
    {
...
BufferMat<T>::BufferMat(Buffer<T>& b, size_t rows, size_t cols, size_t row, int col, bool flush) :
    BufferView(b), buf(b), rows(rows == 0 ? buf.dr : rows), cols(cols == 0 ? buf.dc : cols), row(row), col(col),
    dense(vsip::Domain<2>(buf.rows, buf.cols), bufIndex(this->row, this->col)), allmat(dense),
    subview(vsip::Domain<2>(this->rows, this->cols), allmat.block()), m(subview), doflush(flush)
{
...
template <class T>
void
BufferMat<T>::setStart(size_t row, int col)
{
    if (doflush) {
        internalflush(false);
    } else {
...
template <class T>
void
BufferMat<T>::internalflush(bool rebind)
{
