...
IO::Stream::Ref
StreamBuilder::Make(const QDomElement& stream, const StatusEmitter::Ref& statusEmitter, const std::string& mcastAddress)
{
...
StreamBuilder::release()
{
    ...
    //
    for (std::vector<IO::Module*>::reverse_iterator pos = modules_.rbegin(); pos != modules_.rend(); ++pos) {
        if (needShutdownMonitor_) {
...
StreamBuilder::registerOutputs(const QDomElement& xml, IO::Task::Ref task)
{
    ...
    while (!output.isNull()) {
        ...
        registerOutput(task, type, output.attribute("name").toStdString(), output.attribute("channel").toStdString());
        output = output.nextSiblingElement("output");
    }
}

void
StreamBuilder::registerOutput(IO::Task::Ref task, const std::string& type, std::string name, std::string channelName)
{
    Logger::ProcLog log("registerOutput", Log());
    size_t taskIndex = task->getTaskIndex();
    LOGINFO << taskIndex << " type: " << type << " name: " << name << " channelName: " << channelName << std::endl;
...
StreamBuilder::connectInputs(const QDomElement& xml, IO::Task::Ref task)
{
    ...
    while (!input.isNull()) {
        ...
        connectInput(task, type, input.attribute("name").toStdString(), input.attribute("channel").toStdString());
...
StreamBuilder::connectInput(IO::Task::Ref task, std::string& type, std::string name, std::string channelName)
{
    ...
    if (!type.size()) { type = pos->second.getTypeName(); }
...
StreamBuilder::makeAlgorithm(const QDomElement& xml)
{
    ...
    if (xml.hasAttribute(kThreaded)) {
        ...
        if (tmp == "false" || tmp == "0") threaded = false;
    }
    ...
    if (!controller->openAndInit(dll.toStdString(), name.toStdString(), 0, threadFlags, threadPriority, threaded)) {
    ...
    if (!param.isNull()) {
        ...
        os << "<value><array><data>";
        do {
            os << "<value><string>" << param.attribute("name") << "</string></value><value><" << param.attribute("type")
               << ">" << param.attribute("value") << "</" << param.attribute("type") << "></value>";
...
StreamBuilder::makeFileReader(const QDomElement& xml)
{
    ...
    if (signalEndOfFile) { needShutdownMonitor_ = true; }
}
...
StreamBuilder::makeFileWriter(const QDomElement& xml)
{
    ...
    //
    if (writer->getNumInputChannels() == 0) { connectInput(writer, type, "", xml.attribute("channel").toStdString()); }
...
StreamBuilder::makeDataPublisher(const QDomElement& xml)
{
    ...
    if (!transport.size()) { transport = "multicast"; }
...
void
StreamBuilder::makeMulticastDataPublisher(const QDomElement& xml, const std::string& name, const std::string& type,
                                          uint32_t interface)
{
    ...
    if (interface) { publisher->setInterface(interface); }
...
void
StreamBuilder::makeMulticastDataSubscriber(const QDomElement& xml, const std::string& name, const std::string& type,
                                           uint32_t interface)
{
