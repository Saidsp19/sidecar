    ...
    : configurationName_(loader.getConfigurationName()), cfg_(cfg), serviceName_(), logPath_(), remoteCommand_()
{
    ...
    LOGINFO << "runnerName: " << cfg_.name << " hostName: " << cfg_.host << " multicastAddress: "
            << cfg_.multicastAddress << " scheduler: " << cfg_.scheduler << " priority: " << cfg_.priority 
	    << " cpuAffinity: " << cfg_.cpuAffinity << std::endl;
