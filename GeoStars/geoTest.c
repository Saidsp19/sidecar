/*!
\file geoTest.c
\brief This file contains test routines to test out some of the aspects
of geoPoint and geoAstro, namely the point position and the sun track
portions of the code. There are three test programs in geoTest.c. They
...
Here are the test routine's definitions:
  - \b GEO_TEST_MAIN - This routine tests most of the geoPoint.c  and geoAstro.c routines. It is a command line program that take LLH in DMS as parameters.
  - \b GEO_TEST_ASTRO - this tests the geoAstro.c sun position routines. It has two subtests:
      -# \b GEO_TEST_ASTRO_NOWTEST which gives the sun position for the next 300 seconds.
      -# \b GEO_TEST_ASTRO_DAYTEST which gives a table of hourly sun positions for today
...
// geoAstro.c tests
//#define GEO_TEST_ASTRO 1
//#define GEO_TEST_ASTRO_NOWTEST 1
//#define GEO_TEST_ASTRO_DAYTEST 1
...
*/
   ...
   int i;
    ...
    // Lat = 31-17-32.1  Lon = 85-51-03.8  Hgt = 134 m
    /* Build the location descriptors */
    geoInitLocation(&loc, GEO_TEST_LAT,
                          GEO_TEST_LON,
                          GEO_TEST_HGT, GEO_TEST_DATUM,  "Pointing Site");

    // Use this test to demonstrate the current sun
    // position once a second for the next 300 seconds.
    // (control-c will end it)
#ifdef GEO_TEST_ASTRO_NOWTEST
    time( &ltime );
    newtime = gmtime( &ltime );
    for(i=0;i<300;i++)  // do this for 300 seconds
    {
	  /* Obtain time every second */
          while(newtime->tm_sec == old_time)
          {
             time( &ltime );
             newtime = gmtime( &ltime );
          }
          old_time=newtime->tm_sec;

          /* Get the Sun position */
          geoSunAzElNow(&loc, &az, &el);
	  printf("Az=%3.2f El=%3.2f\r",az,el);
   }
#endif

    // Use this test to get an hourly sun position for today.
#ifdef GEO_TEST_ASTRO_DAYTEST
    time( &ltime );
    newtime = gmtime( &ltime );

    for(i=0;i<24;i++)  // do this for each hour of the day
    {
       newtime->tm_hour = i;
       geoSunAzEltm(&loc, &az, &el, newtime);
       printf("Hr=%d Az=%3.2f El=%3.2f\n",i,az,el);
    }
#endif
