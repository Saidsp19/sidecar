...
int DLL_CALLCONV geoInitLocation(GEO_LOCATION *l, double lat, double lon, double hgt, int datum,  const char *name)
{
...
//    time_t aclock;
    ...
    l->e = (N + hgt) * l->clonclat;  //l->clat * l->clon;
    l->f = (N + hgt) * l->slonclat;  //l->clat * l->slon;
    l->g = (N * (l->m1e2) + hgt) * l->slat;

    /* Compute the Magnetic Declination */
	//time( &aclock );                 /* Get time in seconds */
    //newtime = localtime( &aclock );  /* Convert time to struct tm form */
	//geoMagGetDec(lat,lon,hgt,newtime->tm_mon+1, newtime->tm_mday, newtime->tm_year, &dec); 
    //l->Declination = dec;
    geoMagFillDec(l,&dec);
