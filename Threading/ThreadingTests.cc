...
struct WriteThread : public Threading::Thread {
    WriteThread(std::ostream& os, const char* tag) : Threading::Thread(), os_(os), tag_(tag) { ++alive_; }
...
Tests::testGuard()
{
    ...
    while (WriteThread::alive_ != WriteThread::dead_) WriteThread::condition_->waitForSignal();
...
Tests::testThreads()
{
    ...
    while (Thready::alive_ != Thready::dead_) Thready::condition_->waitForSignal();
...
Threading::Condition::Ref SpinThread::condition_ = Threading::Condition::Make();
...
SpinThread::run()
{
    ...
    while (isRunning()) { pthread_testcancel(); }
}
...
{
    {
        ...
        //
        Threading::Locker lock(SpinThread::condition_);
        while (SpinThread::alive_ != 3) SpinThread::condition_->waitForSignal();
    ...
    //
    Threading::Locker lock(SpinThread::condition_);
    while (SpinThread::alive_ != 0) SpinThread::condition_->waitForSignal();
