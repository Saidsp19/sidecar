...
MyIntValueMonitor::MyIntValueMonitor(MyIntValue::Ref& value)
    : Parameter::Monitor<MyIntValue>(value), timesChanged_(0)
{
    ;
}
...
MonitorTest::MonitorTest()
    : TestObj("Monitor"),
      value1_(MyIntValue::Make("one", "one test", 1)),
      value2_(MyIntValue::Make("two", "two test", 2)),
      monitor1_(value1_), monitor2_(value2_), monitor3_(), monitor4_(),
