...
TimeStampRange::Make(const std::string& spec)
{
    ...
    if (is.eof()) {
	return TimeStampRange(TimeStamp::Min(), TimeStamp(s));
    }
	...
	return TimeStampRange(TimeStamp(s), TimeStamp::Max());
    }
...
TimeStampRange::TimeStampRange(const TimeStamp& start, const TimeStamp& end)
