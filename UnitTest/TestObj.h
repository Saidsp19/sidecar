...
public:
    ...
    */
	...
	{
	    if (TypeTraits<T>::areEqual(expected, actual)) {
		throwError("did not expect", TypeTraits<T>::toString(expected), TypeTraits<T>::toString(actual),
                           file, line);
	    }
	}
	...
	\param line line number of the file where the assertion was made
    */
    void _assertNotEqual(std::string expected, std::string actual, const char* file, int line)
	const throw(UnitTestException)
...
private:
    ...
    */
	...
	\param line line of the file that the test is found on
    */
    void throwError(const char* tag, double expected, double actual, double delta, double epsilon,
