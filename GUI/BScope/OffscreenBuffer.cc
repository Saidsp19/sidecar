...
OffscreenBuffer::OffscreenBuffer(const SampleImaging* imaging,
                                 ViewSettings* viewSettings, int width,
                                 int height, int textureType)
    : Super(imaging, viewSettings->getAzimuthMin(),
            viewSettings->getAzimuthMax(), viewSettings->getRangeMin(),
...
OffscreenBuffer::viewSettingsChanged()
{
    ...
    setBounds(viewSettings->getAzimuthMin(), viewSettings->getAzimuthMax(),
              viewSettings->getRangeMin(), viewSettings->getRangeMax());
}
