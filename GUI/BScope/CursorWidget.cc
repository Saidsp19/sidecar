...
CursorWidget::GetFormattedPosition(double azimuth, double range)
{
    return QString("Az: %1 Range: %2")
        .arg(App::GetApp()->getFormattedAngleRadians(Utils::normalizeRadians(azimuth)))
        .arg(App::GetApp()->getFormattedDistance(range));
}

CursorWidget::CursorWidget(QWidget* parent) :
    Super(parent), app_(App::GetApp()), viewSettings_(app_->getConfiguration()->getViewSettings()), azimuth_(0.0),
    range_(0.0), updateTimer_(), updateAzimuth_(true), updateRange_(true)
{
    connect(App::GetApp(), SIGNAL(distanceUnitsChanged(const QString&)), SLOT(refreshRange()));
    connect(App::GetApp(), SIGNAL(angleFormattingChanged(int)), SLOT(refreshAzimuth()));
    connect(App::GetApp(), SIGNAL(phantomCursorChanged(const QPointF&)), SLOT(showCursorPosition(const QPointF&)));
    refresh();
}
...
CursorWidget::refresh()
{
    if (updateAzimuth_) {
        ...
        azimuthText_ = QString("Az: ") + app_->getFormattedAngleRadians(Utils::normalizeRadians(azimuth_));
    }

    if (updateRange_) {
        updateRange_ = false;
        rangeText_ = QString(" Range: ") + app_->getFormattedDistance(range_);
    }
