...
struct CursorPosition::Private {
    Private(double x, double y, double azimuth, double range) :
        x_(x), y_(y), azimuth_(Utils::normalizeRadians(azimuth)), range_(range), sampleValue_("NA"), toolTip_(),
        widgetText_()
    {
    }
...
CursorPosition::CursorPosition(double x, double y, double azimuth, double range) :
