...
VideoChannel::VideoChannel(History& history, const QString& name) :
    QObject(), videoInfo_(Messages::Video::GetMetaTypeInfo()), history_(history), peakBars_(), historySlot_(size_t(-1)),
...
VideoChannel::~VideoChannel()
{
    static Logger::ProcLog log("~VideoChannel", Log());
    LOGINFO << std::endl;
    if (historySlot_ != size_t(-1)) history_.releaseSlot(historySlot_);
}
...
VideoChannel::displayAdded()
{
    ...
    if (displayCount_++ == 0) { historySlot_ = history_.allocateSlot(); }
...
void
VideoChannel::setSampleToVoltageScaling(int sampleMin, int sampleMax, double voltageMin, double voltageMax)
{
...
VideoChannel::restoreFromSettings(QSettings& settings)
{
    ...
    DefaultViewSettings& viewSettings(App::GetApp()->getDefaultViewSettings());

    setSampleToVoltageScaling(settings.value(kSampleMin, viewSettings.getSampleMin()).toInt(),
                              settings.value(kSampleMax, viewSettings.getSampleMax()).toInt(),
                              settings.value(kVoltageMin, viewSettings.getVoltageMin()).toDouble(),
                              settings.value(kVoltageMax, viewSettings.getVoltageMax()).toDouble());
    settings.endGroup();
}
