...
{
    static Logger::Log& log_ = Logger::Log::Find("ascope.HistoryControlWidget");
    return log_;
}

HistoryControlWidget::HistoryControlWidget(ToolBar* parent) :
    Super(parent), Ui::HistoryControlWidget(), history_(App::GetApp()->getHistory()), activeHistoryPosition_(0)
{
    setupUi(this);
...
HistoryControlWidget::pastFrozen()
{
    ...
    if (!viewPast_->isChecked()) viewPast_->setChecked(true);
}
...
{
    pastSlider_->setEnabled(false);
    if (viewPast_->isChecked()) viewPast_->setChecked(false);
}

void
HistoryControlWidget::manage(HistoryPosition* historyPosition)
{
    connect(historyPosition, SIGNAL(viewingPastChanged(bool)), SLOT(viewingPastChanged(bool)));
    connect(viewPast_, SIGNAL(toggled(bool)), historyPosition, SLOT(viewingPast(bool)));
    connect(this, SIGNAL(positionChange(int)), historyPosition, SLOT(setPosition(int)));
}
...
HistoryControlWidget::on_viewPast__toggled(bool state)
{
    ...
    if (state) { pastSlider_->setMaximum(history_.getFrameCount()); }
}
...
{
    if (activeHistoryPosition_) activeHistoryPosition_->setSynchronized(state);
}
...
HistoryControlWidget::activeDisplayViewChanged(DisplayView* displayView)
{
    if (!displayView) {
        ...
        setEnabled(true);
        activeHistoryPosition_ = displayView->getVisualizer()->getHistoryPosition();
