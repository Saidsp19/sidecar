...
PeakBarCollection::PeakBarCollection(QObject* parent) :
    Super(parent), peakBars_(), changed_(), last_(), width_(-1), enabled_(false)
{
    ...
    connect(&settings, SIGNAL(lifeTimeChanged(int)), SLOT(lifeTimeChanged(int)));
...
PeakBarCollection::adjustBars(bool resetBars)
{
    ...
    int barsNeeded = int(::floor(double(last_->size() + width_ - 1) / width_));
    ...
    for (int index = 0; index < peakBars_.size(); ++index) {
        ...
        if (end > last_->end()) end = last_->end();

        if (pos == end) { LOGDEBUG << "no data for bar " << index << std::endl; }
