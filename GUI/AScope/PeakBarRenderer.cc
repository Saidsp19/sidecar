...
PeakBarRenderer::PeakBarRenderer(ChannelConnection& channelConnection)
    : Super(0), channelConnection_(channelConnection),
      model_(channelConnection.getChannel().getPeakBars()), cache_(),
      lines_(), rects_(), barSize_(-1, -1), origin_(-1, -1), width_(-1),
      fading_(false), enabled_(true), dirty_(true), allDirty_(true),
      transformDirty_(true)
{
    ...
    connect(&settings, SIGNAL(fadingChanged(bool)),
            SLOT(fadingChanged(bool)));
    connect(&settings, SIGNAL(enabledChanged(bool)),
            SLOT(enabledChanged(bool)));

    connect(&model_, SIGNAL(gateTransformChanged()),
            SLOT(gateTransformChanged()));
    connect(&model_, SIGNAL(barCountChanged(int)),
            SLOT(barCountChanged(int)));
    connect(&model_, SIGNAL(barValuesChanged(const QVector<int>&)),
            SLOT(barValuesChanged(const QVector<int>&)));
}
...
void
PeakBarRenderer::calculateGeometry(const QMatrix& transform,
                                   ...
                                   bool showRanges, bool showVoltages)
{
	...
	if (allDirty_ ||
            (! usingLines && rects_[index].isNull()) ||
            (usingLines && lines_[index].isNull())) {

	    int sample = cache_[index].getValue();
	    LOGDEBUG << "index: " << index << " sample: " << sample
		     << std::endl;

	    qreal value = showVoltages ?
		channel.getVoltageForSample(sample) : sample;
...
PeakBarRenderer::render(QPainter& painter)
{
    ...
    if (dirty_) {
	LOGDEBUG << "there are dirty elements!" << std::endl;
    }
	...
	if (! fading_) {
		...
		if (lines_[index].isNull()) {
		    LOGERROR << "invalid line at index " << index
			     << std::endl;
		}
		else {
	...
	for (int index = 0; index < cache_.size(); ++index) {
		...
		LOGERROR << "invalid rect at index " << index
			 << std::endl;
	    }
	    else {
		if (fading_)
...
PeakBarRenderer::barValuesChanged(const QVector<int>& changed)
{
	...
	//
	cache_ = model_.getPeakBars();
	if (cache_.size() != lines_.size()) {
	    barCountChanged(cache_.size());
	}
