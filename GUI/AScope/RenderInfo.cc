...
RenderInfo::RenderInfo(Visualizer& visualizer, VideoChannel& channel,
                       bool visible, bool showPeakBars)
    : QObject(), visualizer_(visualizer), peakBars_(channel.getPeakBars()),
      historySlot_(channel.displayAdded()), color_(channel.getColor()),
      visible_(visible), showPeakBars_(showPeakBars), frozen_(false),
      lastRendered_(), plotPoints_(), barSize_(-1, -1), origin_(-1, -1),
      peakBarPoints_()
{
    ...
    connect(&peakBars_, SIGNAL(transformChanged()),
            SLOT(updatePeakBarCache()));

    PeakBarSettings* peakBarSettings = App::GetApp()->getPeakBarSettings();
    connect(peakBarSettings, SIGNAL(widthChanged()),
            SLOT(adjustPeakBars()));
    connect(peakBarSettings, SIGNAL(fadingChanged()),
            &visualizer, SLOT(update()));
    connect(peakBarSettings, SIGNAL(enabledChanged(bool)),
            &visualizer, SLOT(update()));
}
