...
ViewSettings::ViewSettings(double rangeMin, double rangeMax, int gateMin, int gateMax, double voltageMin,
                           double voltageMax, int sampleMin, int sampleMax, bool showingRanges, bool showingVoltages) :
    bounds_(),
    rangeMin_(rangeMin), rangeMax_(rangeMax), gateMin_(gateMin), gateMax_(gateMax), voltageMin_(voltageMin),
    voltageMax_(voltageMax), sampleMin_(sampleMin), sampleMax_(sampleMax), showingRanges_(showingRanges),
...
ViewSettings::setBounds(const ViewBounds& bounds)
{
    ...
    LOGINFO << "xMin: " << bounds.getXMin() << " xMax: " << bounds.getXMax() << " w: " << bounds.getWidth()
            << "yMin: " << bounds.getYMin() << "yMax: " << bounds.getYMax() << " h: " << bounds.getHeight()
            << std::endl;
    ...
    if (showingRanges_) {
        ...
        LOGDEBUG << "gateMin: " << gateMin_ << " gateMax: " << gateMax_ << std::endl;
    } else {
        ...
        rangeMin_ += dMin * span;
        rangeMax_ += dMax * span;
        LOGDEBUG << "rangeMin: " << rangeMin_ << " rangeMax: " << rangeMax_ << std::endl;
    }
    ...
    if (showingVoltages_) {
        ...
        LOGDEBUG << "sampleMin: " << sampleMin_ << " sampleMax: " << sampleMax_ << std::endl;
    } else {
        ...
        LOGDEBUG << "voltageMin: " << voltageMin_ << " voltageMax: " << voltageMax_ << std::endl;
    }
...
{
    return rangeMin_ == rhs.rangeMin_ && rangeMax_ == rhs.rangeMax_ && gateMin_ == rhs.gateMin_ &&
           gateMax_ == rhs.gateMax_ && voltageMin_ == rhs.voltageMin_ && voltageMax_ == rhs.voltageMax_ &&
           sampleMin_ == rhs.sampleMin_ && sampleMax_ == rhs.sampleMax_ && showingRanges_ == rhs.showingRanges_ &&
