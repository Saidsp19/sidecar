...
HistorySlotManager::allocate()
{
    ...
    LOGINFO << "active: " << active_ << " allocated: " << allocated_.size() << std::endl;
...
HistorySlotManager::release(size_t slot)
{
    ...
    LOGINFO << "slot: " << slot << " active: " << active_ << " allocated: " << allocated_.size() << std::endl;
    ...
    if (oldIndex != -1) {
        ...
        for (size_t slot = 0; slot < allocated_.size(); ++slot) {
            if (allocated_[slot] >= oldIndex) allocated_[slot] -= 1;
        }

        --active_;
    }

    LOGDEBUG << "active: " << active_ << " allocated: " << allocated_.size() << std::endl;
}
