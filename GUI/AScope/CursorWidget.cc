...
CursorWidget::CursorWidget(QWidget* parent) :
    Super(parent), app_(*App::GetApp()), updateTimer_(), x_(0.0), y_(0.0), isRange_(true), isVoltage_(true),
    updateX_(true), updateY_(true)
{
...
CursorWidget::refresh()
{
    if (updateX_) {
        updateX_ = false;
        if (isRange_)
            xText_ = QString("Range: %1").arg(app_.getFormattedDistance(x_));
        else
