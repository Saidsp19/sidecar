...
ViewEditor::ViewEditor(int shortcut)
    : ToolWindowBase("ViewEditor", "View Editor", shortcut),
      Ui::ViewEditor(), activeVisualizer_(0)
{
    setupUi(this);
    setFixedSize();
    ...
    connect(horizontalUnits_, SIGNAL(currentChanged(int)),
            SLOT(updateButtons()));
    connect(verticalUnits_, SIGNAL(currentChanged(int)),
            SLOT(updateButtons()));
...
{
    if (view.getRangeMin() != rangeMin_->value())
	rangeMin_->setValue(view.getRangeMin());
    if (view.getRangeMax() != rangeMax_->value())
	rangeMax_->setValue(view.getRangeMax());
    if (view.getGateMin() != gateMin_->value())
	gateMin_->setValue(view.getGateMin());
    if (view.getGateMax() != gateMax_->value())
	gateMax_->setValue(view.getGateMax());
    if (view.getVoltageMin() != voltageMin_->value())
	voltageMin_->setValue(view.getVoltageMin());
    if (view.getVoltageMax() != voltageMax_->value())
	voltageMax_->setValue(view.getVoltageMax());
    if (view.getSampleMin() != sampleMin_->value())
	sampleMin_->setValue(view.getSampleMin());
    if (view.getSampleMax() != sampleMax_->value())
	sampleMax_->setValue(view.getSampleMax());
...
ViewEditor::makeView() const
{
    ...
    return ViewSettings(rangeMin_->value(), rangeMax_->value(),
                        gateMin_->value(), gateMax_->value(),
                        voltageMin_->value(), voltageMax_->value(),
                        sampleMin_->value(), sampleMax_->value(),
...
ViewEditor::updateButtons()
{
    ...
    if (activeVisualizer_)
	hasChanges = activeVisualizer_->getCurrentView() != current;
    ...
    hasChanges = false;
    if (hasPreset)
	hasChanges = getView(presetIndex) != current;
...
{
    if (value > rangeMax_->value())
	rangeMax_->setValue(value + 1);
    updateButtons();
}
...
{
    if (value < rangeMin_->value())
	rangeMin_->setValue(value - 1);
    updateButtons();
}
...
{
    if (value > gateMax_->value())
	gateMax_->setValue(value + 1);
    updateButtons();
}
...
{
    if (value < gateMin_->value())
	gateMin_->setValue(value - 1);
    updateButtons();
}
...
{
    if (value > voltageMax_->value())
	voltageMax_->setValue(value + 1);
    updateButtons();
}
...
{
    if (value < voltageMin_->value())
	voltageMin_->setValue(value - 1);
    updateButtons();
}
...
{
    if (value > sampleMax_->value())
	sampleMax_->setValue(value + 1);
    updateButtons();
}
...
{
    if (value < sampleMin_->value())
	sampleMin_->setValue(value - 1);
    updateButtons();
}
