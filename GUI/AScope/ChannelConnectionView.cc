...
void
ItemDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option,
                    const QModelIndex& index) const
{
...
ChannelConnectionView::Log()
{
	...
	Logger::Log::Find("ascope.ChannelConnectionView");
    return log_;
}

ChannelConnectionView::ChannelConnectionView(QWidget* parent)
    : Super(parent), contextMenu_(new QMenu(this))
{
    connect(this, SIGNAL(doubleClicked(const QModelIndex&)),
            SLOT(editEntry(const QModelIndex&)));
...
{
    ChannelConnectionModel* model = static_cast<ChannelConnectionModel*>(
	this->model());
    for (int row = 0; row < model->rowCount(); ++row) {
	model->getChannelConnection(row)->setVisible(true);
    }
}
...
ChannelConnectionView::makeAllInvisible()
{
	...
	this->model());
    for (int row = 0; row < model->rowCount(); ++row) {
	model->getChannelConnection(row)->setVisible(false);
    }
}
...
ChannelConnectionView::editEntry(const QModelIndex& index)
{
    ...
    else if (where.column() >= ChannelConnectionModel::kName &&
             where.column() <= ChannelConnectionModel::kVoltageMax) {
	ChannelEditor editor(window());

	ChannelConnectionModel* model = static_cast<ChannelConnectionModel*>(
	    this->model());
	ChannelConnection* connection = model->getChannelConnection(
	    where.row());
	...
	if (editor.exec() == QDialog::Accepted) {
	    ...
	    model->setData(
		index.sibling(where.row(), ChannelConnectionModel::kVisible),
		editor.visible_->isChecked());

	    model->setData(
		index.sibling(where.row(),
                              ChannelConnectionModel::kShowPeakBars),
		editor.showPeakBars_->isChecked());

	    channel.setSampleToVoltageScaling(editor.sampleMin_->value(),
                                              editor.sampleMax_->value(),
                                              editor.voltageMin_->value(),
                                              editor.voltageMax_->value());

	    QVariant dummy;
	    for (int col = ChannelConnectionModel::kSampleMin;
                 col <= ChannelConnectionModel::kVoltageMax; ++col) {
...
void
ChannelConnectionView::rowsInserted(const QModelIndex& parent, int start,
                                    int end)
{
...
void
ChannelConnectionView::rowsAboutToBeRemoved(const QModelIndex& parent,
                                            int start, int end)
{
...
ChannelConnectionView::adjustColumnSizes()
{
    ...
    for (int column = 1; column < ChannelConnectionModel::kNumColumns;
         ++column)
	available -= columnWidth(column);
    if (available > minimum)
	setColumnWidth(0, available);
}
