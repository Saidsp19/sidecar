...
ComponentView::ComponentView(ComponentType ct, QGraphicsItem* parent) :
    QGraphicsItem(parent), subtype(ct), inputConnected(false), outputConnected(false),
    // mw(mw),
    chainup(0), chaindown(0)
{
...
QRectF
ComponentView::boundingRect() const
{
    return QRectF(0, -20, boxw + 2 * portw, boxh + 20);
}

QPainterPath
ComponentView::shape() const
{
    QPainterPath path;
...
void
ComponentView::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
    Q_UNUSED(widget);
    ...
    if (option->levelOfDetail >= 0.4) {
        ...
        QPainterPath input;
        painter->save();
        if (chainup) painter->setBrush(QBrush(QColor(Qt::black)));
        ...
        if (option->levelOfDetail >= 0.5) {
            ...
            if (option->levelOfDetail >= 1) {
                ...
                painter->drawLine(portw + shadowOffset, boxh + shadowOffset, portw + boxw + shadowOffset,
                                  boxh + shadowOffset);
                painter->drawLine(portw + boxw + shadowOffset, shadowOffset, portw + boxw + shadowOffset,
                                  boxh + shadowOffset);
        ...
        QPainterPath output;
        painter->save();
        if (chaindown) painter->setBrush(QBrush(QColor(Qt::black)));
...
void
ComponentView::setSource(LinkView* l)
{
    if (chainup && chainup != l) chainup->release();
...
void
ComponentView::setDrain(LinkView* l)
{
...
QVariant
ComponentView::itemChange(GraphicsItemChange change, const QVariant& value)
{
    switch (change) {
    case ItemPositionChange: updateLinks(); break;
    default: break;
    };
...
void
ComponentView::updateLinks()
{
    if (chainup) chainup->moveEnd();
    if (chaindown) chaindown->moveStart();
    if (parentItem()) {
