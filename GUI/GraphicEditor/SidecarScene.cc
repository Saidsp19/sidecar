...
SidecarScene::SidecarScene(QObject *parent) : QGraphicsScene(parent),
                                              command(A_none),
                                              componentType(ComponentView::CT_algorithm),
                                              currentComponent(0)
{
    setSceneRect(-1000,-1000,2000,2000);
}

void SidecarScene::mousePressEvent(QGraphicsSceneMouseEvent *e)
{
    ...
    case A_link:
        if(item)
        {
            ...
            if(tmp)
            {
                if(currentComponent && currentComponent!=tmp && currentComponent->parentItem()==tmp->parentItem())
                {
