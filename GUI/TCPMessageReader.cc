...
{
    static Logger::Log& log_ = Logger::Log::Find("SideCar.GUI.TCPMessageReader");
    return log_;
}
...
TCPMessageReader::Make(const ServiceEntry* service)
{
    ...
    socket->connectToHost(service->getHost(), service->getPort(), QIODevice::ReadOnly);
    return new TCPMessageReader(service->getMetaTypeInfo(), socket);
}

TCPMessageReader::TCPMessageReader(const Messages::MetaTypeInfo* metaTypeInfo, QTcpSocket* socket) :
