...
PresetManager::PresetManager(const QString& prefix, QObject* parent) :
    QObject(parent), file_(), prefix_(prefix), presets_(), settings_(), globals_(), settingNames_(),
    activePresetIndex_(0), restoring_(false), restored_(false)
{
...
{
    QStringList names;
    for (int index = 0; index < presets_.size(); ++index) { names.append(presets_[index]->getName()); }
...
PresetManager::getPreset(const QString& name) const
{
    for (int index = 0; index < presets_.size(); ++index) {
        if (presets_[index]->getName() == name) return presets_[index];
    }
    ...
    file_.beginGroup(prefix_);
    {
        ...
        file_.beginGroup(kGlobals);
        {
            for (int index = 0; index < globals_.size(); ++index) {
                ...
                QVariant value = file_.value(setting->getName(), setting->getOpaqueValue());
                if (isOldVersion()) value = upgradeSetting(setting->getName(), value);
        ...
        //
        while (presets_.size()) delete presets_.takeLast();
        ...
        for (int index = 0; index < size; ++index) {
            ...
            if (!name.isEmpty()) presets_.append(new Preset(name, this));
        }
        file_.endArray();
        ...
        //
        if (presets_.empty() || presets_[0]->getName() != kDefault) { presets_.insert(0, new Preset(kDefault, this)); }
        ...
        {
            for (int index = 0; index < presets_.size(); ++index) {
                presets_[index]->restore(file_, settings_, index == activePresetIndex_);
                if (index) emit presetDirtyStateChanged(index, false);
            }
        }
...
PresetManager::saveAllPresets(bool saveChanges)
{
    ...
    file_.beginGroup(prefix_);
    {
        ...
        file_.beginGroup(kGlobals);
        {
            for (int index = 0; index < globals_.size(); ++index) {
                Setting* setting = globals_[index];
                file_.setValue(setting->getName(), setting->getOpaqueValue());
            }
        }
        file_.endGroup();
...
PresetManager::addSetting(Setting* setting, bool global)
{
    ...
    if (global) {
        ...
        if (restored_) globalRestore(setting);
        connect(setting, SIGNAL(save(int, const QVariant&)), SLOT(globalSave(int, const QVariant&)));
    } else {
        index = settings_.size();
        settings_.append(setting);
        settingNames_.append(setting->getName());
        connect(setting, SIGNAL(save(int, const QVariant&)), SLOT(settingSave(int, const QVariant&)));
        if (restored_) settingRestore(setting);
    }
    return index;
}
...
PresetManager::globalRestore(Setting* setting)
{
    ...
    file_.beginGroup(prefix_);
    {
        file_.beginGroup(kGlobals);
        {
            setting->setOpaqueValue(file_.value(setting->getName(), setting->getOpaqueValue()));
        }
        file_.endGroup();
    }
    file_.endGroup();
...
PresetManager::settingRestore(Setting* setting)
{
    ...
    file_.beginGroup(prefix_);
    {
        file_.beginGroup(kPresetData);
        {
            for (int index = 0; index < presets_.size(); ++index) { presets_[index]->restoreOne(file_, setting); }
        }
        file_.endGroup();
    }
    file_.endGroup();
...
PresetManager::getSetting(const QString& name) const
{
    for (int index = 0; index < settingNames_.size(); ++index) {
        if (settingNames_[index] == name) return settings_[index];
    }
...
{
    for (int index = 0; index < globals_.size(); ++index) {
        if (globals_[index]->getName() == name) return globals_[index];
    }
...
PresetManager::settingSave(int index, const QVariant& value)
{
    ...
    LOGINFO << "index: " << index << " value: " << value.toString() << std::endl;
...
PresetManager::addPreset(const QString& name)
{
    ...
    //
    for (int index = 0; index < presets_.size(); ++index) {
        if (presets_[index]->getName() == name) return false;
    }
...
PresetManager::removePreset(int index)
{
    ...
    //
    if (index < 1 || index > presets_.size() - 1) return false;
...
{
    if (index < 0 || index > presets_.size() - 1) return false;

    if (activePresetIndex_ == index) return true;
...
PresetManager::getPresetIsDirty(int index) const
{
    ...
    //
    return index > 0 && index < presets_.size() && presets_[index]->isDirty();
}
...
PresetManager::getAnyIsDirty() const
{
    ...
    for (int index = 1; index < presets_.size(); ++index)
        if (presets_[index]->isDirty()) return true;
    return false;
}
