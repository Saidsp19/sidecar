...
static const QString colorNames_[LED::kTurquoise + 1] = {"grey",   "red",    "green",    "blue",
                                                         "yellow", "orange", "turquoise"};

LED::LED(QWidget* parent) : Super(parent), color_(), diameter_(0)
{
    setDiameter(24);
    setColor(kGrey);
}
...
{
    if (value < kGrey || value > kTurquoise) value = kGrey;
