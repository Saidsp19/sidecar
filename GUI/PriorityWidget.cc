...
PriorityWidget::Log()
{
	...
	Logger::Log::Find("SideCar.GUI.PriorityWidget");
    return log_;
}

PriorityWidget::PriorityWidget(QObject* parent)
    : Super(parent), names_()
{
    for (int index = Logger::Priority::kNone;
         index < Logger::Priority::kNumLevels; ++index) {
	QString name = QString::fromStdString(
	    Logger::Priority::GetLongName(Logger::Priority::Level(index)));
	names_ << (name[0] + name.mid(1).toLower());
    }
}

QWidget*
PriorityWidget::createEditor(QWidget* parent,
...
{
    const_cast<QAbstractItemModel*>(active_.model())->setData(
	active_, priority, Qt::EditRole);
}
...
void
PriorityWidget::setModelData(QWidget* widget, QAbstractItemModel* model,
                             const QModelIndex& index) const
{
    LoggerTreeItem* node = getItem(index);
    if (! node) return;
    ...
    model->setData(index, QString::fromStdString(
                       Logger::Priority::GetLongName(
                           Logger::Priority::Level(value))),
                   Qt::DisplayRole);
}

void
PriorityWidget::updateEditorGeometry(QWidget* widget,
                                     const QStyleOptionViewItem& option,
                                     const QModelIndex& index) const
{
...
QSize
PriorityWidget::sizeHint(const QStyleOptionViewItem& option,
                         const QModelIndex& index) const
{
