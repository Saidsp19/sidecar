...
{
    static Logger::Log& log_ = Logger::Log::Find("SideCar.GUI.VideoSampleCountTransform");
    return log_;
}

VideoSampleCountTransform::VideoSampleCountTransform(IntSetting* sampleMin, IntSetting* sampleMax, IntSetting* gain,
                                                     IntSetting* cutoffMin, IntSetting* cutoffMax,
                                                     BoolSetting* showDecibels) :
    Super(),
    sampleMin_(sampleMin), sampleMax_(sampleMax), cutoffMin_(cutoffMin), cutoffMax_(cutoffMax),
    showDecibels_(showDecibels)
{
    ...
    connect(showDecibels, SIGNAL(valueChanged(bool)), SLOT(showDecibelsChanged(bool)));
...
VideoSampleCountTransform::recalculate()
{
    ...
    if (sampleMin > cutoffMax_->getValue() || sampleMax < cutoffMax_->getValue()) { cutoffMax_->setValue(sampleMax); }

    if (sampleMin > cutoffMin_->getValue() || sampleMax < cutoffMin_->getValue()) { cutoffMin_->setValue(sampleMin); }

    thresholdMin_ = cutoffMin_->getValue();
    thresholdMax_ = cutoffMax_->getValue();

    LOGINFO << "min: " << thresholdMin_ << " max: " << thresholdMax_ << std::endl;
