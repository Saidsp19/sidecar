...
CollectionStats::CollectionStats()
    : counters_(kNumStats, 0),
      processingState_(IO::ProcessingState::kInvalid), error_("")
{
    ;
}
...
{
    for (size_t index = 0; index < kNumStats; ++index)
	counters_[index] += rhs.counters_[index];
    ...
    if (error_.isEmpty() && ! rhs.error_.isEmpty())
	error_ = rhs.error_;
