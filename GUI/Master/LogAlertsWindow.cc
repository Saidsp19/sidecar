...
LogAlertsWindow::LogAlertsWindow() : Super(), gui_(new Ui::LogAlertsWindow)
{
    gui_->setupUi(this);
    ...
    connect(gui_->actionCopy, SIGNAL(triggered()), gui_->alerts_, SLOT(copy()));
    connect(gui_->actionClear, SIGNAL(triggered()), gui_->alerts_, SLOT(clear()));
...
LogAlertsWindow::addAlerts(const QStringList& entries)
{
    ...
    //
    int vPos = -1;
    if (gui_->alerts_->cursorRect().top() > gui_->alerts_->viewport()->height() && isVisible()) {
