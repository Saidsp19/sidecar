...
MessageStats::MessageStats() : duplicates_(0), drops_(0), recordingDuplicates_(0), recordingDrops_(0)
{
    ;
}

MessageStats::MessageStats(const IO::StatusBase& status) :
    duplicates_(int(status[RawPRI::kDuplicates])), drops_(int(status[RawPRI::kDrops])),
