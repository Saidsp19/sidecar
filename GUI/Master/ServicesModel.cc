...
ServicesModel::ServicesModel(QObject* parent) :
    Super(parent),
    browser_(new ServiceBrowser(this, QString::fromStdString(Runner::RemoteController::GetZeroconfType()))),
...
ServicesModel::getModelIndex(const TreeViewItem* item, int col) const
{
    ...
    if (item->getParent()) parentIndex = getModelIndex(item->getParent(), 0);
...
ServicesModel::index(int row, int column, const QModelIndex& parent) const
{
    ...
    if (hasIndex(row, column, parent)) {
        TreeViewItem* pobj = parent.isValid() ? GetModelData(parent) : rootItem_;
        if (row >= 0 && row < pobj->getNumChildren()) index = createIndex(row, column, pobj->getChild(row));
    }
...
bool
ServicesModel::postProcessingStateChange(const QStringList& filter, IO::ProcessingState::Value state) const
{
...
bool
ServicesModel::postRecordingStart(const QStringList& configNames, const QStringList& recordingPaths) const
{
    ...
    for (int index = 0; index < configNames.count(); ++index) {
        ...
        if (!postCommand(configNames[index], "recordingChange", args)) return false;
    }
...
bool
ServicesModel::postCommand(const QStringList& filter, const char* cmd, const XmlRpc::XmlRpcValue& args) const
{
    for (int index = 0; index < filter.count(); ++index) {
        if (!postCommand(filter[index], cmd, args)) return false;
    }
    return true;
}

bool
ServicesModel::postCommand(const QString& configName, const char* cmd, const XmlRpc::XmlRpcValue& args) const
{
    ...
    for (int index = 0; index < rootItem_->getNumChildren(); ++index) {
        ...
        if (configName == configItem->getName()) return configItem->executeRequest(cmd, args);
    }
...
bool
ServicesModel::getChangedParameters(const QStringList& configNames, QStringList& changes) const
{
    bool ok = true;
    foreach (QString configName, configNames) {
        ConfigurationItem* configItem = getConfigurationItem(configName);
        if (configItem) {
            if (!configItem->getChangedParameters(changes)) ok = false;
        }
    }
    return ok;
}
...
ServicesModel::foundServices(const ServiceEntryList& found)
{
    ...
    //
    for (ServiceEntryList::const_iterator pos = found.begin(); pos != found.end(); ++pos) {
        ServiceEntry* serviceEntry(*pos);
        LOGDEBUG << serviceEntry->getName() << std::endl;
        connect(serviceEntry, SIGNAL(resolved(ServiceEntry*)), SLOT(resolvedService(ServiceEntry*)));
...
ServicesModel::resolvedService(ServiceEntry* serviceEntry)
{
    ...
    for (int index = 0; index < rootItem_->getNumChildren(); ++index) {
        ...
        RunnerItem* runnerItem = configurationItem->findService(serviceEntry);
        if (runnerItem) {
...
ServicesModel::lostServices(const ServiceEntryList& lost)
{
    ...
    for (ServiceEntryList::const_iterator pos = lost.begin(); pos != lost.end(); ++pos) {
        ServiceEntry* serviceEntry(*pos);
        ...
        for (int index = 0; index < rootItem_->getNumChildren(); ++index) {
            ...
            if (runnerItem) {
                ...
                if (configurationItem->getNumChildren() == 1) {
                    ...
                    beginRemoveRows(getModelIndex(configurationItem), row, row);
...
QVariant
ServicesModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    if (orientation == Qt::Vertical) return QVariant();

    if (role == Qt::TextAlignmentRole) return int(Qt::AlignVCenter | Qt::AlignHCenter);

    if (role == Qt::DisplayRole) return QString(GetColumnName(section));
...
ServicesModel::updateStatus(const QList<QByteArray>& statusReports)
{
    ...
    foreach (QByteArray statusReport, statusReports) {
        ...
        if (!xmlValue.valid() || xmlValue.getType() != XmlRpc::XmlRpcValue::TypeArray) {
            LOGERROR << "invalid type from runner: " << xmlValue.getType() << " size: " << statusReport.size()
                     << std::endl;
            continue;
        }
        ...
        if (!status.isLatestVersion()) {
            LOGWARNING << "skipping status report with version " << status.getVersion() << std::endl;
            continue;
        }

        QString serviceName(QString::fromStdString(status.getServiceName()));
        ...
        if (!runnerItem) {
            ...
            int row = configurationItem->getInsertionPoint(runnerItem->getName());

            LOGDEBUG << "beginInsertRows row: " << row << ' ' << serviceName << std::endl;
            ...
            if (configurationItem->update(status, runnerItem)) {
                emit dataChanged(getModelIndex(runnerItem, kState), getModelIndex(runnerItem, kNumColumns - 1));
                emit statusUpdated();
            }
        }
        ...
        if (!runnerItem->getServiceEntry()) {
            ServiceEntry* serviceEntry = browser_->getServiceEntry(serviceName);
            LOGDEBUG << "serviceName: " << serviceName << " serviceEntry: " << serviceEntry << std::endl;
            if (serviceEntry) {
...
void
ServicesModel::getServiceStats(int& runnerCount, int& streamCount, int& pendingCount, int& failureCount) const
{
    runnerCount = 0;
    streamCount = 0;
    pendingCount = 0;
...
void
ServicesModel::getDropsAndDupes(const QStringList& filter, int& drops, int& dupes) const
{
    drops = 0;
    dupes = 0;
    for (int index = 0; index < rootItem_->getNumChildren(); ++index) {
...
RunnerItem*
ServicesModel::getRunnerItem(const QString& configName, const QString& runnerName) const
{
    for (int index = 0; index < rootItem_->getNumChildren(); ++index) {
        ConfigurationItem* configItem = rootItem_->getChild(index);
        if (configItem->getName() == configName) {
            RunnerItem* runnerItem = configItem->findService(runnerName);
            if (runnerItem) return runnerItem;
            break;
        }
    }
...
ServicesModel::getServiceNames(const QString& configName) const
{
    ...
    if (configItem) {
        for (int index = 0; index < configItem->getNumChildren(); ++index) {
...
ServicesModel::isRecording(const QStringList& filter) const
{
    for (int index = 0; index < rootItem_->getNumChildren(); ++index) {
        ConfigurationItem* configItem = rootItem_->getChild(index);
        if (filter.contains(configItem->getName()) && configItem->isRecording()) return true;
    }
...
ServicesModel::isCalibrating(const QStringList& filter) const
{
    for (int index = 0; index < rootItem_->getNumChildren(); ++index) {
        ConfigurationItem* configItem = rootItem_->getChild(index);
        if (filter.contains(configItem->getName()) &&
            configItem->getProcessingState() == IO::ProcessingState::kCalibrate)
            return true;
    }
