...
public:
    ...
    ConfigurationFilter(const QStringList& loaded) : loaded_(loaded), filter_() {}

    ConfigurationFilter(const QStringList& loaded, const QString& value) : loaded_(loaded), filter_(value) {}

    bool passes(const QString& name) const { return filter_.contains(name) || !loaded_.contains(name); }

    const QString& getDisplayText() const;

    const QString& getValue() const { return value_; }

    bool operator==(const ConfigurationFilter& rhs) const { return value_ == rhs.value_; }
