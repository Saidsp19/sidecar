...
NotesWindow::NotesWindow(RecordingController& controller, RecordingInfo& info) :
    Super(), gui_(new Ui::NotesWindow), controller_(controller), info_(info)
{
    ...
    connect(gui_->recordingStop_, SIGNAL(clicked()), &controller, SLOT(startStop()));
    connect(gui_->actionStop_Recording, SIGNAL(triggered()), &controller, SLOT(startStop()));
    connect(gui_->actionUndo, SIGNAL(triggered()), gui_->notes_, SLOT(undo()));
    connect(gui_->actionRedo, SIGNAL(triggered()), gui_->notes_, SLOT(redo()));
    connect(gui_->actionCut, SIGNAL(triggered()), gui_->notes_, SLOT(cut()));
    connect(gui_->actionCopy, SIGNAL(triggered()), gui_->notes_, SLOT(copy()));
    connect(gui_->actionPaste, SIGNAL(triggered()), gui_->notes_, SLOT(paste()));
    connect(gui_->actionInsert_Time, SIGNAL(triggered()), SLOT(on_insertNow__clicked()));
    connect(gui_->actionInsert_Elapsed, SIGNAL(triggered()), SLOT(on_insertElapsed__clicked()));

    connect(gui_->notes_, SIGNAL(copyAvailable(bool)), gui_->actionCut, SLOT(setEnabled(bool)));
    connect(gui_->notes_, SIGNAL(copyAvailable(bool)), gui_->actionCopy, SLOT(setEnabled(bool)));
    connect(gui_->notes_, SIGNAL(undoAvailable(bool)), gui_->actionUndo, SLOT(setEnabled(bool)));
    connect(gui_->notes_, SIGNAL(redoAvailable(bool)), gui_->actionRedo, SLOT(setEnabled(bool)));
    ...
    connect(gui_->radarTransmitting_, SIGNAL(clicked()), SLOT(radarTransmittingChanged()));
    connect(gui_->radarFrequency_, SIGNAL(valueChanged(double)), &info, SLOT(infoChanged()));
    radarTransmittingChanged();

    connect(gui_->radarRotating_, SIGNAL(clicked()), SLOT(radarRotatingChanged()));
    connect(gui_->radarRotationRate_, SIGNAL(valueChanged(double)), &info, SLOT(infoChanged()));
    radarRotatingChanged();

    connect(gui_->drfmOn_, SIGNAL(clicked()), SLOT(drfmOnChanged()));
    connect(gui_->drfmConfig_, SIGNAL(editingFinished()), &info, SLOT(infoChanged()));
    drfmOnChanged();
    ...
    connect(insertNow, SIGNAL(triggered()), SLOT(on_insertNow__clicked()));

    QAction* insertElapsed = new QAction("Insert Elapsed", this);
    connect(insertElapsed, SIGNAL(triggered()), SLOT(on_insertElapsed__clicked()));
}
...
{
    gui_->notes_->insertPlainText(QString("%1 ").arg(gui_->elapsed_->text()));
    gui_->notes_->setFocus(Qt::OtherFocusReason);
}
...
NotesWindow::saveToFile()
{
    ...
    data.append("\nRadar Transmitting: ").append(gui_->radarTransmitting_->isChecked() ? "YES" : "NO");

    if (gui_->radarTransmitting_->isChecked())
        data.append(" - Frequency: ").append(QString::number(gui_->radarFrequency_->value())).append(" MHz");

    data.append("\nRadar Rotating: ").append(gui_->radarRotating_->isChecked() ? "YES" : "NO");

    if (gui_->radarRotating_->isChecked())
        data.append(" - Rate: ").append(QString::number(gui_->radarRotationRate_->value())).append(" rpm");

    data.append("\nUsing DRFM: ").append(gui_->drfmOn_->isChecked() ? "YES" : "NO");
