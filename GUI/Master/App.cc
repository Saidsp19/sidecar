...
App::App(int& argc, char** argv) :
    Super("Master", argc, argv), configurationWindow_(0), workingDirectory_(QDir::current()), mainWindow_(0),
...
App::restoreWindows()
{
    ...
    if (!logAlertsWindow_) makeAndInitializeNewMainWindow("LogAlertsWindow");
    UtilsWidget* utilsWidget = mainWindow_->getUtilsWidget();
    connect(utilsWidget->showAlerts_, SIGNAL(clicked()), logAlertsWindow_, SLOT(showAndRaise()));
    connect(this, SIGNAL(closingAuxillaryWindows()), logAlertsWindow_, SLOT(close()));
}
...
App::applicationQuit()
{
    if (recording_) {
        if (mainWindow_->getNumActiveMasters() == 1) {
            if (QMessageBox::question(activeWindow(), "Recording Active",
...
App::makeNewMainWindow(const QString& objectName)
{
    ...
    if (objectName == "ConfigurationEditor") return 0;

    if (objectName == "LogViewWindow") return 0;

    if (objectName == "NotesWindow") return 0;
