...
FindLineEdit::FindLineEdit(QWidget* parent)
    : QLineEdit(parent), findMenu_(new QToolButton(this)),
      clearButton_(new QToolButton(this))
{
    ...
    setMinimumSize(qMax(msz.width(), clearButton_->sizeHint().height() +
                        frameWidth * 2 + 2),
                   qMax(msz.height(), clearButton_->sizeHint().height() +
                        frameWidth * 2 + 2));
}
