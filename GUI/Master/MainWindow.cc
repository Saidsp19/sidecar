...
MainWindow::MainWindow()
    : MainWindowBase(), Ui::MainWindow(),
      statusWidget_(new StatusWidget(this)),
      utilsWidget_(new UtilsWidget(this)),  servicesModel_(0),
      browser_(0), configurationController_(0), recordingController_(0),
      checker_(), nowTimerId_(-1), lastFailureCount_(0), 
      preDrops_(0), preDupes_(0) 
{
    ...
    connect(utilsWidget_->runState_, SIGNAL(activated(int)),
            SLOT(changeRunState(int)));
    connect(utilsWidget_->clearStats_, SIGNAL(clicked()),
            SLOT(clearStats()));
    connect(utilsWidget_->find_, SIGNAL(textChanged(const QString&)),
            SLOT(filterRunners()));
    ...
    connect(servicesModel_, SIGNAL(statusUpdated()),
            SLOT(servicesStatusUpdated()));
    ...
    settings.getRecordingSettings().connectWidgets(recordingDurationEnable_,
                                                   recordingDuration_);
    
    configurationController_ = new ConfigurationController(
	*this, settings.getConfigurationSettings());
    connect(configurationController_,
            SIGNAL(activeConfigurationsChanged(const QStringList&)),
            SLOT(configurationStatusChanged(const QStringList&)));
    connect(configurationController_,
            SIGNAL(selectedConfigurationChanged(const ConfigurationInfo*)),
            SLOT(updateRecordingStartStop()));

    connect(servicesModel_, SIGNAL(runnerAdded(const RunnerItem*)),
            configurationController_,
            SLOT(foundRunner(const RunnerItem*)));

    connect(servicesModel_, SIGNAL(runnerRemoved(const RunnerItem*)),
            configurationController_,
            SLOT(lostRunner(const RunnerItem*)));

    recordingController_ = new RecordingController(*this);
    connect(recordingController_,
            SIGNAL(statusChanged(const QStringList&, bool)),
            configurationController_,
            SLOT(recordingStatusChanged(const QStringList&, bool)));

    actionRecordingLimitDuration_->setCheckable(true);
    actionRecordingLimitDuration_->setChecked(
	recordingDurationEnable_->isChecked());

    connect(actionRecordingLimitDuration_, SIGNAL(triggered(bool)),
            recordingDurationEnable_, SLOT(setChecked(bool)));
    connect(recordingDurationEnable_, SIGNAL(clicked(bool)),
            actionRecordingLimitDuration_, SLOT(setChecked(bool)));
    connect(actionRecordingStartStop_, SIGNAL(triggered()),
            recordingStartStop_, SLOT(click()));
    ...
    connect(showConfigurationWindow_, SIGNAL(clicked()), action,
            SLOT(trigger()));
...
MainWindow::doNowTick()
{
    ...
    now_ = QDateTime::currentDateTime().toUTC().time().toString("hh:mm:ss") +
	" UTC";
...
MainWindow::processChecker()
{
    ...
    if (servicesModel_->rowCount() || nextValue == tmp->maximum())
	tmp->cancel();
...
#ifdef darwin
	tmp->setLabelText(
	    QString("Searching for active runners...\n%1%")
	    .arg(int(::rint(nextValue / 0.8))));
#endif
	...
	//
	QStringList invalid =
	    configurationController_->getInvalidRecordingDirectories();
	if (! invalid.empty()) {
...
MainWindow::restoreRecordings()
{
    ...
    QStringList failures = recordingController_->restore(
	*configurationController_);
    if (! failures.isEmpty()) {
...
MainWindow::shutdownConfiguration(const QString& configName)
{
	...
	configurationController_->getConfigurationInfo(configName);
...
MainWindow::forceShutdown(ConfigurationInfo* config)
{
    ...
    if (utilsWidget_->find_->text() != "")
	utilsWidget_->find_->setText("");
...
MainWindow::servicesStatusUpdated()
{
    ...
    int pendingCount = 0;
    int failureCount = 0;
    servicesModel_->getServiceStats(runnerCount, streamCount, pendingCount,
                                    failureCount);
	...
	palette.setColor(QPalette::WindowText,
                         failureCount > 0 ? TreeViewItem::GetFailureColor() :
                         Qt::black);
	...
	if (activeRecording) {
	    ...
	    activeRecording->updateStats(drops - preDrops_, 
                                         dupes - preDupes_);
	}
    }
}
...
MainWindow::updateRecordingStartStop()
{
	...
	(recordingController_->getActiveRecording() ||
         (configurationController_ &&
          ! configurationController_->getRecordableConfigurations().empty())
            );
...
MainWindow::changeRunState(int index)
{
    ...
    QStringList filter(
	configurationController_->getViewableRunningConfigurations());
    ...
    IO::ProcessingState::Value state(
	IO::ProcessingState::Value(IO::ProcessingState::kAutoDiagnostic +
                                   index));

    if (servicesModel_->postProcessingStateChange(filter, state)) {
	statusBar()->clearMessage();
	showMessage(QString("State changed to %1.")
                    .arg(IO::ProcessingState::GetName(state)));
    }
    else {
...
bool
MainWindow::startRecording(const QStringList& configNames,
                           const QStringList& recordingPaths)
{
	...
	QMessageBox::critical(
	    qApp->activeWindow(), "Start Recording Failure",
...
MainWindow::stopRecording()
{
	...
	recordingController_->getActiveRecording()->getConfigurationNames());
}

bool
MainWindow::getChangedParameters(const QStringList& configNames,
...
MainWindow::stopRecording(const QStringList& configNames)
{
	...
	QMessageBox::critical(
	    qApp->activeWindow(), "Stop Recording Failure",
...
void
MainWindow::setRecordingElapsed(const QString& elapsed,
...
MainWindow::filterRunners()
{
	...
	runners_->setConfigurationVisibleFilter(
	    configurationController_->getAllConfigurations(),
...
MainWindow::parameterValuesChanged(const QStringList& changes)
{
    ...
    if (recordingInfo)
	recordingInfo->addChangedParameters(changes);
}
...
MainWindow::eventFilter(QObject* obj, QEvent* event)
{
	...
	if (bits.size() == 1) {
	    configStartStop_->setText(bits[0] + " All");
	}
    }
	...
	QStringList bits = configStartStop_->text().split(' ');
	if (bits.size() == 2) {
	    configStartStop_->setText(bits[0]);
	}
    }
