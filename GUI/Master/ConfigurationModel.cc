...
const char* ConfigurationModel::kColumnNames_[] = {"Name", "Status", "View", "Rec", "Disk"};
...
ConfigurationModel::add(ConfigurationInfo* info)
{
    ...
    int row = 0;
    while (pos != end) {
        int rc = QString::compare((*pos)->getName(), name, Qt::CaseInsensitive);
        if (rc >= 0) break;
        ++row;
        ++pos;
    }
...
QModelIndex
ConfigurationModel::index(int row, int column, const QModelIndex& parent) const
{
    if (row < 0 || row >= configurationList_.size() || column < 0 || column >= kNumColumns || parent.isValid())
        return QModelIndex();
...
{
    if (role != Qt::DisplayRole) return Super::headerData(section, orientation, role);
...
ConfigurationModel::data(const QModelIndex& index, int role) const
{
    ...
    case Qt::TextAlignmentRole:
        ...
        case kStatus:
        case kViewable:
        case kRecordable: value = int(Qt::AlignCenter); break;

        default: value = int(Qt::AlignLeft | Qt::AlignVCenter); break;
        }
        break;
    ...
    case Qt::ForegroundRole:
        ...
        case kName:
        case kStatus:
            if (obj->getStatus() == ConfigurationInfo::kPartial || obj->hasError())
                value = TreeViewItem::GetFailureColor();
            break;

        case kSpaceAvailable:
            if (obj->getRecordingDirPercentUsed() > 0.90) value = TreeViewItem::GetFailureColor();
            break;
    ...
    case Qt::DisplayRole:
        ...
        case kStatus: value = obj->getStatusText(); break;

        case kSpaceAvailable: value = obj->getRecordingDirFreeSpace(); break;
    ...
    case Qt::ToolTipRole:
        switch (index.column()) {
        case kName: value = obj->getPath(); break;

        case kStatus:
            if (obj->hasError()) value = obj->getErrorText();
            break;

        case kViewable:
            value = QString("Click to %1 remote processes in the "
                            "view below")
                        .arg(obj->isViewable() ? "hide" : "show");
            break;

        case kRecordable:
            value = QString("Click to %1 recording of the configuration "
                            "processes")
                        .arg(obj->isRecordable() ? "disable" : "enable");
            break;

        case kSpaceAvailable: value = obj->getRecordingsDirectory(); break;
    ...
    case Qt::CheckStateRole:
        switch (index.column()) {
        case kViewable: value = obj->isViewable() ? Qt::Checked : Qt::Unchecked; break;
        case kRecordable: value = obj->canRecord() ? Qt::Checked : Qt::Unchecked; break;
        default: break;
        }
        break;
...
ConfigurationModel::flags(const QModelIndex& index) const
{
    ...
    switch (index.column()) {
    case kViewable: flags |= Qt::ItemIsEditable | Qt::ItemIsUserCheckable; break;

    case kRecordable:
        flags = Qt::ItemIsEditable | Qt::ItemIsUserCheckable;
        if (obj->hasValidRecordingDirectory()) flags |= Qt::ItemIsEnabled | Qt::ItemIsSelectable;
        break;
...
ConfigurationModel::statusChanged(ConfigurationInfo* obj)
{
    ...
    if (row != -1) { emit dataChanged(index(row, 0), index(row, kStatus)); }
}
...
{
    int row = getRowOf(obj);
    if (row != -1) { emit dataChanged(index(row, kSpaceAvailable), index(row, kSpaceAvailable)); }
}

void
ConfigurationModel::recordingStateChanged()
{
    if (rowCount()) emit dataChanged(index(0, 0), index(rowCount() - 1, kNumColumns - 1));
}
