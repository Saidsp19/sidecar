...
ConfigurationController::Log()
{
	...
	Logger::Log::Find("master.ConfigurationController");
    return log_;
}

ConfigurationController::ConfigurationController(
    MainWindow& window, ConfigurationSettings& settings)
    : Super(&window), mainWindow_(window), settings_(settings),
      model_(0)
{
    ...
    connect(configurations, SIGNAL(doubleClicked(const QModelIndex&)),
            SLOT(editConfiguration(const QModelIndex&)));
    ...
    connect(mainWindow_.configAdd_, SIGNAL(clicked()),
            SLOT(add()));
    connect(mainWindow_.actionConfigAdd_, SIGNAL(triggered()),
            SLOT(add()));

    connect(mainWindow_.configRemove_, SIGNAL(clicked()),
            SLOT(remove()));
    connect(mainWindow_.actionConfigRemove_, SIGNAL(triggered()),
            SLOT(remove()));

    connect(mainWindow_.configReload_, SIGNAL(clicked()),
            SLOT(reload()));
    connect(mainWindow_.actionConfigReload_, SIGNAL(triggered()),
            SLOT(reload()));

    connect(mainWindow_.configStartStop_, SIGNAL(clicked()),
            SLOT(launchShutdown()));
    connect(mainWindow_.actionConfigStartStop_, SIGNAL(triggered()),
            SLOT(launchShutdown()));

    connect(mainWindow_.getUtilsWidget()->cleanup_, SIGNAL(clicked()),
            SLOT(cleanup()));
...
ConfigurationController::restore()
{
	...
	QItemSelectionModel* selectionModel = 
	    mainWindow_.configurations_->selectionModel();
	selectionModel->setCurrentIndex(
	    model_->index(lastSelected, 0),
...
void
ConfigurationController::currentSelectionChanged(const QModelIndex& current,
                                                 const QModelIndex& previous)
{
    ...
    if (current.isValid())
	emit selectedConfigurationChanged(model_->GetModelData(current));
}
...
ConfigurationController::updateButtons()
{
    ...
    LOGINFO << "current: " << current << " overrideCursor: "
	    << QApplication::overrideCursor() << std::endl;

    mainWindow_.getUtilsWidget()->cleanup_->setEnabled(
	model_->rowCount() > 0 && ! App::GetApp()->isRecording());
	...
	if (enabled) {
	    ...
	    mainWindow_.configStartStop_->setToolTip(
		"Start remote processing for the selected configuration");
	    mainWindow_.actionConfigStartStop_->setToolTip(
		"Start remote processing for the selected configuration");
	}
	else {
	    mainWindow_.configStartStop_->setText(kShutdown);
	    mainWindow_.configStartStop_->setToolTip(
		"Stop remote processing for the selected configuration");
...
ConfigurationController::add()
{
	...
	if (! obj)
	    obj = model_->getModelData(model_->rowCount() - 1);
	prev = obj->getPath();
    }
    ...
    if (obj) {
	mainWindow_.getRecordingController().restore(
	    obj->getRecordingsDirectory());
    }
...
ConfigurationController::reload()
{
	...
	QMessageBox::information(
	    ...
	    QString("<p>Successfully reloaded configuration '%1'</p>")
	    .arg(getCurrentConfiguration()->getName()));
    }
    else {
...
ConfigurationController::launchShutdown()
{
	...
	if (bits.size() == 2) {
	    ...
	    if (mainWindow_.shutdownConfiguration(current->getName())) {
...
ConfigurationController::cleanup()
{
	...
	int rc = QMessageBox::critical(
	    qApp->activeWindow(), "Purge Remote Processes", text,
...
ConfigurationController::updatePaths()
{
    ...
    if (mainWindow_.getUtilsWidget()->find_->text() != "")
	mainWindow_.getUtilsWidget()->find_->setText("");
...
ConfigurationController::lostRunner(const RunnerItem* removed)
{
    ...
    if (obj)
	obj->lostRunner(removed->getServiceName());
...
void
ConfigurationController::recordingStatusChanged(const QStringList& names,
                                                bool isRecording)
{
	...
	if (obj && obj->isRunning())
	    obj->setRecordingState(isRecording);
    }
}
...
ConfigurationController::getViewableRunningConfigurations() const
{
    ...
    for (int index = 0; index < model_->rowCount(); ++index) {
	ConfigurationInfo* obj = model_->getModelData(index);
	if (obj->isViewable() && obj->isRunning())
	    found.append(obj->getName());
    }
...
ConfigurationController::getRecordableConfigurations() const
{
    ...
    for (int index = 0; index < model_->rowCount(); ++index) {
	ConfigurationInfo* obj = model_->getModelData(index);
	if (obj->isRunning() && obj->canRecord())
	    found.append(obj->getName());
    }
...
void
ConfigurationController::updateColumns(const QModelIndex& topLeft,
                                       const QModelIndex& bottomRight)
{
    for (int column = topLeft.column(); column <= bottomRight.column();
         ++column) {
...
ConfigurationController::adjustColumnSizes()
{
	...
	available -= configurations->columnWidth(column);
    if (available >= minimum)
	configurations->setColumnWidth(0, available);
}
...
ConfigurationController::getInvalidRecordingDirectories() const
{
	...
	if (! obj->hasValidRecordingDirectory() &&
            ! invalid.contains(obj->getRecordingsDirectory()))
