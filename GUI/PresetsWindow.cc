...
PresetsWindow::PresetsWindow(int shortcut, PresetManager* presetManager)
    : Super("PresetsWindow", "Presets", shortcut),
      presetManager_(presetManager), gui_(new Ui::PresetsWindow)
{
    gui_->setupUi(this);
    ...
    connect(gui_->name_, SIGNAL(textEdited(const QString&)),
            SLOT(updateButtons()));
    connect(gui_->name_, SIGNAL(returnPressed()), SLOT(newPreset()));
    connect(gui_->presets_,
            SIGNAL(itemDoubleClicked(QListWidgetItem*)),
            SLOT(presetDoubleClicked(QListWidgetItem*)));
    connect(gui_->presets_,
            SIGNAL(currentItemChanged(QListWidgetItem*, QListWidgetItem*)),
            SLOT(selectionChanged()));
    connect(presetManager, SIGNAL(presetDirtyStateChanged(int, bool)),
            SLOT(presetDirtyStateChanged(int, bool)));
...
PresetsWindow::deletePreset()
{
	...
	if (presetManager_->removePreset(gui_->presets_->currentRow())) {
	    updateButtons();
	}
    }
}
...
PresetsWindow::activatePreset()
{
    ...
    if (presetManager_->applyPreset(gui_->presets_->currentRow())) {
	updateButtons();
    }
}
...
{
    if (getSelectedRow() != presetManager_->getActivePresetIndex())
	activatePreset();
}
...
PresetsWindow::presetDirtyStateChanged(int index, bool isDirty)
{
    ...
    LOGINFO << "index: " << index << " isDirty: " << isDirty
            << std::endl;
...
PresetsWindow::presetNamesChanged(const QStringList& names)
{
	...
	if (presetManager_->getPresetIsDirty(index))
	    name += kDirtySuffix;
	if (index < alive)
	    gui_->presets_->item(index)->setText(name);
	else {
    ...
    while (alive > names.size())
	delete gui_->presets_->takeItem(--alive);
...
{
    return gui_->presets_->item(
	presetManager_->getActivePresetIndex())->text();
}
...
void::
PresetsWindow::showEvent(QShowEvent* event)
{
