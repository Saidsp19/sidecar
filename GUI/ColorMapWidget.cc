...
ColorMapWidget::ColorMapWidget(VideoSampleCountTransform* transform,
                               QWidget* parent)
    : Super(parent), gui_(new GUI), transform_(transform),
      normalPalette_(), dbPalette_()
{
    gui_->setupUi(this);
    ...
    connect(transform_, SIGNAL(showingDecibels(bool)),
            SLOT(decibelStateChange(bool)));
    connect(gui_->db_, SIGNAL(toggled(bool)), transform_,
            SLOT(setShowDecibels(bool)));
...
ColorMapWidget::setColorMap(const QImage& colorMap)
{
    ...
    if (colorMap.width() != gui_->colorMap_->minimumWidth())
	gui_->colorMap_->setMinimumWidth(colorMap.width());
    if (colorMap.width() != gui_->colorMap_->maximumWidth())
	gui_->colorMap_->setMaximumWidth(colorMap.width());
}
...
{
    gui_->min_->setText(
	QString::number(transform_->inverseTransform(0.0)));
    gui_->q1_->setText(
	QString::number(transform_->inverseTransform(0.25)));
    gui_->q2_->setText(
	QString::number(transform_->inverseTransform(0.50)));
    gui_->q3_->setText(
	QString::number(transform_->inverseTransform(0.75)));
    gui_->max_->setText(
	QString::number(transform_->inverseTransform(1.0)));
}
...
ColorMapWidget::mouseDoubleClickEvent(QMouseEvent* event)
{
	...
	bool ok = false;
	value = QInputDialog::getInteger(window(), "Min Value",
                                         "Set lower cutoff value: ",
                                         value, low, high, 1, &ok);
	if (ok)
	    transform_->setThresholdMin(value);
	event->accept();
    }
	...
	bool ok = false;
	value = QInputDialog::getInteger(window(), "Max Value",
                                         "Set upper cutoff value: ",
                                         value, low, high, 1, &ok);
	if (ok)
	    transform_->setThresholdMax(value);
	event->accept();
    }
    else {
	event->ignore();
