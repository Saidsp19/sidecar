...
static const QStringList kPrefixes(QStringList() << "Show" << "Hide");

QAction*
SideCar::GUI::MakeMenuAction(const QString& text, QObject* parent,
                             const char* slot, int shortcut)
{
...
{
    if (newCheckedState != action->isChecked()) {
	action->setChecked(newCheckedState);
    }
...
QString
SideCar::GUI::RadiansToString(double value, bool decimalFormat, int precision,
                              int fieldWidth)
{
...
QString
SideCar::GUI::DegreesToString(double value, bool decimalFormat, int precision,
                              int fieldWidth)
{
    if (decimalFormat) {
	QString tmp = QString::number(value, 'f', precision);
	...
	.arg(deg, 3, int(10), QLatin1Char('0')).arg(DegreeSymbol())
...
SideCar::GUI::ProjectPoint(double objx, double objy, double objz, const double modelMatrix[16],
                           const double projMatrix[16], const int viewport[4],
