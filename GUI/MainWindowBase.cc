...
MainWindowBase::Log()
{
	...
	Logger::Log::Find("SideCar.GUI.MainWindowBase");
    return log_;
}

MainWindowBase::MainWindowBase()
    : QMainWindow(), windowMenu_(0), contextMenu_(0),
      toolBarMenu_(new QMenu("Toolbars")), showAction_(0),
...
{
    Logger::ProcLog log("~MainWindowBase", Log());
    LOGINFO << "objectName: " << objectName() << " title: " << windowTitle()
	    << std::endl;
}
...
MainWindowBase::makeMenuBar()
{
    ...
    if (! windowMenu_)
	windowMenu_ = getApp()->makeWindowMenu(this);
	...
	if (toolsMenu)
	    mb->insertMenu(existing.front(), toolsMenu);
    }
...
MainWindowBase::changeEvent(QEvent* event)
{
    ...
    LOGINFO << "windowState: " << std::hex << int(windowState()) << std::dec
            << std::endl;

    Super::changeEvent(event);
    if (event->type() == QEvent::WindowStateChange) {
	QWindowStateChangeEvent* tmp =
...
QAction*
MainWindowBase::makeMenuAction(const QString& title, const char* slot,
                               int shortcut)
{
...
MainWindowBase::makeToolBar(const QString& title, Qt::ToolBarArea area)
{
	...
	title.split(' ', QString::SkipEmptyParts).join(""));
