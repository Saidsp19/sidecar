...
WindowManager::WindowManager(QObject* parent) :
    QObject(parent), managedWindows_(), managedWindowMenus_(), managedWindowRaiseActions_(), activeMainWindow_(0)
{
...
WindowManager::makeWindowMenuActions()
{
    ...
    //
    windowMenuActions_.push_back(MakeMenuAction("New", AppBase::GetApp(), SLOT(windowNew()), Qt::CTRL + Qt::Key_N));
    windowMenuActions_.push_back(MakeMenuAction("Close", this, SLOT(windowClose()), Qt::CTRL + Qt::Key_W));
    ...
    windowMenuActions_.push_back(MakeMenuAction("Minimize", this, SLOT(windowMinimize()), Qt::CTRL + Qt::Key_M));
    windowMenuActions_.push_back(
        MakeMenuAction("Maximize", this, SLOT(windowMaximize()), Qt::CTRL + Qt::SHIFT + Qt::Key_M));
    QAction* action = windowMenuActions_.back();
    action->setData(QStringList() << "Maximize"
                                  << "Unmaximize");

    windowMenuActions_.push_back(
        MakeMenuAction("Enter Full-Screen Mode", this, SLOT(windowFullScreen()), Qt::CTRL + Qt::Key_F));
    action = windowMenuActions_.back();
    action->setToolTip("Enter full-screen mode.");
    action->setIcon(QIcon(":/fullscreen.png"));
    action->setData(QStringList() << "Enter"
                                  << "Leave");
...
WindowManager::getManagedWindow(const QString& title) const
{
    for (int index = 0; index < managedWindows_.size(); ++index) {
        if (title == managedWindows_[index]->windowTitle()) return managedWindows_[index];
    }
...
WindowManager::windowTitleChanged(QWidget* window)
{
    ...
    LOGINFO << "window: " << window << " new title: " << window->windowTitle() << std::endl;
    int index = managedWindows_.indexOf(window);
    if (index == -1) return;
    LOGDEBUG << "index: " << index << " old title: " << managedWindowRaiseActions_[index]->text() << std::endl;
    managedWindowRaiseActions_[index]->setText(
        windowTitleToActionText(window->windowTitle(), window->isWindowModified()));
}
...
WindowManager::eventFilter(QObject* obj, QEvent* event)
{
    ...
    if (event->type() == QEvent::ActivationChange) {
        LOGDEBUG << "window: " << widget->windowTitle()
                 << " activation changed - isActive: " << widget->isActiveWindow() << std::endl;
        if (widget->isActiveWindow()) { setActiveWindow(widget); }
    } else if (event->type() == QEvent::WindowTitleChange) {
        LOGDEBUG << "window: " << widget->windowTitle() << " windowTitleChange" << std::endl;
...
WindowManager::manageWindow(QWidget* window, QMenu* windowMenu)
{
    ...
    if (managedWindows_.indexOf(window) != -1) {
        LOGDEBUG << window->windowTitle() << " is already managed" << std::endl;
        return;
    }
    ...
    //
    QAction* action = new QAction(windowTitleToActionText(window->windowTitle(), window->isWindowModified()), this);
    ...
    for (int index = 0; index < managedWindowMenus_.size(); ++index)
        if (managedWindowMenus_[index]) managedWindowMenus_[index]->addAction(action);

    LOGDEBUG << "managedWindows_.size: " << managedWindows_.size()
             << " managedWindowRaiseActions_.size: " << managedWindowRaiseActions_.size() << std::endl;
}
...
WindowManager::unmanageWindow(QWidget* window)
{
    ...
    for (; index < managedWindowRaiseActions_.size() && index < 9; ++index) {
        managedWindowRaiseActions_[index]->setShortcut(Qt::CTRL + Qt::ALT + Qt::Key_1 + index);
    }
...
WindowManager::setActiveWindow(QWidget* window)
{
    ...
    managedWindowRaiseActions_.push_back(managedWindowRaiseActions_.takeAt(index));
    for (int index = 0; index < managedWindowRaiseActions_.size() - 1; ++index) {
    ...
    MainWindowBase* mainWindow = qobject_cast<MainWindowBase*>(window);
    if (mainWindow) setActiveMainWindow(mainWindow);
}
...
WindowManager::setActiveMainWindow(MainWindowBase* window)
{
    ...
    if (window != activeMainWindow_) {
        ...
        if (window) emit activeMainWindowChanged(window);
    }
}
...
WindowManager::windowMenuAboutToShow()
{
    ...
    if (main) {
        ...
        LOGDEBUG << "menu: " << menu << ' ' << menu->actions().size() << std::endl;

        if (windowMenuActions_[kToolBarMenu]->menu() != menu) {
            windowMenuActions_[kToolBarMenu]->setMenu(menu);
            windowMenuActions_[kToolBarMenu]->setEnabled(!menu->actions().empty());
        } else if (!menu) {
    ...
    //
    windowMenuActions_[kSeparator1]->setVisible(true || windowMenuActions_[kNew]->isVisible() ||
                                                windowMenuActions_[kClose]->isVisible());

    if (active) {
        windowMenuActions_[kMinimize]->setText(active->isMinimized() ? "Unminimize" : "Minimize");
        windowMenuActions_[kMaximize]->setText(active->isMaximized() ? "Unmaximize" : "Maximize");
        windowMenuActions_[kFullScreen]->setEnabled(active == main);
        windowMenuActions_[kFullScreen]->setText(active->isFullScreen() ? "Leave Full Screen Mode"
    ...
    for (int index = 0; index < managedWindowRaiseActions_.size(); ++index) {
        ...
        managedWindowRaiseActions_[index]->setText(
            windowTitleToActionText(managedWindows_[index]->windowTitle(), modified));
    }
}
...
{
    if (activeMainWindow_) activeMainWindow_->statusBar()->showMessage(msg, duration);
}
