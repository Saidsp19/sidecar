...
const char* SegmentModel::kColumnNames_[] = {"Duration (s)", "Vel X (m/s)",  "Vel Y (m/s)",  "Vel Z (m/s)",
                                             "Acc X (m/s2)", "Acc Y (m/s2)", "Acc Z (m/s2)", "End (s)"};
...
SegmentModel::SegmentModel(QObject* parent) : Super(parent), segments_(), lastActiveRow_(-1), running_(false)
{
    ;
}
...
{
    while (!segments_.empty()) { delete segments_.takeLast(); }
}
...
QVariant
SegmentModel::headerData(int section, Qt::Orientation orientation, int role) const
{
...
SegmentModel::flags(const QModelIndex& index) const
{
    ...
    if (running_ || index.column() == kEnd) return Qt::NoItemFlags;

    return Qt::ItemIsSelectable | Qt::ItemIsEnabled | Qt::ItemIsEditable;
}

bool
SegmentModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    case kDuration:
        ...
        updateEnds();
        emit dataChanged(this->index(0, kEnd), this->index(rowCount() - 1, kEnd));
        break;
...
SegmentModel::getActiveSegment(double elapsed)
{
    if (lastActiveRow_ == -1) {
        ...
        if (row != lastActiveRow_) setLastActiveRow(row);
    }
...
SegmentModel::setLastActiveRow(int row)
{
    if (row != lastActiveRow_) {
        if (lastActiveRow_ != -1) { emit dataChanged(index(lastActiveRow_, 0), index(lastActiveRow_, kEnd)); }
...
SegmentModel::save(QTextStream& out) const
{
    for (int index = 0; index < segments_.size(); ++index) {
        ...
        out << seg->xVel_ << ' ' << seg->xAcc_ << ' ' << seg->yVel_ << ' ' << seg->yAcc_ << ' ' << seg->zVel_ << ' '
            << seg->zAcc_ << ' ' << seg->duration_ << '\n';
    }
}
