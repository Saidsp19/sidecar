...
TreeModel::TreeModel(QObject* parent)
    : QAbstractItemModel(parent), root_(new RootItem(this))
{
    ;
}
...
TreeModel::data(const QModelIndex& index, int role) const
{
	...
	break;
    case kInfo:
	value = item->getInfo(role);
	break;
    case kConnections:
	value = item->getConnections(role);
	break;
    default: break;
    }
...
TreeModel::index(int row, int column, const QModelIndex& parent) const
{
    ...
    if (! parentItem || row < 0 || row >= parentItem->getChildrenCount() ||
        column < 0 || column >= kNumColumns)
	return QModelIndex();
...
bool
TreeModel::insertItems(int position, const QList<TreeItem*>& items,
...
{
    if (! index.isValid() || role != Qt::EditRole || index.column() != kName)
	return false;
    TreeItem* item = getItem(index);
