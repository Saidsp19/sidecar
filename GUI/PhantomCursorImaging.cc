...
PhantomCursorImaging::Log()
{
	...
	Logger::Log::Find("SideCar.GUI.PhantomCursorImaging");
    return log_;
}

PhantomCursorImaging::PhantomCursorImaging(BoolSetting* enabled,
                                           ColorButtonSetting* color,
                                           DoubleSetting* size,
                                           OpacitySetting* opacity,
                                           IntSetting* radius)
    : ChannelImaging(enabled, color, size, opacity), radius_(radius)
{
    add(radius);
}
...
void
PhantomCursorImaging::drawCursor(const QPointF& pos, double xScale,
                                 double yScale) const
{
    static Logger::ProcLog log("drawCursor", Log());
    LOGINFO << "pos: " << pos << " enabled: " << isEnabled()
	    << " isValid: " << IsValidCursor(pos) << std::endl;
