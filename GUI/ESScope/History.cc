...
History::History(QObject* parent, RadarSettings* radarSettings,
                 TargetPlotImaging* extractionsImaging,
                 RangeTruthsImaging* rangeTruthsImaging,
                 TargetPlotImaging* bugPlotsImaging, BoolSetting* allynHack)
    : QObject(parent), radarSettings_(radarSettings), alphaBetaData_(),
      alphaRangeData_(), extractions_(), rangeTruths_(), bugPlots_(),
      allynHack_(allynHack),
      extractionsLifeTime_(extractionsImaging->getLifeTime()),
      rangeTruthsLifeTime_(rangeTruthsImaging->getLifeTime()),
      rangeTruthsMaxTrailLength_(rangeTruthsImaging->getTrailSize()),
      bugPlotsLifeTime_(bugPlotsImaging->getLifeTime()), scanCounter_(0),
      lastAlphaIndex_(-1)
{
    connect(radarSettings, SIGNAL(scansChanged(int, int, int)),
            SLOT(scansChanged(int, int, int)));
    scansChanged(radarSettings->getAlphaScans(), radarSettings->getBetaScans(),
                 radarSettings->getRangeScans());
    connect(extractionsImaging, SIGNAL(lifeTimeChanged(int)),
            SLOT(extractionsLifeTimeChanged(int)));
    connect(rangeTruthsImaging, SIGNAL(lifeTimeChanged(int)),
            SLOT(rangeTruthsLifeTimeChanged(int)));
    connect(rangeTruthsImaging, SIGNAL(trailSizeChanged(int)),
            SLOT(rangeTruthsMaxTrailLengthChanged(int)));
    connect(bugPlotsImaging, SIGNAL(lifeTimeChanged(int)),
            SLOT(bugPlotsLifeTimeChanged(int)));
}
...
History::scansChanged(int alphaScans, int betaScans, int rangeScans)
{
    ...
    for (int index = 0; index < alphaScans; ++index)
	alphaRangeData_[index].resize(rangeScans);
}
...
History::clearVideo()
{
    for (size_t index = 0; index < alphaBetaData_.size(); ++index) {
	alphaBetaData_[index].reset();
    }
...
History::processVideo(const MessageList& data)
{
	...
	// !!! HACK !!!
	//
	if (! allynHack_->getValue() && msg->getRIUInfo().prfEncoding == 0)
	    continue;
	...
	//
	radarSettings_->setRangeScaling(msg->getRangeMin(),
                                        msg->getRangeFactor());

	int alphaIndex = radarSettings_->getAlphaIndex(msg);
	if (alphaIndex == -1) {
	    LOGERROR << "invalid alpha value: "
		     << radarSettings_->getAlpha(msg)
		     << " - skipping message " << msg->getSequenceCounter()
		     << std::endl;
	    continue;
	}
	...
	if (alphaIndex < lastAlphaIndex_) {
	    LOGINFO << "seq: " << msg->getSequenceCounter()
		    << " lastAlphaIndex: " << lastAlphaIndex_ << " alphaIndex: "
		    << alphaIndex << std::endl;
	    ++scanCounter_;
	...
	//
	if (alphaIndices.empty() || alphaIndices.back() != alphaIndex)
	    alphaIndices.push_back(alphaIndex);

	int betaIndex = radarSettings_->getBetaIndex(msg);
	if (betaIndex == -1) {
	    LOGERROR << "invalid beta value: "
		     << radarSettings_->getBeta(msg)
		     << " - skipping message " << msg->getSequenceCounter()
		     << std::endl;
	    continue;
	}
	...
	//
	if (value > alphaBetaData_[alphaBetaIndex].value ||
            scanCounter_ != alphaBetaData_[alphaBetaIndex].scan) {
	...
	//
	alphaRangeData_[alphaIndex].update(msg, radarSettings_,
                                           scanCounter_);
    }
...
History::processExtractions(const MessageList& data)
{
	...
	QString tag(QString("%1/%2").arg(
                        msg->getMessageSequenceNumber()));
	for (size_t index = 0; index < msg->size(); ++index) {
	    extractions_.push_back(TargetPlot(msg[index],
                                              tag.arg(index)));
	}
    }
}
