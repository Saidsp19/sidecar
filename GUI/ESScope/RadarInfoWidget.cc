...
RadarInfoWidget::Log()
{
    static Logger::Log& log_ = Logger::Log::Find("esscope.RadarInfoWidget");
    return log_;
}

RadarInfoWidget::RadarInfoWidget(QWidget* parent) :
    Super(parent), gui_(new Ui::RadarInfoWidget), app_(App::GetApp()),
    radarSettings_(app_->getConfiguration()->getRadarSettings()), updateTimer_(), alpha_(0.0), beta_(0.0),
    irigTime_(0.0), hasIRIGTime_(true), updateAlpha_(true), updateBeta_(true), updateTime_(true)
{
    gui_->setupUi(this);
    gui_->value_->setTextFormat(Qt::PlainText);
    setTilt(radarSettings_->getTilt());
    setRotation(radarSettings_->getRotation());
    connect(AppBase::GetApp(), SIGNAL(angleFormattingChanged(int)), SLOT(refreshTilt()));
    connect(AppBase::GetApp(), SIGNAL(angleFormattingChanged(int)), SLOT(refreshRotation()));
    connect(radarSettings_, SIGNAL(tiltChanged(double)), SLOT(setTilt(double)));
    connect(radarSettings_, SIGNAL(rotationChanged(double)), SLOT(setRotation(double)));
...
{
    tiltText_ = QString("  Tilt: %1").arg(app_->getFormattedAngleDegrees(value));
    refresh();
}
...
{
    rotationText_ = QString("  Rot: %1").arg(app_->getFormattedAngleDegrees(value));
    refresh();
}
...
RadarInfoWidget::timerEvent(QTimerEvent* event)
{
    ...
    if (event->timerId() == updateTimer_.timerId()) {
        if (updateAlpha_ || updateBeta_ || updateTime_) { refresh(); }
    } else {
