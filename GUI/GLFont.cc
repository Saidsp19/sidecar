...
GLFont::GLFont(const QFont& font)
    : characterRenderInfos_(), font_(font), fontMetrics_(font)
{
    if (textureType_ == -1)
...
GLFont::render(const VertexColorTagArray& tags)
{
	...
	LOGDEBUG << "xy: " << tag.vertex.x << ' ' << tag.vertex.y
	...
	foreach (QChar character, tag.tag) {
	    CharacterRenderInfoHash::const_iterator pos =
		characterRenderInfos_.find(character);
	    if (pos != characterRenderInfos_.end()) {
		glCallList(pos.value().displayList_);
	    }
	    else {
		LOGERROR << "no render info for character '"
			 << QString(character) << "'" << std::endl;
	    }
	}
...
GLFont::analyzeText(const QString& text)
{
	...
	LOGDEBUG << "generating texture for character '" << QString(character)
		 << "'" << std::endl;
	...
	LOGDEBUG << "paddedWidth: " << paddedWidth
		 << " paddedHeight: " << paddedHeight << std::endl;

	QImage paddedImage = QImage(paddedWidth, paddedHeight,
	...
	if (QSysInfo::ByteOrder == QSysInfo::BigEndian) {
		...
		uint32_t* paddedImageRgb = (uint32_t*)paddedImage.scanLine(
		    paddedHeight - 1 - y);
		...
		while (imageRgb < end) {
		    ...
		    // ABGR
		    //
		    *paddedImageRgb++ = ((*imageRgb++ << 24) & 0xFF000000) |
			0x00FFFFFF;
		}
	    }
        }
	...
	//
        ...
        glTexImage2D(textureType_, 0, GL_ALPHA, paddedImage.width(),
                     paddedImage.height(), 0, GL_RGBA, GL_UNSIGNED_BYTE,
                     paddedImage.bits()); 
	...
	LOGERROR << "minX: " << minX << " minY: " << minY << " maxX: " << maxX
		 << " maxY: " << maxY << std::endl;
