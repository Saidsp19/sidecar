...
RecordingInfo::RecordingInfo(const QDir& recordingDir)
    : configurationNames_(), recordingDir_(recordingDir.absolutePath()),
      name_(recordingDir.dirName()), start_(), elapsed_(), dropCount_(0),
      dupeCount_(0)
{
    QStringList bits = recordingDir.dirName().split("-");
    if (bits.size() > 1) {
	QDateTime start = QDateTime::fromString(bits[0] + "-" + bits[1],
                                                "yyyyMMdd-HHmmss");
...
RecordingInfo::loadFromFile()
{
	...
	if (tag == "Configurations:") {
	    ...
	    if (bits.size() > 1)
		elapsed_ = QString("  %1  ").arg(QString(bits[1]));
	}
	else if (tag == "Dropped") {
	    if (bits.size() > 2)
...
RecordingInfo::getRadarFreqency() const
{
	...
	QString::number(radarFrequency_) : QString("");
}
...
RecordingInfo::getRadarRotationRate() const
{
	...
	QString::number(radarRotationRate_) : QString("");
}
