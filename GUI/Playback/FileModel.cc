...
FileModel::FileModel(MainWindow* parent) :
    QAbstractTableModel(parent), parent_(parent), emitters_(), startTime_(), endTime_(), suffix_(parent->getSuffix()),
    loaders_(), failures_(), loaderMeter_(0), dir_()
{
    connect(parent, SIGNAL(suffixChanged(const QString&)), SLOT(suffixChanged(const QString&)));
}
...
FileModel::beginLoad(const QString& path)
{
    ...
    if (files.size() == 0) {
        QMessageBox::warning(parent_, "Failed", "Failed to find any PRI files to load.", QMessageBox::Ok);
        emit loadComplete();
        return;
    }
    ...
    loaderMeter_->setLabelText(QString("Loading %1 file%2...").arg(files.size()).arg(files.size() == 1 ? "" : "s"));
    loaderMeter_->setCancelButton(0);
    loaderMeter_->setRange(0, 0);
    loaderMeter_->setWindowModality(Qt::WindowModal);
    loaderMeter_->show();

    connect(loaderMeter_, SIGNAL(finished(int)), SLOT(finishedLoading()));
    ...
    for (int index = 0; index < files.size(); ++index) {
        ...
        connect(emitter, SIGNAL(loadPercentageUpdate(int)), this, SLOT(updateLoadPercentage(int)),
                Qt::QueuedConnection);
        connect(emitter, SIGNAL(subscriberCountChanged(int)), this, SLOT(updateSubscriberCount(int)));
        emitters_.append(emitter);
        LoaderThread* loader = new LoaderThread(emitter, fileInfo);
        loaders_.append(loader);
        connect(loader, SIGNAL(finished()), SLOT(loaderFinished()), Qt::QueuedConnection);
        loader->start();
    }
...
FileModel::loaderFinished()
{
    ...
    if (emitter->isValid()) {
        connect(parent_, SIGNAL(addressChanged(const QString&)), emitter, SLOT(setAddress(const QString&)));
        connect(parent_, SIGNAL(suffixChanged(const QString&)), emitter, SLOT(setSuffix(const QString&)));
        if (emitter->getStartTime() < startTime_) startTime_ = emitter->getStartTime();
        if (emitter->getEndTime() > endTime_) endTime_ = emitter->getEndTime();
    } else {
        failures_.append(emitter->getName());
    }

    emit dataChanged(createIndex(index, 0), createIndex(index, kDuration));

    if (!loaders_.empty()) {
        loaderMeter_->setLabelText(
            QString("Loaded %1 - %2 remaining...").arg(emitters_.size() - loaders_.size()).arg(loaders_.size()));
        return;
    }
    ...
    if (failures_.size() == emitters_.size()) {
        QMessageBox::warning(parent_, "Failed", "Failed to load any PRI files without errors.", QMessageBox::Ok);
    } else if (!failures_.empty()) {
        if (failures_.size() == 1) {
            QMessageBox::warning(parent_, "Load Failure",
                                 QString("Failed to load the PRI file '%1'").arg(failures_[0]));
        } else {
            QMessageBox::warning(parent_, "Load Failure",
...
{
    if (!pos.isValid() || pos.row() >= rowCount()) return QVariant();
    ...
    case kEmitting:
        if (role == Qt::CheckStateRole || role == Qt::EditRole)
            ...
            value = emitter->getEmitting() ? "Click to stop emitting data for this channel"
    ...
    case kStartTime:
        if (role == Qt::DisplayRole) value = emitter->getFormattedStartTime();
        break;

    case kEndTime:
        if (role == Qt::DisplayRole) value = emitter->getFormattedEndTime();
        break;

    case kDuration:
        if (role == Qt::DisplayRole) value = emitter->getFormattedDuration();
        break;

    case kSubscriberCount:
        if (role == Qt::DisplayRole) value = int(emitter->getSubscriberCount());
        break;
...
FileModel::flags(const QModelIndex& index) const
{
    ...
    if (index.column() == kEmitting) flags |= Qt::ItemIsUserCheckable;
...
{
    suffix_ = suffix;
    emit dataChanged(createIndex(0, kName), createIndex(rowCount() - 1, kName));
}
