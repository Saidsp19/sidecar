...
BinaryOffscreenBuffer::BinaryOffscreenBuffer(int width, int height, int textureType) :
    Super(App::GetApp()->getConfiguration()->getBinaryImaging(), width, height, textureType)
{
    ;
}
...
BinaryOffscreenBuffer::addPoints(const Messages::PRIMessage::Ref& msg)
{
    ...
    if (decimation > 1) {
        ...
        while (decimator) {
            ...
            addPoint(Vertex(range * sine - offset, range * cosine - offset), decimator.getValue() ? on : off);
        }
    } else {
        ...
        while (pos != end) {
            ...
            if (range > rangeMax) break;
            addPoint(Vertex(range * sine - offset, range * cosine - offset), *pos++ ? on : off);
        }
    }
}
