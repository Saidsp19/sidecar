...
{
    static Logger::Log& log_ = Logger::Log::Find("ppidisplay.BackgroundImageSettings");
    return log_;
}

BackgroundImageSettings::BackgroundImageSettings(BoolSetting* enabled, PathSetting* path, OpacitySetting* opacity) :
    Super(enabled), path_(path), opacity_(opacity), image_(), filtered_()
{
    connect(path, SIGNAL(valueChanged(const QString&)), this, SLOT(pathChanged(const QString&)));
    connect(opacity, SIGNAL(valueChanged(double)), this, SLOT(update(double)));
    connect(enabled, SIGNAL(valueChanged(bool)), this, SLOT(detectEnabledChanged(bool)));
    if (!path_->getValue().isEmpty()) pathChanged(path->getValue());
}
...
BackgroundImageSettings::pathChanged(const QString& path)
{
    ...
    image_ = reader.read();
    if (image_.isNull()) {
        LOGERROR << "failed read - " << reader.error() << ' ' << reader.errorString() << std::endl;
    } else {
...
BackgroundImageSettings::update(double opacity)
{
    ...
    if (!filtered_.isNull()) filtered_ = QImage();
