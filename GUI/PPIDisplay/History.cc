...
History::Entry::Entry(size_t lastVideoSize, size_t lastBinarySize) :
    video_(), binary_(), extractions_(), rangeTruths_(), bugPlots_()
{
    size_t capacity = lastVideoSize ? calculateCapacity(lastVideoSize) : 5000;
    video_.reserve(capacity);
    if (lastBinarySize) binary_.reserve(capacity);
    clearAll();
}

PRIMessage::Ref
History::Entry::findByAzimuth(const MessageVector& data, double azimuth) const
{
...
Video::DatumType
History::Entry::getVideoValue(double azimuth, double range, bool& isValid) const
{
...
History::Entry::getBinaryValue(double azimuth, double range, bool& isValid) const
{
...
History::Entry::freeze()
{
    ...
    LOGINFO << "video size: " << (lastVideoSlot_ + 1) << "binary size: " << (lastBinarySlot_ + 1) << std::endl;

    // Reduce vectors if their size is larger than the last recorded entry.
    //
    if ((lastVideoSlot_ + 1) < video_.size()) video_.resize(lastVideoSlot_ + 1);
    if ((lastBinarySlot_ + 1) < binary_.size()) binary_.resize(lastBinarySlot_ + 1);
}
...
History::Entry::addVideo(const Messages::Video::Ref& msg)
{
    ...
    if (msg->getShaftEncoding() == last->getShaftEncoding()) {
        ...
        if (last->getAzimuthStart() > msg->getAzimuthStart() + M_PI) {
            LOGDEBUG << "detected full entry - " << lastVideoSlot_ << std::endl;
            filled = true;
            lastVideoSlot_ = 0;
            video_[0] = msg;
...
History::Entry::addBinary(const Messages::BinaryVideo::Ref& msg)
{
    ...
    if (msg->getShaftEncoding() == last->getShaftEncoding()) {
        ...
        if (last->getAzimuthStart() > msg->getAzimuthStart() + M_PI) {
            LOGDEBUG << "detected full entry - " << lastBinarySlot_ << std::endl;
            lastBinarySlot_ = 0;
            binary_[0] = msg;
...
History::History(QObject* parent, size_t retentionSize, bool enabled) :
    QObject(parent), retained_(), retentionSize_(retentionSize), oldestRetained_(0), liveEntry_(5000, 0),
    pendingEntry_(0), viewedEntry_(&liveEntry_), viewedAge_(0), viewedEntryIsRetained_(false), enabled_(enabled)
{
    ;
}
...
{
    if (enabled_) enabled_ = newSize != 0;
    ...
    if (size) {
        if (oldestRetained_) {
            std::rotate(retained_.begin(), retained_.begin() + oldestRetained_, retained_.end());
            oldestRetained_ = 0;
        }
    }
    ...
    if (size_t(newSize) < size) {
        ...
        for (size_t index = 0; index < reduction; ++index) {
            if (viewedEntryIsRetained_ && retained_[index] == viewedEntry_) {
...
History::addVideo(const Messages::Video::Ref& msg)
{
    ...
    if (filled) {
        IO::MessageManager::AllocationStats stats = IO::MessageManager::GetAllocationStats();
        LOGDEBUG << "liveEntry pendingEntry - video size: " << liveEntry_.getVideo().size()
                 << " binary size: " << liveEntry_.getBinary().size() << std::endl;
        LOGDEBUG << "msg blocks: " << stats.messageBlocks.numBlocks << ' ' << stats.messageBlocks.numAllocated << ' '
                 << stats.messageBlocks.numInUse << ' ' << stats.messageBlocks.numFree << std::endl;
    }
    ...
    if (enabled_) {
        ...
        if (filled && pendingEntry_) {
            ...
            LOGDEBUG << "filled pendingEntry - video size: " << pendingEntry_->getVideo().size()
                     << " binary size: " << pendingEntry_->getBinary().size() << std::endl;

            // If we have yet to expand to the retention limit, just append.
            //
            if (retained_.size() < retentionSize_) {
                LOGDEBUG << "appending to retained array - " << retained_.size() << std::endl;
                oldestRetained_ = 0;
                retained_.push_back(pendingEntry_);
                pendingEntry_ = new Entry(pendingEntry_->getVideo().size(), pendingEntry_->getBinary().size());
                emit retainedCountChanged(retained_.size());
            } else {
                LOGDEBUG << "resusing oldest entry - " << oldestRetained_ << std::endl;
                ...
                if (oldestRetained_ == retentionSize_) oldestRetained_ = 0;
                ...
                if (oldest == viewedEntry_) {
                    ...
                    viewedEntryIsRetained_ = false;
                    oldest = new Entry(pendingEntry_->getVideo().size(), pendingEntry_->getBinary().size());
                } else {
                    oldest->clearAll();
                }
        ...
        if (!pendingEntry_) {
            ...
            pendingEntry_ = new Entry(liveEntry_.getVideo().size(), liveEntry_.getBinary().size());
        }
...
History::addBinary(const Messages::BinaryVideo::Ref& msg)
{
    if (liveEntry_.hasVideo()) {
        ...
        if (pendingEntry_) pendingEntry_->addBinary(msg);
    }
}
...
History::addExtractions(const Messages::Extractions::Ref& msg)
{
    if (liveEntry_.hasVideo()) {
        ...
        if (pendingEntry_) pendingEntry_->addExtractions(msg);
    }
}
...
History::addRangeTruth(const Messages::TSPI::Ref& msg)
{
    ...
    if (pendingEntry_) pendingEntry_->addRangeTruth(msg);
}
...
History::addBugPlot(const Messages::BugPlot::Ref& msg)
{
    ...
    if (pendingEntry_) pendingEntry_->addBugPlot(msg);
}
...
History::showLiveEntry()
{
    ...
    if (viewedEntry_ != &liveEntry_) {
        if (!viewedEntryIsRetained_) delete viewedEntry_;
...
History::showEntry(int slot)
{
    ...
    if (index >= retained_.size()) index -= retained_.size();
    ...
    if (viewedEntry_ == retained_[index]) return;

    if (viewedEntry_ != &liveEntry_ && !viewedEntryIsRetained_) delete viewedEntry_;
...
{
    liveEntry_.clearAll();
    if (pendingEntry_) pendingEntry_->clearAll();
}
...
History::clearVideo()
{
    ...
    if (pendingEntry_) pendingEntry_->clearVideo();
}
...
History::clearBinary()
{
    ...
    if (pendingEntry_) pendingEntry_->clearBinary();
}
...
History::clearExtractions()
{
    ...
    if (pendingEntry_) pendingEntry_->clearExtractions();
}
...
History::clearRangeTruths()
{
    ...
    if (pendingEntry_) pendingEntry_->clearRangeTruths();
}
...
History::clearBugPlots()
{
    ...
    if (pendingEntry_) pendingEntry_->clearBugPlots();
}
...
History::setEnabled(bool enabled)
{
    ...
    if (!enabled_) {
        ...
        pendingEntry_ = 0;
        for (size_t index = 0; index < retained_.size(); ++index) delete retained_[index];
        retained_.clear();
        oldestRetained_ = 0;
    }
}
