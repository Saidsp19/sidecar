...
ScreenCaptureWindow::ScreenCaptureWindow(QAction* action) :
    ToolWindowBase("ScreenCapture", action), timer_(), writer_(new ImageWriter), display_(0)
{
    setupUi(this);
    setFixedSize();
    progressBar_->hide();
    ...
    connect(writer_, SIGNAL(processedFrame()), SLOT(updateStatusBar()), Qt::QueuedConnection);
...
ScreenCaptureWindow::on_startStop__clicked()
{
    ...
    if (!timer_.isActive()) {
        QString filePath =
            QFileDialog::getSaveFileName(0, "Capture File", filePath_->text(), QString("Images (*.%1)").arg(kSuffix));
        if (filePath.isEmpty()) return;

        QFileInfo fileInfo(filePath);
        if (fileInfo.suffix() != kSuffix) filePath += QString(".%1").arg(kSuffix);
...
ScreenCaptureWindow::capture()
{
    ...
    if (elapsed >= duration_->value()) stop();
}
