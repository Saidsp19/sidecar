...
TargetPlot::TargetPlot(const Messages::BugPlot& msg) :
    range_(msg.getRange()), azimuth_(msg.getAzimuth()), elevation_(msg.getElevation()), x_(range_ * ::sin(azimuth_)),
    y_(range_ * ::cos(azimuth_)), tag_(QString::fromStdString(msg.getTag())), age_()
{
    age_.start();
}
...
TargetPlot::TargetPlot(const Messages::TSPI& rangeTruth) :
    range_(rangeTruth.getRange() / 1000.0), azimuth_(rangeTruth.getAzimuth()), elevation_(rangeTruth.getElevation()),
    x_(range_ * ::sin(azimuth_)), y_(range_ * ::cos(azimuth_)), tag_(QString::fromStdString(rangeTruth.getTag())),
    age_()
{
    age_.start();
}
