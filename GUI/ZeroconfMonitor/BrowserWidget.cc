...
BrowserWidget::BrowserWidget(QWidget* parent) : QListWidget(parent), browser_(0), label_(0)
{
...
{
    if (label_) label_->setText(QString("%1 - %2").arg(title_).arg(count()));
}
...
BrowserWidget::foundServices(const ServiceEntryList& services)
{
    for (int index = 0; index < services.count(); ++index) {
        ...
        addItem(item);
        connect(service, SIGNAL(resolved(ServiceEntry*)), SLOT(resolved(ServiceEntry*)));
        service->resolve();
    }
...
BrowserWidget::resolved(ServiceEntry* service)
{
    for (int row = 0; row < count(); ++row) {
        ...
        if (item->toolTip() == service->getName()) {
            ...
            item->setText(
                QString("%1 [%2/%3]").arg(service->getName()).arg(service->getHost()).arg(service->getPort()));
        }
    }
}
