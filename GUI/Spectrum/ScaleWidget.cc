...
ScaleWidget::Log()
{
    static Logger::Log& log_ = Logger::Log::Find("spectrum.ScaleWidget");
    return log_;
}

ScaleWidget::ScaleWidget(QWidget* parent, Qt::Orientation orientation) :
    QWidget(parent), label_(""), inset_(0), majorTickHeight_(10), minorTickHeight_(3), majorTickDivisionsMax_(8),
...
ScaleWidget::makeTicks()
{
    ...
    while (1) {
        double x = inset_ + tickCounter * minorIncrement_;
...
ScaleWidget::drawGridLines(QPainter& painter)
{
    ...
    if (orientation_ == Qt::Horizontal) {
        ...
        painter.save();
        painter.translate(device->width(), 1);
        painter.rotate(90.0);
