...
WorkRequest::WorkRequest(const WorkRequest& copy) :
    weightWindow_(copy.weightWindow_), gateStart_(copy.gateStart_), inputSmoothing_(copy.inputSmoothing_),
...
WorkRequest::~WorkRequest()
{
    ...
    if (plan_ && master_) fftw_destroy_plan(plan_);
}
...
WorkRequest::initialize()
{
    ...
    if (master_) {
        ...
        plan_ = fftw_plan_dft_1d(fftSize_, input_, output_, FFTW_FORWARD, FFTW_MEASURE);
    }
...
WorkRequest::addData(const Messages::Video::Ref& msg)
{
    ...
    if (gateCount <= size_t(gateStart_)) return false;

    gateCount -= gateStart_;
    size_t limit = fftSize_;
    if (limit > gateCount) limit = gateCount;
