...
ColorMapWidget::ColorMapWidget(double minCutoff, double maxCutoff,
                               QWidget* parent)
    : Super(parent), gui_(new GUI), minCutoff_(minCutoff),
...
ColorMapWidget::changeColorMap()
{
    ...
    App::GetApp()->getConfiguration()->getSpectrographImaging()
	->setColorMapIndex(action->data().toInt());
}
...
ColorMapWidget::mouseDoubleClickEvent(QMouseEvent* event)
{
	...
	bool ok = false;
	double value = QInputDialog::getDouble(window(), "Min Cutoff",
                                               "Min Cutoff: ", minCutoff_,
                                               -200.0, maxCutoff_, 1, &ok);
	if (ok) {
	    App::GetApp()->getConfiguration()->
		getSpectrographImaging()->setMinCutoff(value);
	}
	event->accept();
    }
    else if (x > size().width() * 0.80) {
	bool ok = false;
	double value = QInputDialog::getDouble(window(), "Max Cutoff",
                                               "Max Cutoff: ", maxCutoff_,
                                               minCutoff_, 0.0, 1, &ok);
	if (ok) {
	    App::GetApp()->getConfiguration()->
		getSpectrographImaging()->setMaxCutoff(value);
	}
	event->accept();
    }
    else {
