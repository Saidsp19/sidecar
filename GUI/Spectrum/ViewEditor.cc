...
ViewEditor::ViewEditor(int shortcut)
    : ToolWindowBase("ViewEditor", "View Editor", shortcut), Ui::ViewEditor(),
      spectrumWidget_(0), xMinScaleValue_(1.0), xMaxScaleValue_(1.0)
{
    setupUi(this);
    setFixedSize();
...
ViewEditor::showView(const ViewSettings& view)
{
    ...
    if (view.getYMin() != yMin_->value())
	yMin_->setValue(view.getYMin());
    
    if (view.getYMax() != yMax_->value())
	yMax_->setValue(view.getYMax());
}

ViewSettings
ViewEditor::makeView() const
{
    return ViewSettings(xMinValue_, xMaxValue_, yMin_->value(),
                        yMax_->value());
}
...
ViewEditor::updateButtons()
{
    ...
    if (spectrumWidget_)
	hasChanges = spectrumWidget_->getCurrentView() != current;
