...
SpectrographScrollArea::Log()
{
	...
	Logger::Log::Find("spectrum.SpectrographScrollArea");
    return log_;
}

SpectrographScrollArea::SpectrographScrollArea(QWidget* parent)
    : Super(parent), firstShow_(true)
{
    App* app = App::GetApp();
    ...
    connect(imaging, SIGNAL(historySizeChanged(int)),
    ...
    connect(verticalScrollBar(), SIGNAL(valueChanged(int)),
            SLOT(verticalPositionChanged(int)));
    ...
    connect(fftSettings, SIGNAL(frequencyStepChanged(double)),
            SLOT(frequencyStepChanged(double)));
...
SpectrographScrollArea::resizeEvent(QResizeEvent* event)
{
    ...
    LOGINFO << "size: " << size() << " viewport: " << viewport()->size()
	    << ' ' << viewport()->pos() << std::endl;
    Super::resizeEvent(event);

    timeScale_->move(0, viewport()->pos().y());
    timeScale_->setSpan(viewport()->height());
    freqScale_->move(viewport()->pos().x(), 
                     height() - freqScale_->height() - 1);
...
SpectrographScrollArea::verticalPositionChanged(int value)
{
    ...
    LOGERROR << "range: " << range << " upper: " << upper << " lower: "
	     << lower << std::endl;
...
SpectrographScrollArea::currentCursorPositionChanged(const QPointF& pos)
{
	...
	freqScale_->formatTickTag(freqScale_->getValueAt(int(pos.x()))),
