...
LoggerTreeItem::LoggerTreeItem() : log_(Logger::Log::Root()), name_("***"), fullName_("***"), parent_(0), children_()
{
    ;
}

LoggerTreeItem::LoggerTreeItem(Logger::Log& log, LoggerTreeItem* parent) :
    log_(log), name_(QString::fromStdString(log.name())), fullName_(QString::fromStdString(log.fullName())),
...
LoggerTreeItem::getData(int column, int role) const
{
    ...
    switch (column) {
    case 0:
        if (role == Qt::DisplayRole) value = name_;
        break;

    case 1:
        if (role == Qt::DisplayRole) {
            QString name = QString::fromStdString(Logger::Priority::GetLongName(log_.getPriorityLimit()));
...
LoggerTreeItem::dump(std::ostream& os, int indent) const
{
    ...
    os << "name: " << getFullName() << " obj: " << this << " parent: " << parent_ << " children: " << children_.size()
       << std::endl;
