...
PathSetting::PathSetting(PresetManager* mgr, QLabel* viewer,
                         QPushButton* editor, const QString& prompt,
      ...
      prompt_(prompt), types_(types)
{
    ...
    connect(this, SIGNAL(valueChanged(const QString&)), viewer,
...
PathSetting::choosePath()
{
    ...
    if (last.isEmpty())
	last = "/opt/sidecar/data";

    QString file = QFileDialog::getOpenFileName(0, prompt_, last,
                                                types_);
    if (! file.isEmpty()) {
	setValue(file);
    }
