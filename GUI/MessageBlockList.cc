...
int const MessageBlockList::kMetaTypeId = qRegisterMetaType<MessageBlockList::Ref>("MessageBlockList::Ref");

Logger::Log&
MessageBlockList::Log()
{
    static Logger::Log& log_ = Logger::Log::Find("SideCar.GUI.MessageBlockList");
    return log_;
}

MessageBlockList::MessageBlockList() : data_()
{
...
MessageBlockList::~MessageBlockList()
{
    ...
    for (size_t index = 0; index < data_.size(); ++index) data_[index]->release();
    data_.clear();
}
