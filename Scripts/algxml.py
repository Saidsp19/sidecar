...
if os.environ.has_key( 'SIDECAR_SRC' ):
...
class InputOutput:
    ...
    def generateXML( self ):
        if self.name == '' and self.channel == '':
            ...
            print '<input type="%s" channel="%s"/>' % \
                ( self.kTag, self.type, self.channel )
        elif self.channel == '':
            print '<input type="%s" name="%s"/>' % \
                ( self.kTag, self.type, self.name )
        else:
            print '<input type="%s" name="%s" channel="%s"/>' % \
                ( self.kTag, self.type, self.name, self.channel )
...
class Parameter:
    ...
    def generateXML( self ):
        print '<param name="%s" type="%s" value="%s"/>' % ( self.name,
                                                            self.type,
                                                            self.value )
