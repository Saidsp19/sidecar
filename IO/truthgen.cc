...
const Utils::CmdLineArgs::OptionDef opts[] = {
    { 'c', "config", "use FILE for radar configuration", "FILE" },
...
main(int argc, char** argv)
{
    ...
    if (! filePath.exists()) {
	LOGERROR << "input file '" << filePath << "' does not exists"
		 << std::endl;
	return 1;
    }
	...
	LOGINFO << "using '" << value << "' for radar configuration"
		<< std::endl;
	...
	value >> timeOffset;
	LOGINFO << "applying " << timeOffset << " seconds to all timestamps"
		<< std::endl;
    }
    ...
    char c1, c2, c3, c4;
    int counter = 0;
    while (is >> longitude >> c1 >> latitude >> c2 >> height >> c3 >> id
           >> c4 >> timeStamp) {
	LOGDEBUG << "lat: " << latitude
		 << " lo: " << longitude
		 << " hgt: " << height
		 << " id: " << id
		 << " time: " << timeStamp
	...
	Messages::TSPI::Ref msg = Messages::TSPI::MakeLLH(
	    "TruthGen", os.str(), time.asDouble(), latitude,
	    longitude, Utils::feetToMeters(height));
	...
	if (! writer->write(mgr)) {
	    LOGERROR << "failed to write TSPI message - "
                     << writer->getLastError() << std::endl;
	    break;
	}
    }
