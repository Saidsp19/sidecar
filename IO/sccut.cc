...
const std::string about = "Extract a slice of SideCar messages. "
...
Processor::Processor(int argc, char** argv)
    : cla_(argc, argv, about, opts, sizeof(opts), args, sizeof(args)),
      inputTask_(), reader_(), writer_(), count_(0),
      t0_(Time::TimeStamp::Min()), t1_(Time::TimeStamp::Max()),
      useEmitted_(cla_.hasOpt("emitted"))
{
    ;
}
...
Processor::process()
{
	...
	std::cerr << "*** failed to open reader on file '" << inputPath << "'"
		  << std::endl;
	return false;
    }
    ...
    //
    Utils::FilePath outputPath(cla_.arg(1));
    writer_ = IO::FileWriterTask::Make();
    if (! writer_->openAndInit("", outputPath)) {
	std::cerr << "*** failed to open writer to file '" << outputPath << "'"
		  << std::endl;
	return false;
    }
    ...
    std::string spec;
    if (cla_.hasOpt("t0", spec)) {
	t0_ = Time::TimeStamp::ParseSpecification(spec, timeStamp);
    }

    if (cla_.hasOpt("t1", spec)) {
	t1_ = Time::TimeStamp::ParseSpecification(spec, t0_);
    }
	...
	if (writer_->put(data) == -1) {
	    std::cerr << "*** failed to post message to FileWriterTask\n";
	    data->release();
	}
