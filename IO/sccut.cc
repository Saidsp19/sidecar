...
Processor::Processor(int argc, char** argv) :
    cla_(argc, argv, about, opts, sizeof(opts), args, sizeof(args)), inputTask_(), reader_(), writer_(), count_(0),
    t0_(Time::TimeStamp::Min()), t1_(Time::TimeStamp::Max()), useEmitted_(cla_.hasOpt("emitted"))
{
    ;
}
...
Processor::process()
{
    ...
    if (!writer_->openAndInit("", outputPath)) {
        std::cerr << "*** failed to open writer to file '" << outputPath << "'" << std::endl;
        return false;
    }
