...
MulticastVMEReaderTask::Log()
{
	...
	Logger::Log::Find("SideCar.IO.MulticastVMEReaderTask");
    return log_;
}
...
MulticastVMEReaderTask::MulticastVMEReaderTask()
    : Super(), reader_(), bufferSize_(0), timer_(-1)
{
    msg_queue()->deactivate();
}

bool
MulticastVMEReaderTask::openAndInit(const std::string& host, uint16_t port,
                                    int bufferSize, long threadFlags,
                                    long threadPriority)
{
    Logger::ProcLog log("openAndInit", Log());
    LOGINFO << "host: " << host << " port: " << port << " SO_RCVBUV size: "
	    << bufferSize << " threadFlags: " << std::hex << threadFlags
	    << std::dec << " threadPriority: " << threadPriority << std::endl;
...
bool
MulticastVMEReaderTask::deliverDataMessage(ACE_Message_Block* data,
...
MulticastVMEReaderTask::svc()
{
	...
	if (reader_.isMessageAvailable()) {
		...
		Messages::RawVideo::Make("MulticastVMEReader", data));
...
MulticastVMEReaderTask::attemptConnection()
{
	...
	LOGERROR << getTaskName() << " failed to join multicast stream"
		 << std::endl;
	...
	int rc = device.set_option(SOL_SOCKET, SO_RCVBUF, &bufferSize_,
                                   sizeof(bufferSize_));
	if (rc == -1)
	    LOGERROR << getTaskName()
		     << "failed SO_RCVBUF setting using size of "
		     << bufferSize_ << " - " << Utils::showErrno()
		     << std::endl;
    }

    msg_queue()->activate();

    LOGDEBUG << getTaskName() << "threadFlags: " << std::hex << threadFlags_
	     << std::dec << std::endl;
...
int
MulticastVMEReaderTask::handle_timeout(const ACE_Time_Value& duration,
                                       const void* arg)
{
