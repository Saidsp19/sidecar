...
Task::Task(bool usingData)
    : Super(), stream_(), taskName_(""), error_(""), taskIndex_(0), inputs_(), inputStats_(), outputs_(),
      ...
      editingEnabled_(Parameter::BoolValue::Make("editingEnabled", "Editing Enabled", true)),
      connectionInfo_(""), processingState_(ProcessingState::kInvalid),
      lastProcessingState_(ProcessingState::kInvalid),
      alwaysUsingData_(Parameter::BoolValue::Make("alwaysUsingData", "Always Using Data", false)),
      threadParams_(), usingData_(usingData)
{
...
Task::setError(const std::string& error, bool force)
{
	...
	error_ = error;
	if (! stream_.expired()) {
	    LOGERROR << "task: " << taskName_ << " error: " << error << std::endl;
        }
    }
...
Task::registerParameter(const Parameter::Ref& parameter)
{
    ...
    if (parameterMap_.find(parameter->getName()) != parameterMap_.end()) {
        LOGERROR << "duplicate registration of a parameter with name '"
...
Task::unregisterParameter(const Parameter::Ref& parameter)
{
    ...
    if (pos == parameterMap_.end()) {
        LOGERROR << "no registration of a parameter with name '"
                 << parameter->getName() << "'" << std::endl;
        return false;
    }
...
*/
static ProcessingState::Value transitions_[ProcessingState::kNumStates]
[ProcessingState::kNumStates] = {
...
Task::enterProcessingState(ProcessingState::Value goalState)
{
    ...
    LOGINFO << "task: " << taskName_ << " current: " << ProcessingState::GetName(processingState_)
	    << '/' << processingState_ << " goal: " << ProcessingState::GetName(goalState) << '/' << goalState
            << std::endl;

    // Remember the new processing state for use with enterLastProcessingState()
    //
    if (ProcessingState::IsNormal(goalState)) {
	lastProcessingState_ = goalState;
    }
	...
	LOGDEBUG << "current: " << ProcessingState::GetName(processingState_) << " next: "
                 << ProcessingState::GetName(nextState) << std::endl;
...
Task::enterFailureState()
{
    ...
    //
    if (processingState_ != ProcessingState::kStop)
	enterStopState();
...
Task::enterLastProcessingState()
{
    ...
    if (lastProcessingState_ == ProcessingState::kInvalid)
	return true;
...
Task::processControlMessage(ACE_Message_Block* data)
{
    ...
    case ControlMessage::kShutdown:
	ok = doShutdownRequest();
	break;

    case ControlMessage::kClearStats:
	ok = doClearStatsRequest();
	break;

    case ControlMessage::kTimeout:
        ok = doTimeout();
        break;
...
Task::sendManaged(MessageManager& manager, size_t channelIndex)
{
	...
	ok = outputs_.getChannel(channelIndex).deliver(
	    manager.getMessage());
    }
...
Task::fillStatus(StatusBase& status)
{
	...
	LOGDEBUG << "input queue size: " << pendingQueueCount << std::endl;
    }
...
{
    for (size_t index = 0; index < inputStats_.size(); ++index) {
	inputStats_[index].resetAll();
    }
}
