...
public:
    ...
    */
    Utils::Pool::AllocationStats getAllocationStats() const { return pool_.getAllocationStats(); }
    ...
    */
    ...
    void* calloc(size_t count, size_t elemSize, char init = '\0') { return calloc(count * elemSize, init); }
...
PoolAllocator::Log()
{
    static Logger::Log& log_ = Logger::Log::Find("SideCar.IO.MessageManager.PoolAllocator");
    return log_;
}

PoolAllocator::PoolAllocator(size_t objectSize, const char* name) :
    ACE_New_Allocator(), pool_(objectSize, 1024), mutex_(), objectSize_(objectSize), name_(name)
{
...
class MessageBlockAllocatorImpl : public PoolAllocator {
    /** Constructor.
     */
    MessageBlockAllocatorImpl() : PoolAllocator(sizeof(ACE_Message_Block), "MessageBlock") {}
    friend class ACE_Singleton<MessageBlockAllocatorImpl, ACE_Recursive_Thread_Mutex>;
};
...
{
    return new (MessageBlockAllocator::instance()->malloc(sizeof(ACE_Message_Block))) ACE_Message_Block(
        size, type,
        0, // continuation
        0, // raw data pointer
        0, // allocator strategy
        MessageBlockLockingStrategy::instance(), ACE_DEFAULT_MESSAGE_BLOCK_PRIORITY, ACE_Time_Value::zero,
        ACE_Time_Value::max_time, DataBlockAllocator::instance(), MessageBlockAllocator::instance());
}
...
MessageManager::MessageManager(const Messages::Header::Ref& native) : data_(0), metaData_(0)
{
    ...
    data_(0), metaData_(0)
{
    ...
    case kRawData:
        ...
        if (!metaTypeInfo) {
            ...
            if (decoder.isValid()) { metaTypeInfo = Messages::Header::GetMessageMetaTypeInfo(decoder); }
        }
...
MessageManager::makeMetaData(ACE_Message_Block* raw)
{
    data_ = new (MessageBlockAllocator::instance()->malloc(sizeof(ACE_Message_Block)))
        ACE_Message_Block(sizeof(MetaData), kMetaData,
                          raw, // continuation
                          0,   // data data pointer
                          MetaDataAllocator::instance(), MessageBlockLockingStrategy::instance(),
                          ACE_DEFAULT_MESSAGE_BLOCK_PRIORITY, ACE_Time_Value::zero, ACE_Time_Value::max_time,
...
MessageManager::getEncoded() const
{
    ...
    if (!data_->cont()) {
        ...
        //
        ACE_OutputCDR messageEncoder(size_t(0), ACE_CDR_BYTE_ORDER, (ACE_Allocator*)(0), DataBlockAllocator::instance(),
