...
{
    static Logger::Log& log_ = Logger::Log::Find("SideCar.IO.TCPDataSubscriber");
    return log_;
}
...
TCPDataSubscriber::TCPDataSubscriber() : Super(), connector_(0)
{
...
bool
TCPDataSubscriber::openAndInit(const std::string& key, const std::string& serviceName, int bufferSize, int interface)
{
    static Logger::ProcLog log("openAndInit", Log());
    LOGINFO << "key: " << key << " serviceName: " << serviceName << " interface: " << interface << std::endl;
    ...
    return Super::openAndInit(key, serviceName, MakeTwinZeroconfType(key), interface);
}
...
void
TCPDataSubscriber::resolvedService(const Zeroconf::ServiceEntry::Ref& serviceEntry)
{
    static Logger::ProcLog log("resolvedService", Log());
    const Zeroconf::ResolvedEntry& resolved = serviceEntry->getResolvedEntry();
    LOGINFO << getTaskName() << " host: " << resolved.getHost() << '/' << resolved.getPort() << std::endl;
    ...
    remoteAddress.set(resolved.getPort(), resolved.getHost().c_str(), 1, AF_INET);

    if (!connector_->openAndInit(remoteAddress, reactor())) {
        LOGERROR << getTaskName() << "failed to open TCPConnector with address " << resolved.getHost() << '/'
                 << resolved.getPort() << std::endl;
        setError("Failed to initialize connector");
    }

    std::ostringstream os;
    os << "Host: " << host << " Port: " << resolved.getPort() << " Interface: " << serviceEntry->getInterfaceName();
    if (connector_->getMaxSocketBufferSize()) os << " Buffer Size: " << connector_->getMaxSocketBufferSize();
