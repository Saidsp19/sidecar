...
Processor::Processor(int argc, char** argv)
    : cla_(argc, argv, about, opts, sizeof(opts), args, sizeof(args)),
      inputTask_(), reader_(), ofs_(), os_(&std::cout), count_(0),
...
Processor::process()
{
    ...
    *os_ << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
	...
	if (producer != lastProducer) {
		...
		std::clog << "*** warning: diffent producers found in file: "
			  << producer << " and " << lastProducer << "\n";
	    split = true;
	}
	else if (lastType != type) {
	    if (lastType.size())
		std::clog << "*** warning: diffent message types found in "
		    "file: " << type << " and " << lastType << "\n";
	    split = true;
	}
...
Processor::readNext(Time::TimeStamp& timeStamp)
{
    ...
    //
    msg = mgr.getNative();
    timeStamp = useEmitted_ ? msg->getEmittedTimeStamp() :
	msg->getCreatedTimeStamp();
