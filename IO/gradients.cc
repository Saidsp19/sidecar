...
main(int argc, char** argv)
{
	...
	if (! (value >> minPower) || minPower < -32768)
	    cla.usage("invalid 'minpower' value");

    if (cla.hasOpt("maxpower", value))
	if (! (value >> maxPower) || maxPower > 32767)
	    cla.usage("invalid 'maxpower' value");

    if (cla.hasOpt("powerstep", value))
	if (! (value >> powerStep) || powerStep < 1.0)
	    cla.usage("invalid 'powerstep' value");
	...
	for (size_t radial = 0; radial < numRadials; ++radial) {
	    ...
	    Messages::Video::Ref msg(Messages::Video::Make("gradients", vme,
                                                           numGates));
	    msg->setCreatedTimeStamp(clock);
	    clock += timeRate;
	    msg->resize(numGates, int(::rint(powerLevel)));
	    if (powerLevel + powerStep > maxPower ||
                powerLevel + powerStep < minPower)
		powerStep *= -1;
