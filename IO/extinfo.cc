    ...
    while (input.getq(data, 0) != -1) {
        ...
        if (printData) {
            if (printCSV) {
                ...
                Messages::Extractions::Ref extraction = mgr.getNative<Messages::Extractions>();
                Messages::Extractions::const_iterator pos = extraction->begin();
                Messages::Extractions::const_iterator end = extraction->end();
                ...
                while (pos != end) {
                    ...
                    tm* bits = ::localtime(&t);
                    std::cout << date(bits->tm_year + 1900) << date(bits->tm_mon + 1) << date(bits->tm_mday) << ", "
                              << date(bits->tm_hour) << ':' << date(bits->tm_min) << ':' << date(bits->tm_sec) << '.'
                              << date(int(pos->getWhen().getMicro() * Time::TimeStamp::kSecondsPerMicro * 100)) << ", "
                              << coord(pos->getRange()) << ", " << coord(pos->getAzimuth()) << ", "
                              << coord(pos->getElevation()) << ", " << coord(pos->getX()) << ", " << coord(pos->getY())
                              << '\n';
                    ++pos;
                }
            } else {
