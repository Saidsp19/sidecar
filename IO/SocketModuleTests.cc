...
MetaTypeInfo Message::metaTypeInfo_(MetaTypeInfo::kUnassigned, "Message",
                                    &Message::Loader);
...
struct TestTask : public Task
{
    ...
    bool updateValveState(bool state)
	{ return Task::updateValveState(true); }
...
TestTask::deliverDataMessage(ACE_Message_Block* data, ACE_Time_Value* timeout)
{
    ...
    LOGDEBUG << "*** counter: " << counter_ << " of " << kNumMessages
	     << " ***" << std::endl;
...
Test::test1()
{
    ...
    TModule<ClientSocketWriterTask>* cswtm =
	new TModule<ClientSocketWriterTask>(&stream);
    stream.push(cswtm);
    ...
    //
    TModule<ServerSocketReaderTask>* ssrtm =
	new TModule<ServerSocketReaderTask>(&stream);
    stream.push(ssrtm);
...
Test::test2()
{
    ...
    //
    Stream stream("Test", 0);
    TModule<ServerSocketWriterTask>* sswtm =
	new TModule<ServerSocketWriterTask>(&stream);
    stream.push(sswtm);
    ...
    //
    TModule<ClientSocketReaderTask>* csrtm =
	new TModule<ClientSocketReaderTask>(&stream);
    stream.push(csrtm);
    assertEqual(true, csrtm->getTask()->openAndInit("Message", "localhost",
                                                    port));
