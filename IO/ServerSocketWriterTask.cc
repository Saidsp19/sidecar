...
ServerSocketWriterTask::OutputHandler::Log()
{
    static Logger::Log& log_ =
        Logger::Log::Find("SideCar.IO.ServerSocketWriterTask.OutputHandler");
    return log_;
}
...
ServerSocketWriterTask::OutputHandler::open(void* arg)
{
    ...
    if (bufferSize) {
	if (peer().set_option(SOL_SOCKET, SO_SNDBUF, &bufferSize,
                              sizeof(bufferSize)) == -1)
	    LOGERROR << "failed to set SO_SNDBUF to " << bufferSize
		     << std::endl;
    }
...
ServerSocketWriterTask::OutputHandler::close(u_long flags)
{
	...
	//
        reactor()->remove_handler(get_handle(), ACE_Event_Handler::READ_MASK |
...
{
    static Logger::Log& log_ =
        Logger::Log::Find("SideCar.IO.ServerSocketWriterTask.Acceptor");
    return log_;
}
...
{
    static Logger::Log& log_ =
        Logger::Log::Find("SideCar.IO.ServerSocketWriterTask");
    return log_;
}
...
bool
ServerSocketWriterTask::openAndInit(const std::string& key, uint16_t port,
                                    int bufferSize, long threadFlags,
                                    long threadPriority)
{
    Logger::ProcLog log("openAndInit", Log());
    LOGINFO << "key: " << key << " port: " << port << " threadFlags: "
    ...
    if (bufferSize) {
	if (acceptor_->acceptor().set_option(SOL_SOCKET, SO_SNDBUF,
                                             &bufferSize,
                                             sizeof(bufferSize)) == -1)
	    LOGERROR << "failed to set SO_SNDBUF to " << bufferSize
		     << std::endl;
...
ServerSocketWriterTask::close(u_long flags)
{
	...
	msg_queue()->deactivate();
        if (wait() == -1)
            LOGERROR << "failed to join main processing thread" << std::endl;
...
ServerSocketWriterTask::addOutputHandler(OutputHandler* handler)
{
    ...
    ACE_Message_Block* req = new ACE_Message_Block(sizeof(handler),
                                                   ACE_Message_Block::MB_START,
                                                   0, (char*)handler);
...
ServerSocketWriterTask::removeOutputHandler(OutputHandler* handler)
{
    ...
    LOGINFO << handler << std::endl;
    ACE_Message_Block* req = new ACE_Message_Block(
        sizeof(handler), ACE_Message_Block::MB_HANGUP, 0, (char*)handler);
    req->wr_ptr(sizeof(handler));
    if (putq(req) == -1)
	req->release();
}
...
bool
ServerSocketWriterTask::deliverDataMessage(ACE_Message_Block* data,
...
ServerSocketWriterTask::updateClients(ACE_Message_Block* data)
{
	...
	reinterpret_cast<OutputHandler*>(data->rd_ptr());
	...
	OutputHandlerVector::iterator pos = std::find(clients_.begin(),
                                                      clients_.end(),
                                                      outputHandler);
	if (pos == clients_.end()) {
	...
	LOGERROR << "invalid message type - " << data->msg_type()
		 << std::endl;
    }
...
ServerSocketWriterTask::distribute(ACE_Message_Block* data)
{
    ...
    for (; pos != end; ++pos) {
        ...
        if ((*pos)->put(dup) == -1) {
            LOGERROR << "failed to write to client output queue - flushing"
                     << std::endl;
	    dup->release();
	    (*pos)->close(1);
        }
