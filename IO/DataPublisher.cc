...
DataPublisher::publish(const std::string& serviceName)
{
	...
	if (timer_ == -1) {
	    LOGERROR << getTaskName() << " failed to schedule timer for " << "reconnecting" << std::endl;
...
DataPublisher::publishSucceeded()
{
    ...
    LOGINFO << std::endl;
    if (connectionPublisher_->getName() != serviceName_) {
	setServiceName(connectionPublisher_->getName());
    }
    if (timer_ != -1) {
	reactor()->cancel_timer(timer_);
	timer_ = -1;
...
DataPublisher::handle_timeout(const ACE_Time_Value& duration, const void* arg)
{
	...
	LOGERROR << "failed to publish connection with name '" << serviceName_ << " type: "
                 << connectionPublisher_->getType() << std::endl;
