...
struct CounterTask : public Task {
    ...
    void setUsingData(bool isOpen) { Task::setUsingData(true); }

    int count_;
};

bool
CounterTask::deliverDataMessage(ACE_Message_Block* data, ACE_Time_Value* timeout)
{
    Logger::ProcLog log("deliverDataMessage", Log());

    MessageManager manager(data);
    LOGDEBUG << "*** " << count_ << " " << manager.getMessageType() << std::endl;
...
main(int argc, const char* argv[])
{
    ...
    for (int repetition = 0; repetition < 4; ++repetition) {
        ...
        int rc = Test().mainRun();
        if (rc) {
            LOGERROR << "failed in repetition " << (repetition + 1) << std::endl;
            return rc;
        }
    }
