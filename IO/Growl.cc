...
struct Growler::Destination
{
	...
	: name_(name), address_(), emitRegistration_(true)
...
struct PayloadBuffer
{
    ...
    PayloadBuffer& operator<<(bool value)
	{ return operator<<(static_cast<unsigned char>(value ? 1 : 0)); }

    PayloadBuffer& operator<<(char value)
	{ return operator<<(static_cast<unsigned char>(value)); }

    PayloadBuffer& operator<<(uint16_t value);

    PayloadBuffer& operator<<(int16_t value)
	{ return operator<<(static_cast<uint16_t>(value)); }

    PayloadBuffer& operator<<(uint32_t value)
	{ return operator<<(static_cast<uint16_t>(value & 0xFFFF)); }

    PayloadBuffer& operator<<(int32_t value)
	{ return operator<<(static_cast<uint16_t>(value & 0xFFFF)); }

    PayloadBuffer& operator<<(size_t value)
	{ return operator<<(static_cast<uint16_t>(value & 0xFFFF)); }
...
PayloadBuffer::PayloadBuffer(int type, const std::string& password)
    : buffer_(), password_(password)
{
...
PayloadBuffer::operator<<(const std::string& value)
{
	...
	buffer_.push_back(static_cast<uint8_t>(value[index]));
    }
    return *this;
}
...
PayloadBuffer::getPayload()
{
    ...
    for (int index = 0; index < Utils::MD5::kDigestSize; ++index)
	buffer_.push_back(digest[index]);

    return buffer_;
}

Growler::Growler(const std::string& name, const std::string& password)
    : name_(name), password_(password), notifications_(), defaults_(),
      destinations_(), socket_(), usingBroadcasting_(false)
{
...
Growler::addHost(const std::string& host)
{
	...
	//
	for (DestinationList::const_iterator pos = destinations_.begin();
             pos != destinations_.end(); ++pos) {
	    if ((*pos)->name_ == host)
		return;
	}
...
{
    if (value == usingBroadcasting_ && socket_)
	return;
	...
	//
	socket_.reset(
	    new ACE_SOCK_Dgram_Bcast(ACE_INET_Addr(uint16_t(0))));
	boost::shared_ptr<Destination> ref(
	    new Destination("255.255.255.255"));
	destinations_.push_back(ref);
    }
    else {
	...
	//
	socket_.reset(
	    new ACE_SOCK_Dgram(ACE_INET_Addr(uint16_t(0))));
    }
}
...
{
    if (enabled)
	defaults_.push_back(notifications_.size());
...
Growler::emitRegistration(boost::shared_ptr<Destination> destination)
{
    ...
    buffer << name_.size()
	   << uint8_t(notifications_.size())
	   << uint8_t(defaults_.size())
	   << name_;

    // Add the names of the notifications.
    //
    for (size_t index = 0; index < notifications_.size(); ++index)
	buffer << notifications_[index].size()
	       << notifications_[index];

    // Add the indices of those notifications that are enabled
    //
    for (size_t index = 0; index < defaults_.size(); ++index)
	buffer << defaults_[index];

    // Emit the payload to a specific destination.
    //
    PayloadBuffer::PayloadType payload(buffer.getPayload());
    bool rc = socket_->send(&payload[0], payload.size(),
                            destination->address_, 0) ==
	ssize_t(payload.size());
    if (rc)
	destination->emitRegistration_ = false;

    return rc;
}

bool
Growler::notify(const std::string& notification, const std::string& title,
                const std::string& description, int priority, bool sticky)
{
    // The notification name must exist in our registry.
    //
    if (std::find(notifications_.begin(), notifications_.end(),
                  notification) == notifications_.end())
	return false;
    ...
    for (DestinationList::iterator pos = destinations_.begin();
         pos != destinations_.end();) {

	bool ok = true;

	if ((*pos)->emitRegistration_)
	    ok = emitRegistration(*pos);

	if (ok)
	    socket_->send(&payload[0], payload.size(), (*pos)->address_, 0);
