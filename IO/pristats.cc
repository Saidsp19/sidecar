...
const std::string about =
    "Print the average magnitude squared, power, and median power for certain samples and messages of a PRI file.";
...
const Utils::CmdLineArgs::ArgumentDef args[] = {{"INFILE", "path to input file"}};
...
main(int argc, char** argv)
{
    ...
    std::string start_message_str(""), end_message_str(""), start_sample_str(""), end_sample_str("");
    ...
    if (!counter->openAndInit("Video", infilePath.c_str())) {
        std::cerr << "*** failed to open/start counter on file '" << cla.arg(0) << "'" << std::endl;
        return 1;
    }
    ...
    if (cla.hasOpt("start_message", start_message_str)) {
        ...
        //
        if (start_msg_ < 0) { start_msg_ = numMessages + start_msg_; }
    }
    if (cla.hasOpt("end_message", end_message_str)) {
        end_msg_ = atoi(end_message_str.c_str());
        // Handle negative indices
        //
        if (end_msg_ < 0) { end_msg_ = numMessages + end_msg_; }
    }
    ...
    //
    if (cla.hasOpt("start_sample", start_sample_str)) { start_sample_ = atoi(start_sample_str.c_str()); }

    if (cla.hasOpt("end_sample", end_sample_str)) { end_sample_ = atoi(end_sample_str.c_str()); }
    ...
    std::cerr << "Grabbing messages " << start_msg_ << " through " << end_msg_ << " and samples " << start_sample_
              << " through " << end_sample_ << "\n";
    ...
    while (input.getq(data, 0) != -1) {
        ...
        if (int(messageCounter_) >= start_msg_ && int(messageCounter_) <= end_msg_) {
            ...
            if (start_index_ < 0 || start_index_ >= int(video->size()) || end_index_ < 0 ||
                end_index_ >= int(video->size()) || end_index_ < start_index_) {
                std::cout << "Error: Illegal sample indices: [" << start_index_ << ", " << end_index_
            ...
            for (size_t index_ = 0; index_ < length_; index_++) { out.push_back(video[start_index_ + index_]); }
            ...
            std::cout << mag_sqrd_sum_ / sample_cnt_ << ", " << 10.0 * ::log10(20.0 * mag_sqrd_sum_ / sample_cnt_)
