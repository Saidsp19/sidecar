...
const Utils::CmdLineArgs::OptionDef options[] = {
    { 'a', "arc", "number of radials on/off in ring", "ARC" },
...
main(int argc, char** argv)
{
	...
	if (! (value >> numRadials) || numRadials < 360)
	    cla.usage("invalid 'radials' value");
	arcWidth = numRadials;
    }

    if (cla.hasOpt("count", value))
	if (! (value >> numRings) || numRings < 1 ||
            numRings > numGates)
	    cla.usage("invalid 'count' value");

    if (cla.hasOpt("width", value))
	if (! (value >> ringWidth) || ringWidth < 1 || ringWidth > 100)
	    cla.usage("invalid 'width' value");

    if (cla.hasOpt("arc", value))
	if (! (value >> arcWidth) || arcWidth < 1 || arcWidth > numRadials)
	    cla.usage("invalid 'arc' value");
	...
	if (! (value >> noise))
	    cla.usage("invalid 'noise' value");
	power -= noise;
    }

    double timeRate = 1.0 / frequency;
    double beamWidth = (M_PI * 2.0) / numRadials;
    double shaftChange = double(Messages::RadarConfig::GetShaftEncodingMax())
	/ numRadials;
    double ringSpacing = double(numGates) / double(numRings);

    if (cla.hasOpt("spacing", value))
	if (! (value >> ringSpacing))
	    cla.usage("invalid 'spacing' value");

    std::clog << "numRadials: " << numRadials << " beamWidth: "
	      << Utils::radiansToDegrees(beamWidth) << " degrees "
	      << beamWidth << " radians\n";
    std::clog << "shaft change: " << shaftChange << '\n';
    std::clog << "numGates: " << numGates << " minRange: " << minRange
	      << " maxRange: " << maxRange << '\n';
    std::clog << "numRings: " << numRings << " width: " << ringWidth << " arc: "
	      << arcWidth << " spacing: " << ringSpacing << '\n';
    std::clog << "time change: " << timeRate << " frequency: " << frequency
	      << std::endl;
	...
	for (size_t radial = 0; radial < numRadials; ++radial) {
	    ...
	    Messages::Video::Ref msg(Messages::Video::Make("rings", vme,
                                                           numGates));
