...
TSPIReaderTask::Log()
{
	...
	Logger::Log::Find("SideCar.IO.TSPIReaderTask");
    return log_;
}
...
TSPIReaderTask::TSPIReaderTask()
    : Super(), reader_(), timer_(-1)
{
    ;
}

bool
TSPIReaderTask::openAndInit(const std::string& host, uint16_t port,
                            ...
                            long threadPriority)
{
	...
	if (device.set_option(SOL_SOCKET, SO_RCVBUF, &bufferSize,
                              sizeof(bufferSize)) == -1) {
	    LOGERROR << "failed SO_RCVBUF setting using size of "
		     << bufferSize << " - " << Utils::showErrno()
		     << std::endl;
	}
    }
...
bool
TSPIReaderTask::deliverDataMessage(ACE_Message_Block* data,
...
TSPIReaderTask::svc()
{
	...
	if (reader_.isMessageAvailable()) {
		...
		Messages::TSPI::MakeRaw("TSPIReaderTask", data));
	    data->release();
...
int
TSPIReaderTask::handle_timeout(const ACE_Time_Value& duration,
                               const void* arg)
{
