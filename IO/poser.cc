...
const Utils::CmdLineArgs::OptionDef options[] = {{'L', "logger", "use CFG for Logger configuration file", "CFG"},
...
const Utils::CmdLineArgs::ArgumentDef args[] = {{0, 0}, {"KEY", "search key for -r and -t options"}};
...
Args::Args(int argc, char** argv) : inputFile(""), recordIndex(""), timeIndex(""), gap(60), searchKey(-1)
{
    Utils::CmdLineArgs cla(argc, argv, description, options, sizeof(options), args, sizeof(args));

    std::string value;
    if (cla.hasOpt("logger", value)) { Logger::ConfiguratorFile cf(value); }
    ...
    if (cla.hasArg(0, value)) { value >> searchKey; }
