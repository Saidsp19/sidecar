...
public:
    ...
    /** Constructor.
        ...
        \param owner 
    */
    ReaderThread(MulticastDataSubscriber* owner)
	: Super(), owner_(owner), reader_(), active_(false) {}
    ...
    */
    int svc()
	{
		...
		if (reader_.isMessageAvailable()) {
		    owner_->acquireExternalMessage(reader_.getMessage());
		}
	    }
                                     ...
                                     int interface, long threadFlags, long threadPriority)
{
    ...
    LOGINFO << "key: " << key << " serviceName: " << serviceName
	    << " bufferSize: " << bufferSize << " interface: "
	    << interface << " threadFlags: " << std::hex << threadFlags
...
void
MulticastDataSubscriber::resolvedService(
    const Zeroconf::ServiceEntry::Ref& service)
{
...
MulticastDataSubscriber::attemptConnection()
{
    ...
    if (! reader_->openAndInit(address_, bufferSize_, threadFlags_,
                               threadPriority_)) {
	LOGERROR << getTaskName() << " failed to start reader thread"
		 << std::endl;
...
MulticastDataSubscriber::sendHeartBeat(const char* msg) const
{
	...
	LOGERROR << "address: " << heartBeatAddress_.get_host_addr() << " port: "
                 << heartBeatAddress_.get_port_number() << std::endl;
    }
}
