...
const Utils::CmdLineArgs::ArgumentDef args[] = {
    {"FILE", "path to output file"}, {0, 0}, {"SPEC", "waveform specification: CYCLE/AMP/DC/OFFSET"}};

class Generator {
public:
    Generator() {}

    Generator(int gatesPerCycle, double amplitude, double dc, double offset) :
        offset_(int(::rint(offset* gatesPerCycle))), gatesPerCycle_(gatesPerCycle), amplitude_(amplitude), dc_(dc),
        gateChange_(2.0 * M_PI / gatesPerCycle), values_(), gate_(0)
    {
    ...
    double getValue()
    {
        if (values_.empty()) {
            ...
            for (int index = 0; index < gatesPerCycle_; ++index) {
                if (counter == gatesPerCycle_) counter = 0;
                values_.push_back(::sin(counter++ * gateChange_) * scaledAmplitude_ + scaledDC_);
            }
        }

        if (gate_ == gatesPerCycle_) gate_ = 0;
...
struct SetAmplitudeSum {
    ...
    void operator()(Generator& generator) { generator.setAmplitudeSum(sum_); }
};
...
public:
    ...
    double getValue() { return std::accumulate(container_.begin(), container_.end(), 0.0); }

private:
    void updateAmplitudeSum(double amplitude)
    {
        sum_ += amplitude;
        std::for_each(container_.begin(), container_.end(), SetAmplitudeSum(sum_));
    }
...
main(int argc, char** argv)
{
    ...
    if (cla.hasOpt("minpower", value))
        if (!(value >> minPower) || minPower < 0.0) cla.usage("invalid 'minpower' value");

    if (cla.hasOpt("maxpower", value))
        if (!(value >> maxPower) || maxPower < 1.0) cla.usage("invalid 'maxpower' value");

    if (minPower >= maxPower) cla.usage("min power must be less than max power");

    if (cla.hasOpt("radials", value))
        if (!(value >> numRadials) || numRadials < 1) cla.usage("invalid 'radials' value");
    ...
    std::clog << "numRadials: " << numRadials << " beamWidth: " << beamWidth << " ("
              << Utils::radiansToDegrees(beamWidth) << " deg.)\n";
    std::clog << "numGates: " << numGates << " minRange: " << minRange << " maxRange: " << maxRange << '\n';
    std::clog << "minPower: " << minPower << " maxPower: " << maxPower << '\n';
    std::clog << "priRate: " << priRate << " frequency: " << frequency << std::endl;
    ...
    while (cla.hasArg(index++, value)) {
        ...
        char slash;
        if (!(is >> cycle)) cla.usage("invalid CYCLE generator component");
        if (!(is >> slash) || slash != '/') cla.usage("invalid generator specification");
        if (!(is >> amplitude) || amplitude <= 0.0 || amplitude > 1.0) cla.usage("invalid AMP generator component");
        if (!(is >> slash) || slash != '/') cla.usage("invalid generator specification");
        if (!(is >> dc) || fabs(dc) + amplitude > 1.0) cla.usage("invalid DC generator component");
        if (!(is >> slash) || slash != '/') cla.usage("invalid generator specification");
        if (!(is >> offset) || offset < -1.0 || offset > 1.0) cla.usage("invalid OFFSET generator component");

        cycle = 1.0 / cycle;
        int gatesPerCycle = int(::rint(cycle / gateRate));

        std::clog << "generator spec - gatesPerCycle: " << gatesPerCycle << " amp: " << amplitude << " DC: " << dc
    ...
    vme.header.msgDesc = (Messages::VMEHeader::kIRIGValidMask | Messages::VMEHeader::kAzimuthValidMask |
    ...
    std::clog << "powerOffset: " << powerOffset << " powerConversion: " << powerConversion << std::endl;
    ...
    for (size_t scan = 0; scan < numScans; ++scan) {
        ...
        for (size_t radial = 0; radial < numRadials; ++radial) {
            ...
            Messages::Video::Ref msg(Messages::Video::Make("sines", vme, useComplex ? numGates * 2 : numGates));
            ...
            for (size_t gate = 0; gate < numGates; ++gate) {
                ...
                if (amplitude > maxAmp)
                    maxAmp = amplitude;
                else if (amplitude < minAmp)
                    minAmp = amplitude;
                msg->push_back(int(::rint(amplitude * powerConversion + powerOffset)));
                if (useComplex) msg->push_back(0);
            }
