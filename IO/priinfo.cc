...
main(int argc, char** argv)
{
    ...
    int gateDelta = 2;
    if (cla.hasOpt("gatedelta"))
	cla.opt("gatedelta")[0] >> gateDelta;
	...
	if (xml) {
		...
		std::cout << "<msgs producer=\""
			  << msg->getGloballyUniqueID().getProducerName()
			  << "\" type=\""
	...
	if (! messageCounter++) {
	    ...
	    if (msg->getMetaTypeInfo().getKey() ==
                Messages::MetaTypeInfo::Value::kVideo) {
		Messages::Video::Ref video = mgr.getNative<Messages::Video>();
		std::pair<iterator,iterator> mm(
		    boost::minmax_element(video->begin(), video->end()));
		powerMin = *mm.first;
		powerMax = *mm.second;
		...
		if (gateCount < gateCountMin) {
		    std::clog << "** MIN gate count changed - Msg: "
			      << messageCounter << " Az: "
			      << az(video->getRIUInfo().shaftEncoding)
			      << " old: " << gateCountMin
			      << " new: " << video->size() << std::endl;
		    gateCountMin = video->size();
		}
		else if (gateCount > gateCountMax) {
		    std::clog << "** MAX gate count changed - Msg "
			      << messageCounter << " Az: "
			      << az(video->getRIUInfo().shaftEncoding)
			      << " old: " << gateCountMax
			      << " new: " << video->size() << std::endl;
		...
		if (gateCount > lastGateCount + gateDelta) {
		    std::clog << "** Gate count changed - Msg "
			      << messageCounter << " Az: "
			      << az(video->getRIUInfo().shaftEncoding)
			      << " old: " << lastGateCount
		...
		if (sequenceNumber == lastSequenceNumber) {
			...
			std::clog << "** Dup msg - Msg: "
				  << (messageCounter - duplicateCounter - 1)
				  << " sequenceNumber: " << lastSequenceNumber
				  << " occurances: " << duplicateCounter
				  << std::endl;
			duplicateCounter = 0;
		    }
		    ...
		    std::clog << "** Gap of " << delta << " - Msg: "
			      << messageCounter << " current: "
			      << sequenceNumber << " last: "
			      << lastSequenceNumber << std::endl;
		}
	    }
	...
	if (deltaAverage)
	    frequency = ::rint(1.0 / deltaAverage);
    }

    double radialsPerScan = ::rint((M_PI * 2.0) /
                                   Messages::RadarConfig::GetBeamWidth());
    ...
    std::cout << "    # Messages: " << messageCounter
	      ...
	      << "\n   Dupe Seq#: " << duplicateMessages << " ("
	      << (duplicateMessages * 100.0 / messageCounter) << "%)"
	      << "\n     # Scans: " << (messageCounter / radialsPerScan)
	      << "\n   Frequency: " << frequency << " Hz"
	      << "\n  Beam Width: "
	      << Utils::radiansToDegrees(
		  Messages::RadarConfig::GetBeamWidth())
	      << " degrees (" << Messages::RadarConfig::GetBeamWidth()
	      << " radians)"
