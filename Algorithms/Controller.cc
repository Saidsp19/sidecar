...
struct Controller::IncomingNotifier : public ACE_Event_Handler, public ACE_Notification_Strategy
{
    static Logger::Log& Log() {
        static Logger::Log& log_ = Logger::Log::Find("SideCar.Algorithms.Controller." "IncomingNotifier");
        return log_;
    }
	...
	: ACE_Event_Handler(controller->reactor()), ACE_Notification_Strategy(this, kEvent),
...
Controller::Controller()
    : Super(), self_(), algorithmName_(""), algorithm_(), recorders_(),
      ...
      recordingEnabled_(Parameter::BoolValue::Make("recordingEnabled", "Recording Enabled", false)),
      processingStat_(), xmlConfiguration_(), recording_(false), statsManaged_(true), threaded_(true),
      timerThread_()
{
                        ...
                        long threadFlags, long threadPriority, bool threaded)
{
    ...
    LOGINFO << algorithmName << ' ' << serviceName << ' ' << algorithm
...
Controller::loadAlgorithm()
{
    ...
    //
    union { void* v; AlgorithmMaker p; } symbol;
...
{
    if (algorithm_) {
	algorithm_->getLog().setPriorityLimit(logLevel_->getValue());
    }
}
...
Controller::doRecordingStateChange(const IO::RecordingStateChangeRequest& msg)
{
	...
	if (recorders_.empty()) {
	    while (recorders_.empty() ||
                   recorders_.size() < getNumOutputChannels()) {
...
Controller::startRecordings(const std::string& fileNamePrefix)
{
	...
	LOGINFO << getTaskName() << " recording into file " << path
		<< " on channel " << index << std::endl;
