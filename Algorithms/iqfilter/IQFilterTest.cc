...
Test::test()
{
    ...
    Utils::TemporaryFilePath testOutputPath("iqfilterTestOutput");
    {
        ...
        IQFilter* alg = dynamic_cast<IQFilter*>(controller->getTask()->getAlgorithm());
        assertTrue(alg);
        ...
        for (int index = 0; index < IQFilter::kNumFilterTypes; ++index) {
            ...
            controller->getTask()->injectControlMessage(ParametersChangeRequest(parameterChange, false));
            VMEDataMessage vme;
