...
IQFilter::IQFilter(Controller& controller, Logger::Log& log)
    : Super(controller, log),
      filterType_(FilterTypeParameter::Make("filter", "Filter", FilterType(kDefaultFilter))),
...
IQFilter::process(const Messages::Video::Ref& in)
{
	...
	while (pos != end) {
	    ...
	    float i = *pos++;
	    outData.push_back(
		::rintf(10.0 * ::log10f(::sqrtf(r * r + i * i))));
	}
	break;
    ...
    default:
	LOGERROR << "invalid filter type - " << filterType_->getValue()
		 << std::endl;
	break;
    }
