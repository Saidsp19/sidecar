...
MatchedFilter::MatchedFilter(Controller& controller, Logger::Log& logger)
    : Super(controller, logger, kDefaultEnabled, kDefaultMaxBufferSize),
      numWorkers_(Parameter::PositiveIntValue::Make(
                      "numWorkers",
                      "Number of worker threads for processing",
                      kDefaultNumWorkers)),
      numFFTThreads_(Parameter::PositiveIntValue::Make(
                         "numFFTThreads",
                         "Number of FFT threads for processing",
                         kDefaultNumFFTThreads)),
      txPulseStartBin_(Parameter::IntValue::Make(
                           "txPulseStartBin",
                           "First complex bin of reference pulse in Tx<br>"
                           "(-1 for last, etc.)",
                           kDefaultTxPulseStartBin)),
      txPulseSpan_(Parameter::IntValue::Make(
                       "txPulseSpan",
                       "Number of complex bins for reference pulse in Tx<br>"
                       "(&lt; 1 = msgSize + value)",
                       kDefaultTxPulseSpan)),
      rxFilterStartBin_(Parameter::IntValue::Make(
                            "rxFilterStartBin",
                            "First complex bin to filter in Rx<br>"
                            "(-1 for last, etc.)",
                            kDefaultRxFilterStartBin)),
      rxFilterSpan_(Parameter::IntValue::Make(
                        "rxFilterSpan",
                        "Number of complex bins to filter in Rx<br>"
                        "(&lt; 1 = msgSize + value)",
                        kDefaultRxFilterSpan)),
      fftSize_(Parameter::PositiveIntValue::Make(
                   "fftSize",
                   "Size of FFT for filtering in frequency domain<br>"
                   "(best if power of 2)",
                   kDefaultFftSize)),
      scaleWithSumMag_(Parameter::BoolValue::Make(
                           "scaleWithSumMag",
                           "Scale Tx pulse using sum of magnitudes instead of "
                           "just max value",
                           kDefaultScaleWithSumMag)),
      domain_(DomainParameter::Make("domain_", "Domain",
                                    Domain(kDefaultDomain))),
      txThreshold_(Parameter::IntValue::Make(
		       "txThreshold",
		       "If no Tx pulse values pass threshold,<br>"
		       "pass unfiltered Rx", kDefaultTxThreshold)),
      txThresholdStartBin_(Parameter::IntValue::Make(
			       "txThresholdStartBin",
			       "First complex bin for Tx pulse search", 
			       kDefaultTxThresholdStartBin)),
      txThresholdSpan_(Parameter::IntValue::Make(
			   "txThresholdSpan",
			   "Number of complex bins to search for Tx pulse", 
			   kDefaultTxThresholdSpan)),
      fwdFFT_(), txPulseVec_(),
      workerThreads_(0),
      idleWorkRequests_(new WorkRequestQueue("idle")),
      pendingWorkRequests_(new WorkRequestQueue("pending")),
      finishedWorkRequests_(new WorkRequestQueue("finished")),
      noPulseDetected_(false), restartWorkerThreads_(true)
{
    Logger::ProcLog log("MatchedFilter", logger);

    if (! fftw_init_threads())
	LOGERROR << "failed fftw_init_threads" << std::endl;

    numWorkers_->connectChangedSignalTo(
	boost::bind(&MatchedFilter::numWorkersChanged, this, _1));
    fftSize_->connectChangedSignalTo(
	boost::bind(&MatchedFilter::fftSizeChanged, this, _1));
    rxFilterSpan_->connectChangedSignalTo(
	boost::bind(&MatchedFilter::rxFilterSpanChanged, this, _1));
}
...
{
    return registerParameter(txPulseStartBin_) &&
	registerParameter(txPulseSpan_) && 
	registerParameter(rxFilterStartBin_) &&
	registerParameter(rxFilterSpan_) &&
	registerParameter(domain_) &&
	registerParameter(fftSize_) &&
	registerParameter(scaleWithSumMag_) &&
	registerParameter(numWorkers_) &&
	registerParameter(numFFTThreads_) &&
        registerParameter(txThreshold_) &&
        registerParameter(txThresholdStartBin_) &&
        registerParameter(txThresholdSpan_) &&
	Super::startup();
}
...
MatchedFilter::getWorkRequest()
{
    ...
    if (idleWorkRequests_->message_count() &&
        idleWorkRequests_->dequeue_head(data) != -1)
	return data;
    return WorkRequest::Make(txPulseVec_.get(), numFFTThreads_->getValue(),
                             fftSize_->getValue());
}
...
MatchedFilter::startThreads()
{
    ...
    //
    for (size_t index = 0; index < idleWorkRequests_->message_count();
         ++index) {
	...
	ACE_Message_Block* data;
	idleWorkRequests_->dequeue_head(data);
	WorkRequest::FromMessageBlock(data)->reconfigure(
	    txPulseVec_.get(), numFFTThreads_->getValue(),
	    fftSize_->getValue());
    ...
    //
    workerThreads_.reset(new WorkerThreads(*this, *pendingWorkRequests_,
                                           *finishedWorkRequests_));
    LOGDEBUG << "attempting to start " << numWorkers << " worker threads" 
	     << std::endl;

    int rc = workerThreads_->activate(THR_NEW_LWP | THR_JOINABLE |
                                      THR_INHERIT_SCHED, numWorkers);

    if (rc == -1) {
	LOGERROR << "failed to activate worker threads" << std::endl;
    }
}
...
ChannelBuffer*
MatchedFilter::makeChannelBuffer(int index, const std::string& name,
                                 size_t maxBufferSize)
{
    ...
    if (name == "transmit") {
	tx_ = new TChannelBuffer<Messages::Video>(*this, index,
                                                  maxBufferSize);
	return tx_;
    }
    else if (name == "receive") {
	rx_ = new TChannelBuffer<Messages::Video>(*this, index,
                                                  maxBufferSize);
	return rx_;
    }
    else {
...
MatchedFilter::processChannels()
{
    ...
    LOGDEBUG << "txThreshold: " << txThreshold
	     << " txThresholdStart: " << txThresholdStart
	     << " txThresholdSpan: " << txThresholdSpan
	     << " txSize: " << txSize << std::endl;
    if (! Utils::normalizeSampleRanges(txThresholdStart, txThresholdSpan,
                                       txSize)) {
    ...
    LOGDEBUG << "txPulseStart : "<< txPulseStart << " txPulseSpan : "
	     << txPulseSpan << " txSize: " << txSize << std::endl;
    ...
    LOGDEBUG << "rxFilterStart : "<< rxFilterStart << " rxFilterSpan : "
	     << rxFilterSpan << " rxSize: " << rxSize << std::endl;

    // Make sure rxFilterStart and rxFilterSpan have reasonable values given the number of samples in the
    // message.
    //
    if (! Utils::normalizeSampleRanges(rxFilterStart, rxFilterSpan,
                                       rxSize)) {
    ...
    //
    std::copy(rxMsg->begin(), rxMsg->begin() + rxFilterStart * 2,
	...
	std::copy(rxMsg->begin() + offset, rxMsg->end(),
                  outputData.begin() + offset);
	...
	int numWindows = static_cast<int>(
	    ::ceil(static_cast<float>(rxFilterSpan) / fftSize));
	...
	//
	for (int index = txPulseSpan; index < fftSize; ++index)
	    txPulseVec_->put(index, ComplexType(0.0, 0.0));
	...
	*txPulseVec_ = vsip::conj(*txPulseVec_);
	...
	for (int index = 0; index < rxFilterSpan - txPulseSpan; ++index) {
		...
		Messages::Video::DatumType(
		    ::rint(rxVec.get(index).real())));
	    outputData.push_back(
		Messages::Video::DatumType(
		    ::rint(rxVec.get(index).imag())));
	}

	// Copy any remaining unfiltered data to the output msg
	//
	for (size_t index = (rxFilterStart + rxFilterSpan - txPulseSpan) * 2;
             index < rxMsg->size(); ++index) {
	    outputData.push_back(rxMsg[index]);
	}
    } // end if processing in time domain

    if (outputData.size() > 30 * 1024)
	LOGERROR << "abnormally large size: " << outputData.size() << std::endl;
...
FormatInfo(const IO::StatusBase& status, int role)
{
    ...
    if (! status[ManyInAlgorithm::kEnabled] ||
        status[MatchedFilter::kNoPulseDetected])
	return "Rx Passthrough";
