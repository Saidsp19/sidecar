...
MatchedFilter::MatchedFilter(Controller& controller, Logger::Log& log)
    : Super(controller, log, kDefaultEnabled, kDefaultMaxBufferSize),
      ...
      fftSize_(Parameter::PositiveIntValue::Make(
                   "fftSize",
                   "Size of FFT for filtering in frequency domain",
                   kDefaultFftSize)),
      ...
      fwdFFT_(), invFFT_(), txPulseVec_(), rxVec_(), rxOp_()
{
...
MatchedFilter::startup()
{
    ...
    ok = ok 
	&& registerParameter(txPulseStartBin_)
...
MatchedFilter::processChannels()
{
    ...
    LOGERROR << "txPulseStart : "<< txPulseStart << " txPulseSpan : "
    ...
    // message.
    //
    LOGERROR << "rxFilterStart: " << rxFilterStart << " rxFilterSpan: "
    ...
    //
    for (int index = 0; index < rxFilterStart * 2; ++index)
	...
	const int numWindows = static_cast<int>(
	...
	for (int index = rxFilterSpan; index < totalFFTSize; ++index)
	    rxVec_->put(index, kZero);
	...
	*txPulseVec_ = vsip::conj(*txPulseVec_);
	...
	for (int windowIndex = 0; windowIndex < numWindows; ++windowIndex) {
		...
		(*rxOp_)(index) = (*rxVec_)(offset + index);
		...
		outputData.push_back(
		    Messages::Video::DatumType((*rxOp_)(index).real()));
		outputData.push_back(
		    Messages::Video::DatumType((*rxOp_)(index).imag()));
	    }
	}

	// Copy any remaining unfiltered data to the output msg. Again with the doubling of the limiting factor.
	//
	for (size_t index = (rxFilterStart + rxFilterSpan) * 2;
