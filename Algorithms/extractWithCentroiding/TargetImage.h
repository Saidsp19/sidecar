...
template <class TYPE> class TargetImage;
...
public :
    ...
    TargetImage (RANGEBIN minRange,
                 RANGEBIN maxRange,
                 AzimuthDataPtr az,
                 Logger::Log& logg)
        : m_az(az), m_minRange(minRange), m_maxRange(maxRange), log(logg)
    {
    ...
    TargetImage (RANGEBIN minRange,
                 ...
                 TYPE* data,
                 Logger::Log& logg)
        : m_data(data), m_az(az), m_minRange(minRange), m_maxRange(maxRange), log(logg)
    {
        m_cols = maxRange - minRange + 1;
        m_rows = az->size();
    }
    ...
    TargetImage (RANGEBIN minRange,
                 ...
                 TYPE* data,
                 Logger::Log& logg)
        : m_data(data), m_minRange(minRange), m_maxRange(maxRange), log(logg)
    {
        m_cols = maxRange - minRange + 1;
        m_rows = rows;
    }

    void SetAzimuthData (AzimuthDataPtr az) {
        m_az = az;
    }

    AzimuthDataPtr GetAzimuthData () {
        return m_az;
    }
    ...
    void Dump() {
        LOGDEBUG << m_rows << "x" << m_cols
                 << " range=" << m_minRange << " az=" << m_az->front()
                 << std::endl;
        for (int r = 0; r < m_rows; r++) {
            LOGDEBUG << std::setw(3) << (*m_az)[r] << ":";			
            for (int c = 0; c < m_cols; c++) {
                LOGDEBUG << (m_data[(r*m_cols)+c] ? "X" : " ");
            }
            LOGDEBUG << std::endl;
        }
    }
    ...
    //  mask is zero.
    void MaskGradient (BinaryTargetImagePtr maskImage,
                       TargetImagePtr& xGradient,
                       TargetImagePtr& yGradient)
    {
...
#define KERNEL_3_3 ((int)1)
        ...
        xGradient = TargetImagePtr (new TargetImage (m_minRange,
                                                     m_maxRange,
                                                     m_az,
                                                     x_grad,
                                                     log));
        yGradient = TargetImagePtr (new TargetImage (m_minRange,
                                                     m_maxRange,
                                                     m_az,
                                                     y_grad,
                                                     log));
        ...
        for (PRI_COUNT r = 1; r < (m_rows-1); r++) {
            ...
            for (RANGEBIN c = 1; c < (m_cols-1); c++) {
                ...
                if (mask[centerPrevRow-1] && mask[centerPrevRow+0] && mask[centerPrevRow+1] &&
                    mask[center-1] && mask[center+0] && mask[center+1] &&
                    mask[centerNextRow-1] && mask[centerNextRow+0] && mask[centerNextRow+1]) {
                    int x,y;
                    ...
                    x =   KERNEL_1_3 * (int)m_data[centerPrevRow-1]					
                        + KERNEL_2_3 * (int)m_data[centerPrevRow+0]
                        + KERNEL_3_3 * (int)m_data[centerPrevRow+1]
						
                        + KERNEL_1_2 * (int)m_data[center-1]
                        + KERNEL_2_2 * (int)m_data[center+0]
                        + KERNEL_3_2 * (int)m_data[center+1]
					
                        + KERNEL_1_1 * (int)m_data[centerNextRow-1]
                        + KERNEL_2_1 * (int)m_data[centerNextRow+0]
                        + KERNEL_3_1 * (int)m_data[centerNextRow+1];

                    y =   KERNEL_1_1 * (int)m_data[centerPrevRow-1]
                        + KERNEL_2_1 * (int)m_data[center-1]
                        + KERNEL_3_1 * (int)m_data[centerNextRow-1]
						
                        + KERNEL_1_2 * (int)m_data[centerPrevRow]						
                        + KERNEL_2_2 * (int)m_data[center]
                        + KERNEL_3_2 * (int)m_data[centerNextRow]
						
                        + KERNEL_1_3 * (int)m_data[centerPrevRow+1]
                        + KERNEL_2_3 * (int)m_data[center+1]
                    ...
                    x_grad[center] = (VIDEODATA)std::min<int>(std::max<int>(x, (int)std::numeric_limits<VIDEODATA>::min()),
                                                              (int)std::numeric_limits<VIDEODATA>::max());
                    y_grad[center] = (VIDEODATA)std::min<int>(std::max<int>(y, (int)std::numeric_limits<VIDEODATA>::min()),
                                                              (int)std::numeric_limits<VIDEODATA>::max());
                } else {
                    x_grad[center] = 0;
                    y_grad[center] = 0;
    ...
    BinaryTargetImagePtr IdentifyPeaksX () {
        ...
        //point the shared array pointers to the new data
        BinaryTargetImagePtr peaksImg (new BinaryTargetImage (m_minRange,
                                                              m_maxRange,
                                                              m_az,
                                                              peaks,
                                                              log));
    ...
    BinaryTargetImagePtr operator&& (const TargetImage& op) {
        ...
        if ((op.m_rows * op.m_cols) != (m_rows*m_cols)) {
            LOGERROR << "TargetImage::operator& : unable to and image arrays together" << std::endl;
            abort();
        }
        ...
        //point the shared array pointers to the new data
        BinaryTargetImagePtr resultImg (new BinaryTargetImage (m_minRange,
                                                               m_maxRange,
                                                               m_az,
                                                               result,
                                                               log));

        for (unsigned int i = 0; i < (unsigned int)(m_rows*m_cols); i++)
            result[i] = m_data[i] && op.m_data[i];
