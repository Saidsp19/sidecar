...
void SegmentedTargetImage::Dump()
{
    ...
    if (!m_currentRow.Empty()) {
        ...
        m_currentRow.DumpSorted(log);
    }
    LOGDEBUG << std::endl;
}

TargetSize SegmentedTargetImage::GetSize ()
{
...
void SegmentedTargetImage::UpdateMinMaxRanges (RANGEBIN start, RANGEBIN stop)
{
...
void SegmentedTargetImage::AddDataToLastRow (RANGEBIN start, RANGEBIN stop)
{
    ...
    if (stop < start) {
        LOGERROR << "TargetImage::AddDataToLastRow : attempted to add an invalid segment" << std::endl;
        abort();
    }
#endif
...
void SegmentedTargetImage::FinalizeRow (AZIMUTH az) {
...
SegmentedTargetImagePtr SegmentedTargetImage::Merge (TargetVector& mergers, RANGEBIN start,
                                                     RANGEBIN stop,
    ...
    for (TargetVector::size_type i = 1; i < mergers.size(); i++) {
        if (mergers[i]->m_image.size() > masterIm->m_image.size()) {
            masterIm = mergers[i];
        }
    }
        ...
        if (mergers[i] != masterIm) {
            ...
            for (TargetVector::size_type srcRow = 0; srcRow < mergers[i]->m_image.size(); srcRow++) {
                ...
                if (masterIm->m_image[srcRow+destRowOffset].az != mergers[i]->m_image[srcRow].az) {
                    LOGERROR << "SegmentedTargetImage::Merge : tried to merge incompatible rows (az)" << std::endl;
                    abort();
                }
#endif
...
BinaryTargetImagePtr SegmentedTargetImage::MakeBinaryTargetImage () {
    ...
    BinaryTargetImagePtr bti (new BinaryTargetImage (m_minRange,
                                                     m_maxRange,
                                                     az,
                                                     data,
                                                     log));
...
void SegmentedTargetImage::TruncateToOneRow()
{
    if (m_image.size() > 0) {
