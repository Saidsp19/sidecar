...
ExtractWithCentroiding::ExtractWithCentroiding(Controller& controller,
                                               Logger::Log& log)
    : Algorithm(controller, log), m_is(log), m_videoHistory(log),
      m_centroidRangeMin(
	  Parameter::DoubleValue::Make("centroidRangeMin",
                                       "Centroid Range Min (km)", 7.0)),
      m_centroidAzMin(
	  Parameter::DoubleValue::Make("centroidAzMin",
                                       "Centroid Az Min (rad)",
                                       0.30)),
      m_discardRangeMin(
	  Parameter::DoubleValue::Make("discardRangeMin",
                                       "Discard Range Min (km)", 120.0)),
      m_discardAzMin(
	  Parameter::DoubleValue::Make("discardAzMin", "Discard Az Min (rad)",
                                       M_PI))
{
    ;
}
...
{
    registerProcessor<ExtractWithCentroiding,Messages::BinaryVideo>(
	&ExtractWithCentroiding::process);

    return registerParameter(m_centroidRangeMin) &&
	registerParameter(m_centroidAzMin) &&
	registerParameter(m_discardRangeMin) &&
	registerParameter(m_discardAzMin) &&
	Algorithm::startup();
}
...
ExtractWithCentroiding::process(const Messages::BinaryVideo::Ref& msg)
{
    ...
    minCentroidSize.maxRange = (RANGEBIN)
	((m_centroidRangeMin->getValue() - video->getRangeMin()) /
         video->getRangeFactor());
    minCentroidSize.maxRangeValid = true;
    minCentroidSize.maxAz = (AZIMUTH)(m_centroidAzMin->getValue());
    minCentroidSize.maxAzValid = true;

    minDiscardTargetSize.maxRange = (RANGEBIN)
	((m_discardRangeMin->getValue() - video->getRangeMin()) /
         video->getRangeFactor());
    ...
    //detection)
    m_is.AppendScanLine(BinaryScanLineVector(&(msg->getData())),
                        msg->getAzimuthStart(),
                        minDiscardTargetSize);
	...
	if (! extractions)
	    extractions = Messages::Extractions::Make("Extract", msg);
	...
	if (!(size > minCentroidSize)) { //notice, this is not the same as size < minCentroidSize
		...
		Messages::Extraction(Time::TimeStamp::Now(), range, pos.az,
                                     0.0));
	    LOGDEBUG << "target found at az=" << pos.az
		     << " range=" << range
		     << " rangebin=" << pos.range
		     << std::endl;
	}
	    ...
	    LOGDEBUG << "centroiding a target:" << size
		     << " dRange="
		     << msg->getRangeAt(size.RangeExtent())
		     << std::endl;
		...
		extractions->push_back(
		    Messages::Extraction(Time::TimeStamp::Now(), cenRange,
                                         cenPos.az, 0.0));

		LOGDEBUG << "  centroided target found at az=" << cenPos.az
			 << " range=" << cenRange
			 << " rangebin=" << cenPos.range
			 << std::endl;
	    }
