...
ImageSegmentation::ImageSegmentation (Logger::Log& logg)
    : m_maxRowDepth(0), log(logg)
{
...
void ImageSegmentation::CloseAllOpenTargets()
{
    m_closedTargets.insert(m_closedTargets.end(), m_openTargets.begin(), m_openTargets.end());
    m_openTargets.clear();
}

bool ImageSegmentation::IsClosedTargetsEmpty()
{
    return m_closedTargets.empty();
}

SegmentedTargetImagePtr ImageSegmentation::PopClosedTarget()
{
...
void ImageSegmentation::IdentifyNeighborsAndSetMergePending (RANGEBIN range)
{
...
void ImageSegmentation::MergePendingTargetsAndSegment(RANGEBIN start, RANGEBIN stop)
{
    ...
    default :
        ...
        newTarget = SegmentedTargetImage::Merge(m_mergePendingTargets, start, stop,
                                                m_currentMap, m_nextMap, log);
...
void ImageSegmentation::AppendScanLine(const BinaryScanLine& binLine,
                                       AZIMUTH az,
                                       TargetSize discardSize /*= TargetSize()*/)
{
    ...
    for (region = m_openTargets.begin(); region != m_openTargets.end();) {
        if ((*region)->WasUpdated()) {
            ...
            if ((*region)->GetSize() > discardSize) {
                ...
                LOGDEBUG << "truncating a Target rangebin=["
                         << size.minRange << ", " << size.maxRange << "] az=["
