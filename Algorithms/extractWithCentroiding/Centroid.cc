...
Centroid::Centroid (BinaryTargetImagePtr mask,
                    VideoTargetImagePtr video,
                    Logger::Log& logg) :
    m_mask(mask), m_video(video), log(logg), m_is(logg), m_useOriginalTarget(false)
{
}

void Centroid::Process ()
{
    ...
    for (int r = 0; r < rows; r++) {
        m_is.AppendScanLine(BinaryScanLineArray(data + r*cols, cols), (*az)[r]);
    }
    ...
    while (!IsClosedTargetsEmpty()) {
        ...
        for (int a = 0; a < m_mask->m_az->size(); a++)
            if ((*(m_mask->m_az))[a] == tppos.az) { p = a; break; }
...
bool Centroid::IsClosedTargetsEmpty ()
{
...
TargetPosition Centroid::PopClosedTargetPosition ()
{
    if (m_useOriginalTarget) {
