...
VideoStorage(Logger::Log& logg)
    ...
    VideoScanLineVector* GetPRI (PRI_COUNT pri) {
        if (pri >= GetDepth()) {
            LOGERROR << "VideoStorage::GetWindow : pri too large" << std::endl;
            abort();
        }
        return &(m_video[pri]);
    }
    ...
    VideoTargetImagePtr GetWindow (const TargetSize& size) {
        if (!size.priValid) {
            LOGERROR << "VideoStorage::GetWindow : unable to extract video data" << std::endl;
            abort();
        }

        if ((PRI_COUNT)m_video.size() < size.priCount) {
            LOGERROR << "VideoStorage::GetWindow : too few lines in history to extract video" << std::endl;
            abort();
        }
        ...
        for (int i = size.priCount-1; i >= 0; i--) {
            ...
            for (RANGEBIN r = size.minRange; r <= size.maxRange; r++) {
                rawLine[r-size.minRange] = m_video[i][r];
            }
            rawLine += cols;
        }

        VideoTargetImagePtr video (new VideoTargetImage(size.minRange,
                                                        size.maxRange,
                                                        rows,
                                                        rawVid,
                                                        log));
