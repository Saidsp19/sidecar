...
ClutterMap::ClutterMap(Controller& controller, Logger::Log& log) :
    ...
    beginInLearningMode_(
        Parameter::BoolValue::Make("beginInLearningMode", "Begin in Learning Mode", kDefaultBeginInLearningMode)),
    learningScanCount_(
        Parameter::PositiveIntValue::Make("learningScanCount", "Learning Scan Count", kDefaultLearningScanCount)),
    ...
    resetBuffer_(Parameter::NotificationValue::Make("resetBuffer", "Reset Map Buffer", 0)), lastShaftEncoding_(0),
    scanCounter_(-1), mapBuffer_()
{
...
ClutterMap::startup()
{
    ...
    return registerParameter(enabled_) && registerParameter(beginInLearningMode_) &&
           registerParameter(learningScanCount_) && registerParameter(radialPartitionCount_) &&
           registerParameter(alpha_) && registerParameter(loadFilePath_) && registerParameter(loadMap_) &&
           registerParameter(saveFilePath_) && registerParameter(saveMap_) && registerParameter(resetBuffer_) &&
           Algorithm::startup();
}
...
ClutterMap::reset()
{
    ...
    if (!mapBuffer_ && !beginInLearningMode_->getValue() && loadFilePath_->getValue().size() > 0) {
        if (loadMap(loadFilePath_->getValue())) { beginInLearningMode_->setValue(true); }
    }
...
ClutterMap::process(const Messages::Video::Ref& msg)
{
    ...
    if (shaftEncoding + kShaftEncodingHysteresis < lastShaftEncoding_) {
        LOGINFO << "north mark crossing - scanCounter: " << scanCounter_ << " limit: " << learningScanCount_->getValue()
                << std::endl;
        ...
        if (!mapBuffer_) {
            ...
            if (++scanCounter_ == learningScanCount_->getValue()) {
                ...
                if (path.size()) {
                    if (!saveMap(path)) { LOGERROR << "failed to save frozen map" << std::endl; }
                }
            }
        }
    }
    ...
    lastShaftEncoding_ = shaftEncoding;
    if (!enabled_->getValue() || !mapBuffer_) { return send(msg); }
...
ClutterMap::alphaChanged(const Parameter::DoubleValue& value)
{
    ...
    if (mapBuffer_) { mapBuffer_->setAlpha(value.getValue()); }
}
...
ClutterMap::saveMapNotification(const Parameter::NotificationValue& value)
{
    ...
    LOGERROR << std::endl;
    std::string path = saveFilePath_->getValue();
    if (path.size() > 0 && mapBuffer_ && mapBuffer_->isFrozen()) { saveMap(path); }
}
...
FormatInfo(const IO::StatusBase& status, int role)
{
    ...
    return Algorithm::FormatInfoValue(
        QString("Building (%1 scan%2 remaining)").arg(remaining).arg(remaining == 1 ? "" : "s"));
}
