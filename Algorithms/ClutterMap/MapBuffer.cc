...
MapBuffer::Log()
{
	...
	Logger::Log::Find("SideCar.Algorithms.ClutterMap.MapBuffer");
    return log_;
}

MapBuffer::MapBuffer(const std::string& name, size_t radialPartitionCount,
                     float alpha)
    : name_(name), maxRangeBin_(RadarConfig::GetGateCountMax() + 1),
      alpha_(0.05),
      partitionScaling_(float(radialPartitionCount) /
                        float(RadarConfig::GetShaftEncodingMax() + 1)),
      buffer_(radialPartitionCount * maxRangeBin_, 0.0),
      radialCounts_(radialPartitionCount, 0), isLearning_(true)
{
    static Logger::ProcLog log("MapBuffer", Log());
    LOGINFO << "radialPartitionCount: " << radialPartitionCount
	    << " shaftMax: " << (RadarConfig::GetShaftEncodingMax() + 1)
	    << " partionScaling: " << partitionScaling_
...
MapBuffer::add(const Video::Ref& msg)
{
    ...
    size_t radialIndex = size_t(::floor(msg->getShaftEncoding() *
                                        partitionScaling_));
    LOGINFO << "encoding: " << msg->getShaftEncoding() << " index: "
	    << radialIndex << std::endl;
    if (radialIndex >= radialCounts_.size()) {
	LOGERROR << "radialIndex is too big!" << std::endl;
    }
...
MapBuffer::save(std::ostream& os)
{
    ...
    std::copy(radialCounts_.begin(), radialCounts_.end(),
              std::ostream_iterator<float>(os, "\n"));
    return os.good();
}
