...
CPIIntegrate::CPIIntegrate(Controller& controller, Logger::Log& log)
    : Super(controller, log, kDefaultEnabled, kDefaultCpiSpan),
      numCPIs_(Parameter::PositiveIntValue::Make("numCPIs", "Number of CPIs to average<br>", kDefaultNumCPIs)),
      cpis_(), vals_()
{
...
CPIIntegrate::startup()
{
    ...
    for(itr = vals_.begin(); itr != vals_.end(); itr++) {
	(*itr).resize(1000, 0.0);
    }
...
CPIIntegrate::processCPI() 
{
    ...
    //
    if(cpis_.size() < size_t(numCPIs + 1)) {
        return rc;
    }
