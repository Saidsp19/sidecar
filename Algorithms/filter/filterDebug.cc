...
std::ostream&
operator<<(std::ostream& os, vsip::Vector<VIDEODATA> v)
{
    int idx = 0;
    int size = v.size();
    for (; idx < size; ++idx) os << v.get(idx) << '\t';
    return os;
}

std::ostream&
operator<<(std::ostream& os, vsip::Vector<COMPLEXVIDEODATA> v)
{
    int idx = 0;
    int size = v.size();
    for (; idx < size; ++idx) {
...
std::ostream&
operator<<(std::ostream& os, vsip::Matrix<VIDEODATA> m)
{
    int idx = 0;
    int size = m.size(0); /* m.size(1) is the size of a dimension of the matrix */
    for (; idx < size; ++idx) os << (vsip::Vector<VIDEODATA>)m.row(idx) << std::endl;
    return os;
}

// Prints out a matrix row-wise
std::ostream&
operator<<(std::ostream& os, vsip::Matrix<COMPLEXVIDEODATA> m)
{
    int idx = 0;
    int size = m.size(0); /* m.size(1) is the size of a dimension of the matrix */
    for (; idx < size; ++idx) os << (vsip::Vector<COMPLEXVIDEODATA>)m.row(idx) << std::endl;
    return os;
}

vsip::Matrix<VIDEODATA>*
...
main(int argc, const char* argv[])
{
    ...
    */
    ...
    using FFT_r2c =
        vsip::Fft<vsip::const_Matrix, VIDEODATA, COMPLEXVIDEODATA, 1, vsip::by_reference, 1, vsip::alg_time>;
    using FFT_c2c =
        vsip::Fft<vsip::const_Matrix, COMPLEXVIDEODATA, COMPLEXVIDEODATA, vsip::fft_fwd, vsip::by_reference>;
    using IFFT_c2c =
        vsip::Fft<vsip::const_Matrix, COMPLEXVIDEODATA, COMPLEXVIDEODATA, vsip::fft_inv, vsip::by_reference>;

    vsip::Matrix<COMPLEXVIDEODATA> zeroPaddedKernel(input->size(0), input->size(1), COMPLEXVIDEODATA(0.0f, 0.0f));
    vsip::Matrix<COMPLEXVIDEODATA> fft_image(input->size(0), input->size(1), COMPLEXVIDEODATA(0.0f, 0.0f));
    vsip::Matrix<COMPLEXVIDEODATA> fft_zeroPaddedKernel(input->size(0), input->size(1), COMPLEXVIDEODATA(0.0f, 0.0f));
    ...
    cout << "converting the image from real to complex data" << std::endl;
    for (int r = 0; r < input->size(0); r++)
        for (int c = 0; c < input->size(1); c++) fft_image(r, c) = COMPLEXVIDEODATA((float)(*input)(r, c), 0.0f);
