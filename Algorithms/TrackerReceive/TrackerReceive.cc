...
TrackerReceive::TrackerReceive(Controller &controller, Logger::Log& log) :
    Algorithm(controller, log)
{
}

bool TrackerReceive::startup()
{
    // Spawn the receive thread
    int started=ACE_Thread_Manager::instance()->spawn(
        TrackerReceive::threaded_receiver,
...
void * TrackerReceive::threaded_receiver(void *arg)
{
    ...
    {
        FD_ZERO(&fds);
        for (iter = rp_sockets.begin(); iter != rp_sockets.end(); iter++)
            FD_SET(*iter, &fds);
                ...
                for (size_t curTrackNum=0; curTrackNum<track_msg.num_tracks(); curTrackNum++)
                {
                    ...
                    for (int cc=0; cc<2; cc++)
                        for (int dd=0; dd<2; dd++)
                            noise[cc][dd] = trackInfo->noise[cc][dd];

                    double cov[9][9];

                    for (int cc=0; cc<9; cc++)
                        for (int dd=0; dd<9; dd++)
                            cov[cc][dd] = trackInfo->cov[cc][dd];
...
// DLL support
//
extern "C" ACE_Svc_Export Algorithm* TrackerReceiveMake(Controller &controller,
                                                        Logger::Log& log);

Algorithm* TrackerReceiveMake(Controller &controller, Logger::Log& log)
{
