...
IQ2AmplitudePhase::IQ2AmplitudePhase(Controller& controller, Logger::Log& log) :
    Algorithm(controller, log), param_k(Parameter::DoubleValue::Make("k", 2)), old(0), older(0)
{
}

bool
IQ2AmplitudePhase::startup()
{
...
IQ2AmplitudePhase::process(Messages::Video::Ref msg, uint port)
{
    ...
    switch (slot) {
    case 0: I.push_back(msg); break;
    case 1: Q.push_back(msg); break;
    default:
        // unknown channel
        return false;
    }
    ...
    while (!I.empty() && !Q.empty()) {
        ...
        if (n1 == n2) {
            ...
            if (!send(amp, 0)) return false;

            if (!send(phase, 1)) return false;
        } else if (n1 < n2) {
            I.pop_front();
        } else {
            Q.pop_front();
