...
Printer::Printer(Controller& controller, Logger::Log& log)
    : Super(controller, log, kDefaultEnabled, kDefaultMaxBufferSize),
      isIQ_(Parameter::BoolValue::Make(
                "isIQ", "Is the data complex", kDefaultIsIQ)),
      startBin_(Parameter::NonNegativeIntValue::Make(
                    "binStart", "Index of first sample to print",
                    kDefaultBinStart)),
      count_(Parameter::PositiveIntValue::Make(
                 "binCount", "Number of samples to print",
                 kDefaultBinCount))
{
    ;
}
...
Printer::startup()
{
    return Super::startup() && registerParameter(startBin_) &&
	registerParameter(count_) && registerParameter(isIQ_);
}

ChannelBuffer*
Printer::makeChannelBuffer(int channelIndex, const std::string& name,
                           size_t maxBufferSize) 
{
    const IO::Channel& channel(
	getController().getInputChannel(channelIndex));

    switch (channel.getTypeKey()) {
    case Messages::MetaTypeInfo::Value::kVideo:
	printers_.push_back(
	    new TMessagePrinter<Printer,Messages::Video>(
		channel.getName(), this, &Printer::printVideo));
	return new TChannelBuffer<Messages::Video>(*this, channelIndex,
                                                   maxBufferSize);
	break;

    case Messages::MetaTypeInfo::Value::kBinaryVideo:
	printers_.push_back(
	    new TMessagePrinter<Printer,Messages::BinaryVideo>(
		channel.getName(), this, &Printer::printBinaryVideo));
	return new TChannelBuffer<Messages::BinaryVideo>(*this, channelIndex,
                                                         maxBufferSize);
	break;
...
Printer::processChannels()
{
    ...
    //
    LOGDEBUG << "enabled: " << isEnabled() << "  isIQ: " << isIQ_->getValue()
	     << std::endl;
	...
	for (size_t index = 0; index < getChannelCount(); ++index)
	    getGenericChannelBuffer(index)->popFront();
	return true;
    }
...
{
    std::cout << "Seq: " << msg->getRIUInfo().sequenceCounter
	      << " MsgDesc: " << msg->getRIUInfo().msgDesc
	      << " When: " << msg->getRIUInfo().timeStamp
	      << " Shaft: " << msg->getRIUInfo().shaftEncoding
	      << " PRF: " << msg->getRIUInfo().prfEncoding
	      << " IRIG: " << msg->getRIUInfo().irigTime
	      << " Range: " << msg->getRIUInfo().rangeMin << '/'
	      << msg->getRIUInfo().rangeFactor << '\n';
}
...
Printer::printChannelName(const std::string& name)
{
    ...
    std::cout << "Channel: " << std::setw(maxNameWidth)
	      << name.substr(0, maxNameWidth) << ' ';
}

void
Printer::printVideo(const std::string& channelName,
                    const Messages::Video::Ref& msg, int start, int count)
{
    printChannelName(channelName);
    if (start < msg->size()) {
	if (start + count > msg->size())
	    count = msg->size() - start;
...
void
Printer::printBinaryVideo(const std::string& channelName,
                          const Messages::BinaryVideo::Ref& msg, int start,
                          int count)
{
    printChannelName(channelName);
    if (start < msg->size()) {
