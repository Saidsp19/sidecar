...
{
    registerProcessor<WindowedMinMax, Messages::Video>(&WindowedMinMax::processInput);
    return registerParameter(enabled_) && registerParameter(windowSize_) && registerParameter(windowCount_) &&
           registerParameter(windowOffset_) && registerParameter(valueGenerator_) && Super::startup();
}
...
WindowedMinMax::processInput(const Messages::Video::Ref& msg)
{
    ...
    //
    Messages::Video::Ref out(Messages::Video::Make("WindowedMinMax::processInput", msg));
    ...
    for (int count = 0; count < windowCount; ++count) {
        ...
        //
        int sum = 0;
        for (int sample = 0; sample < windowSize; ++sample) sum += msg[index++];
    ...
    for (int sample = 0; sample < msgSize; ++sample) {
        ...
        for (size_t count = 0; count < windowIndices_.size(); ++count) {
            ...
            if (index >= 0 && index < msgSize) value -= msg[index];
...
WindowedMinMax::getAverage() const
{
    ...
    for (size_t index = 0; index < size; ++index) sum += windowSums_[index];
...
WindowedMinMax::getMin() const
{
    ...
    for (size_t index = 1; index < size; ++index)
        if (windowSums_[index] < found) found = windowSums_[index];
    return found;
}
...
WindowedMinMax::getMaxMin() const
{
    ...
    return std::max(*std::min_element(windowSums_.begin(), windowSums_.begin() + size),
                    *std::min_element(windowSums_.begin() + size, windowSums_.end())) +
           std::min(*std::max_element(windowSums_.begin(), windowSums_.begin() + size),
                    *std::max_element(windowSums_.begin() + size, windowSums_.end()));
}
...
WindowedMinMax::valueGeneratorChanged(const ValueGeneratorParameter& param)
{
    ...
    default: LOGERROR << "invalid selection - " << param.getValue() << std::endl; break;
    }
}
