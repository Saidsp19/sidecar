...
Test::test()
{
    ...
    assertTrue(controller->openAndInit("SimpleNOOP"));

    SimpleNOOP* alg = dynamic_cast<SimpleNOOP*>(controller->getAlgorithm());
    assertTrue(alg);
...
{
    // Change the following checks so that they apply to the SimpleNOOP algorithm.
    //
