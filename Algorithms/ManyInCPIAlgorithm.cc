...
//
ManyInCPIAlgorithm::ManyInCPIAlgorithm(Controller& controller,
                                       Logger::Log& log, bool enabled,
                                       size_t maxBuffersize, size_t cpiSpan)
    : Super(controller, log, enabled, maxBuffersize),
      cpiSpan_(Parameter::PositiveIntValue::Make(
                   "cpiSpan", "Number of PRIs in a CPI", cpiSpan))
{
    ;
}
...
{
    ChannelCPIPrune(size_t cpiSpan)
	: cpiSpan_(cpiSpan), ok_(true), valid_(false) {}
    ...
    void operator()(ChannelBuffer* channel)
	{
		...
		//
		cpiBoundary = (last->getRIUInfo().prfEncoding != 
                               (*itr)->getRIUInfo().prfEncoding);
		...
		if (cpiBoundary) {
			...
			// the next CPI
			//
			if (! channel->pruneToSequenceCounter(
                                (*itr)->getSequenceCounter()))
			    ok_ = false;
...
{
    VerifyChannelSize(size_t cpiSpan)
	: cpiSpan_(cpiSpan), ok_(true), valid_(false) {}
...
{
    return std::for_each(channels_.begin(), channels_.end(),
                         VerifyChannelSize(min));
}
...
ManyInCPIAlgorithm::processMessageReceived()
{
    ...
    //
    bool ok = pruneToSequenceCounter(maxSeq) && pruneToCPIBoundary(cpiSpan)
	&& verifyChannelSize(cpiSpan);
