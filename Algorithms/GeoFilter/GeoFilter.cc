...
struct Filter {
	...
	: name(),
	  ...
	  azMin(0.0), azMax(0.0),
	  attenuation(1.0), offset(0.0),
	  clampMin(std::numeric_limits<short>::min()),
	  clampMax(std::numeric_limits<short>::max()) {}
    QString name;
    double rangeMin;
    double rangeMax;
    double azMin;
...
GeoFilter::GeoFilter(Controller& controller, Logger::Log& log)
    : Super(controller, log),
      ...
      load_(Parameter::NotificationValue::Make("load", "Load Configuration", 0)),
      p_(new Private)
{
...
GeoFilter::startup()
{
    ...
    return p_->stateEmitter_.open(getController().getTaskName()) &&
	registerParameter(enabled_) &&
	registerParameter(configPath_) &&
	registerParameter(load_) &&
	Super::startup();
}
...
GeoFilter::loadConfigFile(const std::string& path)
{
    ...
    int errorColumn;
    QDomDocument dom;
    if (! dom.setContent(&file, false, &errorText, &errorLine,
                         &errorColumn)) {
	LOGERROR << "failed to parse file: line " << errorLine << " - "
		 << errorText << std::endl;
	return false;
    }
    ...
    static const QRegExp trueRE("(t(rue)?)|(y(es)?)|1",
                                Qt::CaseInsensitive);
    static const QRegExp falseRE("(f(alse)?)|(n(o)?)|0",
                                 Qt::CaseInsensitive);
	...
	if (trueRE.exactMatch(value)) {
	    ...
	    LOGERROR << "invalid/missing enabled attribute value - "
		     << value << std::endl;
	    return false;
	}
	...
	for (int index = 0; index < children.count(); ++index) {
	    ...
	    if (tag == "range") {
		QStringList values = element.text().split(
		...
		bool ok = false;
		filter.azMin =
		    Utils::normalizeRadians(
			Utils::degreesToRadians(values[0].toDouble(&ok)));
		if (! ok) {
		...
		filter.azMax =
		    Utils::normalizeRadians(
			Utils::degreesToRadians(values[1].toDouble(&ok)));
		if (! ok) {
	...
	if (enabled) {
	    LOGWARNING << "adding filter - " << filter.name
		       << " az: "
		       << Utils::radiansToDegrees(filter.azMin) << '/'
		       << Utils::radiansToDegrees(filter.azMax)
		       << " range: "
		       << filter.rangeMin << '/' << filter.rangeMax
		       << " attenuation: " << filter.attenuation
		       << " offset: " << filter.offset
		       << " clamp: "
		       << filter.clampMin << '/' << filter.clampMax
		       << std::endl;
...
GeoFilter::processInput(const Messages::Video::Ref& inMsg)
{
	...
	if (filter.azMin <= azimuth && azimuth <= filter.azMax) {
	    ...
	    Messages::Video::const_iterator in = inMsg->begin() +
		size_t(offset);
	    Messages::Video::iterator out = outMsg->begin() +
		size_t(offset);
	    ...
	    Messages::Video::const_iterator end = inMsg->begin() +
		size_t(offset);
...
FormatInfo(const IO::StatusBase& status, int role)
{
    ...
    QString configPath(QString::fromStdString(
                           status[GeoFilter::kConfigPath]));
    int filterCount = status[GeoFilter::kActiveFilterCount];
    return Algorithm::FormatInfoValue(QString("Active: %1  Path: %2")
                                      .arg(filterCount).arg(configPath));
}
