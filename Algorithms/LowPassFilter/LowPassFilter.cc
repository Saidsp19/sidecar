...
LowPassFilter::loadKernel()
{
    ...
    if (file == "") {
        LOGERROR << "No kernel file provided.  Using default!" << std::endl;
    }
	...
	if(!(converter >> val)) {
            LOGERROR << "Unable to load kernel for filter.  Bad value: " << line
                     << std::endl;
            return false;
	} else {
	...
	kernel_->put(i, ComplexType(alphas[i], 0.0));
    }	
