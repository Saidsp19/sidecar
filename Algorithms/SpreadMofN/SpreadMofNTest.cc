...
Test::test()
{
    ...
    Utils::TemporaryFilePath testOutputPath("SpreadMofNTest");
    {
        ...
        SpreadMofN* spreadMofN = dynamic_cast<SpreadMofN*>(controller->getAlgorithm());

        assertTrue(spreadMofN);

        BinaryVideo::DatumType data[41][5] = {// CPI # 1
        ...
        //
        BinaryVideo::Ref msg(
            BinaryVideo::Make("test", vme, (BinaryVideo::DatumType*)data, (BinaryVideo::DatumType*)data + 5));
        ...
        for (int i = 0; i < 5; i++) {
            ...
            controller->injectControlMessage(ParametersChangeRequest(parameterChange, false));
    ...
    for (int i = 0; i < 5; i++) {
        ...
        assertTrue(reader->isMessageAvailable());
        {
            ...
            for (size_t j = 0; j < 5; j++) { assertEqual(results[i][j], *pos++); }
