...
SegmentStats::SegmentStats(Controller& controller, Logger::Log& log) :
    ...
    maxRangeDrop(Parameter::NormalizedValue::Make("maxRangeDrop",
                                                  "Max Range Drop", 0.5)),
    maxAzDrop(Parameter::NormalizedValue::Make("maxAzimuthDrop",
					       "Max Azimuth Drop", 0.5)),
    minPower(Parameter::NormalizedValue::Make("minPower", "Min Power", 0.5)),
    azimuthEncodings(RadarConfig::GetShaftEncodingMax() + 1),
    rangeGates(RadarConfig::GetGateCountMax()),
    buffer(azimuthEncodings, rangeGates, deltaAz->getValue(),
	   deltaRange->getValue()),
    row(buffer)
{
...
{
    registerProcessor<SegmentStats,SegmentMessage>(
	&SegmentStats::processSegmentMessage);
    registerProcessor<SegmentStats,Video>(
	&SegmentStats::processVideo);

    return registerParameter(deltaRange) &&
	registerParameter(deltaAz) &&
	registerParameter(maxRangeDrop) &&
	registerParameter(maxAzDrop) &&
	registerParameter(minPower) &&
	Algorithm::startup();
}
...
SegmentStats::processSegmentMessage(SegmentMessage::Ref pri)
{
    ...
    SegmentMessage::Ref output(new SegmentMessage("SegmentStats",
                                                  pri,
                                                  pri->getRangeMin(),
                                                  pri->getRangeFactor()));
...
int
SegmentStats::findRangeEnd(size_t az, size_t start, int maxDistance, int bound,
			   VideoT threshold)
{
    int dir=1;
    if(maxDistance<0)
...
int
SegmentStats::findAzEnd(size_t range, size_t start, int maxDistance, int bound,
			size_t restart, VideoT threshold)
{
    int dir=1;
    if(maxDistance<0)
        dir=-1;
    ...
    if(shortStop!=stop)
    {
        ...
        for(int az=restart; az!=newStop; az+=dir)
        {
            ...
            VideoT p=buffer.get(az, range);
            if(p<threshold || p<maxDrop*p0)
                return (az-restart)+(shortStop-start);
            p0=p;
        }
    }
