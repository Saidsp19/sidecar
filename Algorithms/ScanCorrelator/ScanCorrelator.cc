...
ScanCorrelator::ScanCorrelator(Controller& controller, Logger::Log& log)
    : Algorithm(controller, log),
      param_searchRadius(Parameter::DoubleValue::Make("searchRadius",
						      "Search Radius", kDefaultSearchRadius)),
      ...
      param_numScans(Parameter::IntValue::Make("numScans", "Num scans to correlate over", kDefaultNumScans))
{
    param_searchRadius->connectChangedSignalTo(
        boost::bind(&ScanCorrelator::on_searchRadius_changed, this, _1));
    param_scanTime->connectChangedSignalTo(
        boost::bind(&ScanCorrelator::on_scanTime_changed, this, _1));
...
{
    registerProcessor<ScanCorrelator,Messages::Extractions>(
	&ScanCorrelator::process);
...
ScanCorrelator::process(const Messages::Extractions::Ref& msg)
{
    ...
    Messages::Extractions::Ref result =
	Messages::Extractions::Make("ScanCorrelator", msg);
...
ScanCorrelator::corr(Extraction &ext, time_t time)
{
...
#ifdef SCAN_CORR_DEBUG
    cerr
	<< "Correlating extraction ("
	<< ext.getX() << ", "
	<< ext.getY() << ") into ["
	<< binX << ", "
	<< binY << "] numBins=" << numBins << endl;
#endif
    ...
    if (!d.extraction.getCorrelated())
	d.extraction.setNumCorrelations(0);
