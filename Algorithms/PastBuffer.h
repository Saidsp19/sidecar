...
{
public:
    PastBufferBase(size_t capacity)
	: capacity_(capacity), maxMsgSize_(0) {}
...
public:
    ...
    PastBuffer(size_t capacity)
	: PastBufferBase(capacity), msgs_() {}
    ...
    void clear() { msgs_.clear(); setMaxMsgSize(0); }

    void add(typename T::Ref msg)
	{
	    msgs_.push_front(msg);
	    while (msgs_.size() > getCapacity()) {
		msgs_.pop_back();
	    }
		...
		std::for_each(msgs_.begin() + 1, msgs_.end(),
                              ResizeMsg(getMaxMsgSize()));
	    }
	}
...
private:
	...
	void operator()(typename T::Ref msg) const
	    { msg->resize(size_, 0); }
    };
