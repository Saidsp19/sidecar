...
//
CPIAlgorithm::CPIAlgorithm(Controller& controller, Logger::Log& log, 
                           bool enabled, int cpiSpan)
    : Super(controller, log),
      cpiSpan_(Parameter::PositiveIntValue::Make("cpiSpan", "# of PRIs composing CPI", cpiSpan)),
      dropIncompleteCPI_(Parameter::BoolValue::Make("dropIncompleteCPI", "Drop a CPI if any of its PRIs are missing", 0)),
...
CPIAlgorithm::startup()
{
    ...
    return registerParameter(enabled_) &&
        registerParameter(dropIncompleteCPI_) &&
        registerParameter(cpiSpan_) &&
        Super::startup();
}
