...
static const char* kColorEncodingNames[] = {
    "HSV",
    "RGB"
};
...
CIntegrate::CIntegrate(Controller& controller, Logger::Log& log)
    : Super(controller, log),
      ...
      min_(Parameter::NonNegativeIntValue::Make(
               "min", "Minimum magnitude value to normalize", kDefaultMin)), 
      max_(Parameter::NonNegativeIntValue::Make(
               "max", "Maximum magnitude value to normalize", kDefaultMax)), 
      numPRIs_(Parameter::PositiveIntValue::Make(
                   "numPRIs", "# PRIs to integrate", kDefaultNumPRIs)),
      colorEncoding_(ColorEncodingParameter::Make(
                         "colorEncoding_", "Color Encoding",
                         ColorEncoding(kDefaultColorEncoding))),
      moveSlidingWin_(Parameter::PositiveIntValue::Make(
                          "moveSlidingWin",
                          "# PRIs to advance window each CPI",
                          kDefaultMoveSlidingWin)),
      dopplerBin_(Parameter::NonNegativeIntValue::Make(
                      "dopplerBin",
                      "Doppler bin to use for output channel",
                      kDefaultDopplerBin)),
      PRIs_(), fftOut_(), N_(kDefaultNumPRIs), maxSpan_(0), buffer_(), H_(),
      resultsIndex_(controller.getOutputChannelIndex("results")),
      rgbIndex_(controller.getOutputChannelIndex("rgb")),
      maxAmpIndex_(controller.getOutputChannelIndex("maxAmp")),
      dopplerBinOutIndex_(controller.getOutputChannelIndex(
                              "dopplerBinOutput")),
      kInvalidChannel_(getController().getNumOutputChannels())
{
    numPRIs_->connectChangedSignalTo(
	boost::bind(&CIntegrate::numPRIsChanged, this, _1));
    min_->connectChangedSignalTo(
	boost::bind(&CIntegrate::minValueChanged, this, _1));
    max_->connectChangedSignalTo(
	boost::bind(&CIntegrate::maxValueChanged, this, _1));
}

bool
CIntegrate::startup()
{
    registerProcessor<CIntegrate,Messages::Video>(
        &CIntegrate::processInput);

    scalingChanged();

    return Super::startup() &&
	registerParameter(enabled_) &&
	registerParameter(min_) &&
	registerParameter(max_) &&
	registerParameter(numPRIs_) &&
	registerParameter(moveSlidingWin_) &&
	registerParameter(dopplerBin_) &&
...
CIntegrate::processInput(const Messages::Video::Ref& msg)
{
	...
	for (; colIndex < maxSpan_; ++colIndex)
	    row.put(colIndex, zero);
    }
...
CIntegrate::processHSV(Messages::Video::Container& outputData)
{
	...
	void operator+=(const RGBType& rhs) {
	...
	outputData.push_back(
	    Messages::Video::DatumType(EncodeRGB(sum.R, sum.G, sum.B)));
    }
}
...
CIntegrate::processRGB(Messages::Video::Container& outputData)
{
	...
	size_t j = 0;
	float G = 0.0;
	for (size_t limit = ::rint(shift); j < limit; ++j)
	    G += scaleAmp(ComplexType(col(j)).real());
	float B = 0.0;
	for (size_t limit = ::rint(shift + span); j < limit; ++j)
	    B += scaleAmp(ComplexType(col(j)).real());
	float R = 0.0;
	for (size_t limit = ::rint(shift + span * 2); j < limit; ++j)
	    R += scaleAmp(ComplexType(col(j)).real());
	for (; j < N_; ++j)
	    G += scaleAmp(ComplexType(col(j)).real());
...
CIntegrate::dimensionsChanged()
{
	...
	// every sample.
	//
	fftm_.reset(new ForwardFFTM(vsip::Domain<2>(N_, maxSpan_),
                                    1.0 / N_));
    }
