...
CPIMarker::CPIMarker(Controller& controller, Logger::Log& log) :
    Super(controller, log), last_(0), enabled_(Parameter::BoolValue::Make("enabled", "Enabled", kDefaultEnabled)),
...
CPIMarker::startup()
{
    ...
    bool ok = true;
    ok = ok && registerParameter(scale_) && registerParameter(freqMHz_) && registerParameter(enabled_);
...
CPIMarker::processInput(const Messages::Video::Ref& msg)
{
...
#if 1
    uint32_t delta = msg->getRIUInfo().timeStamp - last_->getRIUInfo().timeStamp + 1;
    uint32_t prfFreq = uint32_t(delta / 1000);

    LOGDEBUG << "Msg: " << msg->getRIUInfo().sequenceCounter << " has delta = " << delta << " and prfFreq = " << prfFreq
             << std::endl;
    ...
    LOGDEBUG << "Msg: " << msg->getRIUInfo().sequenceCounter << " has size = " << msg->size()
             << " and prfFreq = " << prfFreq << std::endl;
