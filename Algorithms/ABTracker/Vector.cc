...
Vector::Vector()
    : flags_(kHasMagnitudeSquared | kHasMagnitude | kHasDirection | kHasElevation),
      x_(0.0), y_(0.0), z_(0.0), magnitudeSquared_(0.0), magnitude_(0.0), direction_(0.0), elevation_(0.0)
{
    ;
}

Vector::Vector(double x, double y, double z)
    : flags_(0), x_(x), y_(y), z_(z)
{
    ;
}
...
Vector::Vector(const UnitVector& uv, double length)
    : flags_(kHasMagnitudeSquared | kHasMagnitude | kHasDirection | kHasElevation),
      x_(uv.getX() * length), y_(uv.getY() * length), z_(uv.getZ() * length), magnitudeSquared_(length * length),
      magnitude_(length), direction_(uv.getDirection()), elevation_(uv.getElevation())
{
    ;
}
...
Vector::operator/=(double scale)
{
	...
	std::cerr << "Vector::operator/=(): attempt to divide by very small " << "number - " << scale
                  << std::endl;
