...
struct Test : public UnitTest::TestObj, public ACE_Event_Handler {
    ...
    Test() : UnitTest::TestObj("ABTracker"), ACE_Event_Handler(), controller_(), iteration_(0) {}
...
WatchdogTimer::WatchdogTimer() : ACE_Event_Handler(ACE_Reactor::instance())
{
...
Sink::deliverDataMessage(ACE_Message_Block* data, ACE_Time_Value* timeout)
{
    ...
    if (mgr.hasNative()) {
        ...
        if (mgr.getNativeMessageType() == MetaTypeInfo::Value::kTSPI) {
            ...
            msgs_.push_back(msg);
            if (msgs_.size() == outputCount) ACE_Reactor::instance()->end_reactor_event_loop();
        }
    }
...
Test::test()
{
    ...
    for (size_t index = 0; index < sink->msgs_.size(); ++index) { testOutput(index, sink->msgs_[index]); }
...
Test::generateInput()
{
    ...
    for (size_t index = 0; index < inputCount; ++index) {
        ...
        Extraction extraction(input.when, input.range, Utils::degreesToRadians(input.azimuth), input.elevation);
...
Test::testOutput(size_t counter, const TSPI::Ref& msg)
{
    ...
    assertEqualEpsilon(output.azimuth, Utils::radiansToDegrees(msg->getAzimuth()), 0.0001);
