...
public:
    ...
    */
	...
	{
	    if (other->getGateMin() < gateMin_)
		gateMin_ = other->getGateMin();
	    if (other->getGateMax() > gateMax_)
		gateMax_ = other->getGateMax();
	    valid_ = true;
	}
    ...
    */
    ...
    double getIRIGTime() const
	{ return (irigEnd_ + irigStart_) / 2.0; }
    ...
    */
    float getAzimuth() const
	{
	    ...
	    return Utils::normalizeRadians(
		(azimuthStart_ + azimuthEnd) / 2.0);
	}
...
private:
    ...
    */
	...
	: irigStart_(irig), irigEnd_(irig), azimuthStart_(azimuth),
	  azimuthEnd_(azimuth), gateMin_(gate), gateMax_(gate),
	  valid_(true) {}
...
}}				// end namespace SideCar::Algorithms

using namespace SideCar;
using namespace SideCar::Algorithms;
using namespace SideCar::Messages;

Extract::Extract(Controller& controller, Logger::Log& log)
...
Extract::process(const Messages::BinaryVideo::Ref& msg)
{
    ...
    // the same time.
    //
    if (msg->size() > gateTargets_.size())
	gateTargets_.resize(msg->size(), Target::Ref());
	...
	if (data[gate]) {
		...
		if (gateTargets_[gate]) {
		    ...
		    target = Target::Make(msg->getIRIGTime(), gate,
                                          msg->getAzimuthStart());
	    ...
	    //
	    target->update(msg->getIRIGTime(), gate - 1,
                           msg->getAzimuthStart());
	    target.reset();
	}
    }
	...
	if (target->testValidAndReset()) {
	    ++it;
	    for (int gate = target->getGateMin(); gate <= target->getGateMax();
                 ++gate)
	    ...
	    if (! extractions)
		extractions = Extractions::Make("Extract", msg);
