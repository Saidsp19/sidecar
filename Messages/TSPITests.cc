...
Test::test()
{
    ...
    TSPI::Ref msg = TSPI::MakeLLH("Test", "site", 0.0, targetLatitude,
                                  targetLongitude, targetHeight);

    assertEqual(Utils::degreesToRadians(targetLatitude),
                msg->getLatitude());
    assertEqual(Utils::degreesToRadians(targetLongitude),
                msg->getLongitude());
	...
	{
	    msg = TSPI::MakeLLH("Test", "north", 0.0, targetLatitude + 10.0,
                                targetLongitude, targetHeight);
	    IO::MessageManager mgr(msg);
	    assertTrue(writer->write(mgr.getMessage()));
	}
	{
	    msg = TSPI::MakeLLH("Test", "south", 0.0, targetLatitude - 10.0,
                                targetLongitude, targetHeight);
	    IO::MessageManager mgr(msg);
	    assertTrue(writer->write(mgr.getMessage()));
	}
	{
	    msg = TSPI::MakeLLH("Test", "east", 0.0, targetLatitude,
                                targetLongitude + 10.0, targetHeight);
	    IO::MessageManager mgr(msg);
	    assertTrue(writer->write(mgr.getMessage()));
	}
	{
	    msg = TSPI::MakeLLH("Test", "west", 0.0, targetLatitude,
                                targetLongitude - 10.0, targetHeight);
	    IO::MessageManager mgr(msg);
	    assertTrue(writer->write(mgr.getMessage()));
	}
	{
	    msg = TSPI::MakeXYZ("Test", "xyz", 0.0, 0.0,
                                0.0, 0.0);
	    IO::MessageManager mgr(msg);
	    assertTrue(writer->write(mgr.getMessage()));
	}
	...
	double epsilon = 1.0E-8;
	{
	    ...
	    assertEqualEpsilon(targetLatitude,
                               Utils::radiansToDegrees(msg->getLatitude()),
                               epsilon);
	    assertEqualEpsilon(targetLongitude,
...

	{
	    ...
	    assertEqualEpsilon(targetLatitude + 10.0,
                               Utils::radiansToDegrees(msg->getLatitude()),
                               epsilon);
	    assertEqualEpsilon(targetLongitude,
                               Utils::radiansToDegrees(msg->getLongitude()),
...

	{
	    ...
	    assertEqualEpsilon(180.0,
                               Utils::radiansToDegrees(msg->getAzimuth()),
                               0.001);
	    assertEqualEpsilon(355.001,
                               Utils::radiansToDegrees(msg->getElevation()),
                               0.001);
	}
...

	{
	    ...
	    assertEqualEpsilon(targetLatitude,
                               Utils::radiansToDegrees(msg->getLatitude()),
                               epsilon);
	    assertEqualEpsilon(targetLongitude + 10.0,
                               Utils::radiansToDegrees(msg->getLongitude()),
                               epsilon);
	    assertEqualEpsilon(targetHeight, msg->getHeight(), epsilon);

	    assertEqualEpsilon(879363.732, msg->getRange(), 0.001);
	    assertEqualEpsilon(86.929,
                               Utils::radiansToDegrees(msg->getAzimuth()),
                               0.001);
	    assertEqualEpsilon(356.0521,
                               Utils::radiansToDegrees(msg->getElevation()),
                               0.001);
	}
...

	{
	    ...
	    assertEqualEpsilon(273.071,
                               Utils::radiansToDegrees(msg->getAzimuth()),
                               0.001);
	    assertEqualEpsilon(356.052,
                               Utils::radiansToDegrees(msg->getElevation()),
                               0.001);
	}
