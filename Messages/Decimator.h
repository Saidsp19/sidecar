...
public:
    ...
    */
	...
	{ return source_->Header::getMessageSize() + size() * sizeof(DatumType); }
    ...
    */
	...
	ptrdiff_t distance_to(const_iterator const& other) const
            { return std::distance(cit_, other.cit_) / factor_; }
    ...
    */
	...
	{ return const_iterator(source_->begin() + size() * factor_, factor_); }
...
private:
    ...
    */
    TDecimator(const SourceRef& source, size_t factor)
	: Header(*source), source_(source), factor_(factor)
