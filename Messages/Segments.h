...
{
public:
    Segment(size_t azimuth=0, size_t start=0, size_t stop=0) :
        azimuth(azimuth),
        start(start),
        stop(stop)
        {
	...
	{ return azimuth == rhs.azimuth && start == rhs.start &&
		stop == rhs.stop; }
...
public:
    SegmentList() :
        ...
        span(0),
        cellCount(0),
        peakPower(std::numeric_limits<VideoT>::min())
        {
        }
    ...
    void pop(const SegmentList &other)
        {
            ...
            for(index=other.segments.begin(); index!=stop; index++)
            {
                pop(*index);
            }
        }
...
public:
    ...
    SegmentMessage()
	: Header("FromFile", GetMetaTypeInfo(), Header::Ref()),
	  list(new SegmentList), rangeMin_(), rangeFactor_() {}

    SegmentMessage(const std::string& producer, const Header::Ref& basis,
                   double rangeMin, double rangeFactor) :
        Header(producer, GetMetaTypeInfo(), basis), list(new SegmentList()),
	rangeMin_(rangeMin), rangeFactor_(rangeFactor) {}
    
    // Takes control of the SegmentList (i.e. deletes on destruction)
    SegmentMessage(const std::string& producer, const Header::Ref& basis,
		   SegmentList *l, double rangeMin, double rangeFactor) :
        Header(producer, GetMetaTypeInfo(), basis), list(l),
	rangeMin_(rangeMin), rangeFactor_(rangeFactor) {}
    ...
    double getRangeAt(size_t gateIndex) const
	{ return rangeFactor_ * gateIndex + rangeMin_; }
