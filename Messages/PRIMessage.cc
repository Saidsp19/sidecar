...
{
    return os << " Desc: " << std::hex << msgDesc << std::dec
	      << " TimeStamp: " << timeStamp
	      << " Seq#: " << sequenceCounter
	      << " Shaft: " << shaftEncoding
	      << " IRIG: " << irigTime;
...
PRIMessage::LoadV1(PRIMessage* obj, ACE_InputCDR& cdr)
{
    ...
    riuInfo.msgDesc = (VMEHeader::kPackedReal << 1) |
	VMEHeader::kAzimuthValidMask | VMEHeader::kPRIValidMask;
    riuInfo.timeStamp = 0;
    riuInfo.sequenceCounter = ++priCounter;
    riuInfo.shaftEncoding = static_cast<uint32_t>(
	azimuthStart / (2 * M_PI) * RadarConfig::GetShaftEncodingMax());
...
{
    if (data.size()) {
	cdr.write_char_array(&data[0], data.size());
    }
}
...
{
    if (data.size()) {
	cdr.write_short_array(&data[0], data.size());
    }
}
...
{
    Traits::Int16::Type operator()(const QString& v) const
	{ return v.toShort(); }
};
...
{
    if (data.size()) {
	cdr.write_short_array(reinterpret_cast<const int16_t*>(&data[0]), data.size() * 2);
    }
}
...
{
    if (data.size()) {
	cdr.write_float_array(&data[0], data.size());
    }
}
...
{
    if (data.size()) {
	cdr.write_double_array(&data[0], data.size());
    }
}
