...
protected:
    ...
    */
    static const CircularBuffer* Get(const std::string& name, const Messages::MetaTypeInfo& typeInfo);
...
private:
    ...
    */
    MessageRef get(size_t index) const { return buffer_[index == buffer_.size() ? 0 : index]; }
...
public:
    using Super = std::iterator<std::random_access_iterator_tag, Messages::PRIMessage::Ref, std::ptrdiff_t,
                                Messages::PRIMessage::Ref*, Messages::PRIMessage::Ref&>;
    ...
    /** Determine if this iterator is the same as another one.
        ...
        \return true if so
    */
    bool operator==(const CircularBufferIterator& rhs) const { return buffer_ == rhs.buffer_ && index_ == rhs.index_; }
    ...
    /** Determine if this iterator is not the same as another one
        ...
        \return true if so
    */
    bool operator!=(const CircularBufferIterator& rhs) const { return !operator==(rhs); }
...
protected:
    ...
    */
    CircularBufferIterator(const CircularBuffer* buffer, int index) : buffer_(buffer), index_(index) {}
    ...
    */
    Messages::PRIMessage::Ref operator*() const { return buffer_->get(index_); }
    ...
    */
    Messages::PRIMessage* operator->() const { return buffer_->get(index_).get(); }
...
public:
    ...
    */
    TCircularBufferIterator(const TCircularBufferIterator& it) : Super(it) {}
    ...
    */
    ...
    {
        TCircularBufferIterator tmp(*this);
        increment();
        return tmp;
    }
    ...
    {
        Super::moveToNewer(value);
        return *this;
    }

    TCircularBufferIterator operator+(int value) const { return TCircularBufferIterator(*this) += value; }
    ...
    */
    typename T::Ref operator*() const { return boost::dynamic_pointer_cast<T>(Super::operator*()); }
...
public:
    ...
    */
    ...
    /** Obtain iterator to the newest element in the buffer. NOTE: if another thread may add to the the buffer,
        ...
        \return iterator
    */
    TCircularBufferIterator<T> newest() const { return TCircularBufferIterator<T>(Super::newest()); }
    ...
    /** Obtain iterator to the oldest element in the buffer. NOTE: if another thread may add to the the buffer,
        ...
        \return iterator
    */
    TCircularBufferIterator<T> oldest() const { return TCircularBufferIterator<T>(Super::oldest()); }
