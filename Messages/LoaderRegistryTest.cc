...
ACE_InputCDR& Test::loaderV1(Test*, ACE_InputCDR& cdr)
{ called_ = 1; return cdr; }

ACE_InputCDR& Test::loaderV3(Test*, ACE_InputCDR& cdr)
{ called_ = 3; return cdr; }

ACE_InputCDR& Test::loaderV4(Test*, ACE_InputCDR& cdr)
{ called_ = 4; return cdr; }

ACE_InputCDR& Test::loaderV8(Test*, ACE_InputCDR& cdr)
{ called_ = 8; return cdr; }
...
Test::test()
{
	...
	TLoaderRegistry<Test>::VersionedLoader loaders[] = {
	    TLoaderRegistry<Test>::VersionedLoader(8, &Test::loaderV8),
...

	{
	    ...
	    ACE_InputCDR is(os); called_ = -1;
	    {
		// Create input stream from output stream, and check that the correct loader procedure is
		// invoked.
		//
		lr.load(this, is); // 1
		assertEqual(1, called_); called_ = -1;
		lr.load(this, is); // 2
		assertEqual(3, called_); called_ = -1;
		lr.load(this, is); // 3
		assertEqual(3, called_); called_ = -1;
		lr.load(this, is); // 4
		assertEqual(4, called_); called_ = -1;
		lr.load(this, is); // 5
		assertEqual(8, called_); called_ = -1;
		lr.load(this, is); // 6
		assertEqual(8, called_); called_ = -1;
		lr.load(this, is); // 7
		assertEqual(8, called_); called_ = -1;
