...
MetaTypeInfo BugPlot::metaTypeInfo_(MetaTypeInfo::Value::kBugPlot, "BugPlot", &BugPlot::CDRLoader, &BugPlot::XMLLoader);
...
BugPlot::Ref
BugPlot::Make(const std::string& producer, double when, double range, double azimuth, double elevation,
...
BugPlot::BugPlot() : Header(GetMetaTypeInfo())
{
    ;
}

BugPlot::BugPlot(const std::string& producer) : Header(producer, GetMetaTypeInfo())
{
    ;
}

BugPlot::BugPlot(const std::string& producer, double when, double range, double azimuth, double elevation,
                 const std::string& tag) :
    Header(producer, GetMetaTypeInfo()),
    when_(when), range_(range), azimuth_(azimuth), elevation_(elevation), tag_(tag)
{
    Logger::ProcLog log("BugPlot", Log());
    LOGINFO << "range: " << range << " azimuth: " << azimuth << " elevation: " << elevation << " tag: " << tag
            << std::endl;
}
...
{
    return os << "When: " << when_ << " Range: " << range_ << " Azimuth: " << azimuth_ << " Elevation: " << elevation_
              << " Tag: " << tag_;
}
...
BugPlot::printDataXML(std::ostream& os) const
{
    return os << "<plot tag=\"" << tag_ << "\" when=\"" << when_ << "\" range=\"" << getRange() << "\" azimuth=\""
              << Utils::radiansToDegrees(getAzimuth()) << "\" elevation=\"" << getElevation() << "\" />";
}
