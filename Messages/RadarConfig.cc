...
RadarConfig::Initializer::setConfigurationFilePath(const std::string& path)
{
    ...
    if (! radarConfigFileWatcher_) {
        radarConfigFileWatcher_ = new RadarConfigFileWatcher;
    }
...
const std::string&
RadarConfig::GetName() { return name_; }

uint32_t
RadarConfig::GetGateCountMax() { return gateCountMax_; }

uint32_t
RadarConfig::GetShaftEncodingMax() { return shaftEncodingMax_; }

double
RadarConfig::GetRotationRate() { return rotationRate_; }

double
RadarConfig::GetRangeMin_deprecated() { return rangeMin_; }

double
RadarConfig::GetRangeMax() { return rangeMax_; }

double
RadarConfig::GetRangeFactor_deprecated() { return rangeFactor_; }

double
RadarConfig::GetBeamWidth() { return beamWidth_; }
...
RadarConfig::Load(const QDomElement& config)
{
    ...
    if (rangeMin_ >= rangeMax_ || rangeMin_ < 0.0 || rangeMax_ <= 0.0) {
        LOGERROR << "invalid range specification - " << rangeMin_ << ","
                 << rangeMax_ << std::endl;
        return false;
    }
