...
XmlRpcClient::XmlRpcClient( const std::string& host, int port,
			    const std::string& uri )
    : _host( host ), _uri( uri ), _port( port )
{
    XmlRpcUtil::log( 1, "XmlRpcClient new client: host %s, port %d.",
		     host.c_str(), port );
    if ( ! uri.size() ) _uri = "/RPC2";
...
XmlRpcClient::XmlRpcClient( const std::string& host, int port )
    : _host( host ), _uri( "/RPC2" ), _port( port )
{
    XmlRpcUtil::log( 1, "XmlRpcClient new client: host %s, port %d.",
		     host.c_str(), port );
    _connectionState = NO_CONNECTION;
...
XmlRpcClient::doConnect()
{
  ...
  {
    this->close();
    XmlRpcUtil::error("Error in XmlRpcClient::doConnect: Could not connect to server %s/%d (%s).", _host.c_str(), _port, XmlRpcSocket::getErrorMsg().c_str());
    return false;
  }
...
XmlRpcClient::parseResponse(XmlRpcValue& result)
{
  ...
  if ((XmlRpcUtil::nextTagIs(PARAMS_TAG,_response,&offset) &&
      ...
      XmlRpcUtil::nextTagIs(FAULT_TAG,_response,&offset))
  {
      _isFault = true;
