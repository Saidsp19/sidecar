...
again:
	...
	else 
	{
	    ...
	    tv.tv_usec = ((int)floor(1000000.0 * (timeout-floor(timeout)))) %
		1000000;
	    std::clog << tv.tv_sec << ' ' << tv.tv_usec << std::endl;
	...
	if (nEvents < 0)
	{
	    ...
	    XmlRpcUtil::error("Error in XmlRpcDispatch::work: error in select (%d).",
			      errno );
	    _inWork = false;
	    return;
	}
