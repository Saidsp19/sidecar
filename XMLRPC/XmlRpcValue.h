...
public:
    ...
    XmlRpcValue(ValueArray* valueArray) : _type(TypeArray)
	{ _value.asArray = valueArray; }

    XmlRpcValue(ValueStruct* valueStruct) : _type(TypeStruct)
	{ _value.asStruct = valueStruct; }
    ...
    operator ValueArray&()     { assertTypeOrInvalid(TypeArray); return *_value.asArray; }
    ...
    operator const ValueArray&() const { assertType(TypeArray); return *_value.asArray; }
    operator const ValueStruct&() const { assertType(TypeStruct); return *_value.asStruct; }

    void push_back(const XmlRpcValue& value)
	{ assertArray(size() + 1); _value.asArray->at(size() - 1) = value; }
