...
operator<<(std::ostream& os, const timeval& when)
{
    ...
    return os << buffer << '.' << std::setfill('0') << std::setw(2)
	      << subs << std::setfill(' ');
}
...
Pattern::format(std::ostream& os, const Msg& msg) throw(std::out_of_range)
{
	...
	if (c != '%') {
		...
		//
		seconds = msg.when_.tv_sec;
		::strftime(buf, sizeof(buf), timeFormat.c_str(),
                           ::localtime(&seconds));
		os << buf;
		break;
		...
		//
	    case 'S': os << msg.when_.tv_sec; break;

		// Emit timestamp microseconds values
		//
	    case 'M': os << msg.when_.tv_usec; break;

		// Emit msg channel
		//
	    case 'c': os << msg.channel_; break;

		// Emit msg priority numeric value
		//
	    case 'l': os << msg.level_; break;

		// Emit msg priority name
		//
	    case 'p': os << Priority::GetShortName(msg.level_); break;

		// Emit msg priority value
		//
	    case 'P': os << Priority::GetLongName(msg.level_); break;

		// Emit msg message value
		//
	    case 'm': os << msg.message_; break;
