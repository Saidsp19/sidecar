...
Configurator::MissingPriorityName::MissingPriorityName(const Log& log)
    ...
    : ConfiguratorException<MissingPriorityName>("setPriorityLimit", "failed to read priority value for log ",
                                                 log)
{
    ;
}
...
Configurator::MissingFormatterPattern::MissingFormatterPattern(const Log& log)
    : ConfiguratorException<MissingFormatterPattern>("makeFormatter",
                                                     "failed to read formatting pattern for log ", log)
{
    ;
}

Configurator::InvalidStreamFlag::InvalidStreamFlag(const Log& log, const std::string& flag)
    : ConfiguratorException<InvalidStreamFlag>("makeStreamWriter", "invalid StreamWriter flag found for log ",
                                               log)
{
    *this << " - " << flag;
}
...
Configurator::InvalidRemoteSyslogFlag::InvalidRemoteSyslogFlag(const Log& log, const std::string& flag)
    : ConfiguratorException<InvalidRemoteSyslogFlag>("makeRemoteSyslogWriter",
                                                     "invalid remote syslog flag for log ", log)
{
    *this << " - " << flag;
}
...
Configurator::load(std::istream& is)
{
	...
	std::string what;
	while ((iss >> what)) {
	    if (what == "priority") setPriorityLimit(iss, log);
	    else if (what == "propagate") log.setPropagate(true);
	    else if (what == "writer") addWriter(iss, log);
	    else {
...
Configurator::readQuotedString(std::istream& is) const
{
    ...
    //
    if (delim != ' ' && c != delim)
	throw UnterminatedString(currentLine_);
