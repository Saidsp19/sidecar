...
LogMap::LogMap()
    : map_(), lock_(Threading::Mutex::Make()), newLogSignal_()
{
    ;
}
...
LogMap::getNames()
{
    ...
    std::for_each(map_.begin(), map_.end(),
                  [&names](auto& v){if (! v.second->isHidden()) names.push_back(v.first);});
    return names;
}
...
extern "C" {
    static void InitializeStub() { RuntimeData::Initialize(); }
    static void DestroyPerThreadInfoStub(void* obj) { delete static_cast<PerThreadInfo*>(obj); }
}
...
RuntimeData::RuntimeData()
    : logMap_(), clock_(SystemClockSource::Make()), perThreadInfoKey_()
{
...
Log::MakeObj(const std::string& name, const std::string& fullName, Log* parent, bool hidden)
{
	...
	if (! obj) {
	    Utils::Exception ex("Log::FetchObj: failed to insert new Log " "object for key: ");
	    ex << fullName;
...
{
    if (parent) {
	maxPriorityLimit_ = std::max(priority, parent->getMaxPriorityLimit());
    }
}
...
Log::~Log()
{
	...
	if (pos != parent_->children_.end()) {
	    parent_->children_.erase(pos);
        }
    }
}
...
Log::setPriorityLimit(Priority::Level priority)
{
    ...
    if (parent_) {
	priority = parent_->getMaxPriorityLimit();
    }
...
ProcLog::ProcLog(const char* name, Log& log)
    : log_(Log::Find(Log::MakeFullName(log.fullName(), name), false))
{
    ;
}

ProcLog::ProcLog(const std::string& name, Log& log)
    : log_(Log::Find(Log::MakeFullName(log.fullName(), name), false))
{
    ;
}
